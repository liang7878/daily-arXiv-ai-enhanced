...
### [1] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 本文介绍了用于评估AI生成材料科学内容的基准数据集HalluMatData和多阶段幻觉检测框架HalluMatDetector，降低了幻觉率并引入新评分指标。
...
Method: 引入HalluMatData数据集，提出HalluMatDetector多阶段检测框架，还引入Paraphrased Hallucination Consistency Score（PHCS）。

Result: 发现材料科学子领域幻觉水平差异大，高熵查询事实不一致性更大；使用检测框架使幻觉率比标准大语言模型输出降低30%。

Conclusion: 所提出的数据集、检测框架和评分指标有助于检测和减轻大语言模型的幻觉，深入了解模型可靠性。
...
TL;DR: 提出双向RAG架构，可安全扩展语料库，在多数据集实验中表现优于标准RAG和简单回写方法，证明严谨验证下自改进RAG可行。


<details>
  <summary>Details</summary>
Motivation: 传统RAG架构使用静态语料库，无法从用户交互中进化，需要改进。

Method: 引入Bidirectional RAG架构，用多阶段接受层结合接地验证等方法防止幻觉污染并实现知识积累。

Result: 在四个数据集、三次随机种子共12次实验中，Bidirectional RAG平均覆盖率达40.58%，近于标准RAG的两倍，添加文档比简单回写少72%。

Conclusion: 严谨验证下自改进RAG是可行且安全的，为从部署中学习的RAG系统提供了实际路径。
...
TL;DR: 研究大语言模型在无明确提示下的说服情况，发现监督微调会使模型更易产生说服倾向，且在良性话题数据集微调后对争议和有害话题也有说服倾向。


<details>
  <summary>Details</summary>
Motivation: 以往研究关注模型在恶意使用下的说服情况，本文旨在探究模型在无明确提示下的说服情况及相关风险。

Method: 研究两种场景下的无提示说服，一是通过内部激活引导模型展现人物特质，二是对模型进行监督微调使其展现相同特质。

Result: 引导特质不一定增加模型无提示说服倾向，监督微调会增加；在良性话题数据集上微调的模型对争议和有害话题也有较高说服倾向。

Conclusion: 模型存在无提示的有害说服风险，需要进一步研究。
...
TL;DR: 提出GamiBench基准测试MLLMs的空间推理和2D到3D规划能力，实验显示领先模型在单步空间理解上有困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准大多关注静态图像或最终输出，未考虑空间推理的顺序性和视角依赖性，需新基准评估MLLMs空间推理能力。

Method: 引入GamiBench基准，包含规则和不可能的2D折痕模式及对应3D折叠形状，通过三个VQA任务评估，引入新诊断指标。

Result: 实验表明，如GPT - 5和Gemini - 2.5 - Pro等领先模型在单步空间理解方面存在困难。
...
### [5] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: 现有RAG系统评估指标存在不足，本文提出DICE框架用于RAG评估，提高可解释性和鲁棒性，降低计算复杂度，验证显示其优于现有指标。


<details>
  <summary>Details</summary>
Motivation: 随着RAG系统架构日益复杂，现有标量指标在可解释性、不确定性量化和计算效率方面存在问题，阻碍RAG技术的负责任部署，因此需要新的评估方法。

Method: 提出DICE框架，结合深度分析推理与概率{A, B, Tie}评分，采用瑞士制锦标赛降低计算复杂度。

Result: 在八系统评估中计算复杂度降低42.9%，在中文金融QA数据集上与人类专家的一致性达85.7%，大幅优于现有基于大语言模型的指标。

Conclusion: DICE是一种可靠、可解释且高效的RAG系统评估范式。
...
### [6] [LLM Agents as VC investors: Predicting Startup Success via RolePlay-Based Collective Simulation](https://arxiv.org/abs/2512.22608)
*Zhongyang Liu,Haoyu Pei,Xiangyi Xiao,Xiaocong Du,Yihui Li,Suting Hong,Kunpeng Zhang,Haipeng Zhang*

Main category: cs.AI

TL;DR: 提出SimVC - CAS集体代理系统用于创业公司融资预测，提升了预测准确性并提供多视角推理。


<details>
  <summary>Details</summary>
Motivation: 现有创业公司成功预测方法多从单一决策者角度建模，忽略投资者群体集体动态，而现实中风险投资决策由投资者群体主导。

Method: 提出SimVC - CAS系统，设计角色扮演代理和基于GNN的监督交互模块，将创业公司融资预测转化为群体决策任务。

Result: 使用PitchBook真实数据，在严格控制数据泄露下，SimVC - CAS显著提高预测准确性，如平均精度@10相对提升约25%。

Conclusion: SimVC - CAS能提升预测准确性并提供可解释的多视角推理，也为其他复杂群体决策场景提供了思路。
...
TL;DR: 本文提出公平感知AI框架用于孟加拉国洪灾援助分配，用2022年洪灾数据，实验显示能降低偏差、公平差距，保证援助到最需地区。


<details>
  <summary>Details</summary>
Motivation: 发展中国家灾后援助分配存在系统偏差，不利弱势群体，本文旨在解决孟加拉国洪灾援助分配的公平性问题。

Method: 使用2022年孟加拉国洪灾数据，开发对抗性去偏模型，采用公平感知表征学习技术和梯度反转层。

Result: 该框架使统计奇偶差异减少41.6%，区域公平差距减少43.2%，保持较强预测准确性（R平方=0.784，基线为0.811）。

Conclusion: 算法公平技术能有效应用于人道主义领域，为决策者提供实施更公平灾难恢复策略的工具。
...
TL;DR: 文章介绍Agentic Risk & Capability (ARC) 框架，助组织应对代理式AI系统风险，该框架开源。


<details>
  <summary>Details</summary>
Motivation: 代理式AI系统有自主行动能力，带来机遇同时有新风险，对组织治理提出挑战，需有效应对。

Method: 引入Agentic Risk & Capability (ARC) 框架，从新视角分析系统，提炼风险源，建立风险与控制联系，提供实施方法。

Result: 框架能帮助组织应对代理式AI复杂性，实现创新同时确保安全、可靠部署。

Conclusion: ARC框架为组织应对代理式AI提供强大且灵活方法。
...
TL;DR: 研究通过实验发现人类难以可靠识别AI生成图像，凸显需提高认识和制定道德准则。


<details>
  <summary>Details</summary>
Motivation: 检验人们认为能轻易区分AI生成图像和真实照片的假设。

Method: 进行交互式网络实验，让参与者对20张图像进行真假分类，使用含120个困难案例的数据集，有165名用户参与233次测试。

Result: 用户平均准确率54%，略高于随机猜测，重复尝试提升有限；平均响应时间7.3秒，部分图像欺骗性更强。

Conclusion: 即使是简单肖像图像，人类也难可靠检测AI生成内容，随着合成媒体发展，仅靠人类判断不足以区分，需提高认识和制定伦理准则。
...
TL;DR: 用更强大模型生成的错误CoT合成数据训练语言模型，能提升其推理能力，研究揭示原因并验证。


<details>
  <summary>Details</summary>
Motivation: 探究提升语言模型推理能力的方法，验证错误CoT合成数据训练的效果。

Method: 用错误CoT合成数据训练模型并对比人类标注数据；用模型改写人类标注数据；引入不同缺陷的CoT数据研究模型容错性。

Result: 合成数据训练在推理任务上表现更好；改写人类标注数据能提升性能；明确模型对不同缺陷数据的容错性。

Conclusion: 构建接近模型分布的数据集很重要，最终答案正确不代表推理过程可靠。
...
TL;DR: 本文提出Logic Sketch Prompting (LSP) 框架，并与其他提示方法在三个开源模型上对比，发现LSP在两个药理学逻辑合规任务中表现最优，可用于关键决策支持系统。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要严格规则遵循、确定性和可审计性的任务上不可靠，因此需要一个新的提示框架。

Method: 提出LSP提示框架，使用两个药理学逻辑合规任务，对比LSP与零样本提示、思维链提示和简洁提示在三个开源模型（Gemma 2、Mistral和Llama 3）上的表现。

Result: 在两个任务和所有模型中，LSP始终具有最高的准确率（0.83 - 0.89）和F1分数（0.83 - 0.89），显著优于其他提示方法，McNemar测试显示LSP有统计显著提升。

Conclusion: LSP在不牺牲性能的情况下提高了确定性、可解释性和一致性，可应用于临床、受监管和安全关键的决策支持系统。
...
TL;DR: 介绍统一基准测试工具包SciEvalKit，用于评估跨多学科和任务能力的科学AI模型，开源且积极维护。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专注科学智能核心能力、跨多学科的AI模型评估工具，需要开发统一工具来评估下一代科学基础模型和智能体。

Method: 构建专家级科学基准，基于真实世界特定领域数据集；采用灵活、可扩展评估管道，支持批量评估、自定义模型和数据集集成。

Result: 创建了可进行跨模型和数据集批量评估、提供透明可重复可比结果的评估工具包。

Conclusion: SciEvalKit提供标准化且可定制的基础设施，有助于社区驱动的AI4Science发展。
...
TL;DR: 研究如何确保AI系统符合人类价值观并保持安全，通过AI协助和AI关闭游戏框架，指出应对挑战需能在不确定下推理、处理不完整和非阿基米德偏好的AI。


<details>
  <summary>Details</summary>
Motivation: 探讨如何确保AI系统与人类价值观一致且安全。

Method: 通过AI协助和AI关闭游戏的框架来研究问题。

Result: 应对AI协助和关闭问题的挑战，需要能在不确定下推理、处理不完整和非阿基米德偏好的AI。

Conclusion: 解决AI系统安全和价值对齐问题需特定推理和偏好处理能力的AI。
...
TL;DR: 提出Agent2World框架用于世界模型生成，在推理和训练上表现出色。


<details>
  <summary>Details</summary>
Motivation: 训练大语言模型生成世界模型缺乏大规模可验证监督，现有静态验证方法无法捕捉交互执行中的行为级错误。

Method: 提出Agent2World框架，采用三阶段流水线，包括Deep Researcher进行知识综合、Model Developer实现可执行模型、Testing Team进行测试和验证。

Result: 在三个基准测试中推理性能优越，经多轮训练后模型生成世界模型平均相对提升30.95%。

Conclusion: Agent2World框架能有效实现推理时世界模型生成，还可作为监督微调的数据引擎。
...
TL;DR: 本文针对含控制参数的数值规划问题，识别出易处理子集并提出乐观编译方法，使传统数值启发式可应用于有无限可能动作的场景。


<details>
  <summary>Details</summary>
Motivation: 含控制参数的数值规划会产生无限个适用动作，现成的利用动作结构的数值启发式不可行。

Method: 识别出可控的简单数值问题子集，将控制相关表达式抽象为有界常量效果和宽松前提条件，把问题转化为简单数值任务。

Result: 所提方法能有效使用子目标启发式估计含控制参数数值规划问题的目标距离。

Conclusion: 该方法是将传统数值启发式应用于有无限可能动作场景的有效且计算可行的方式，推动了技术发展。
...
TL;DR: 提出GatedBias框架，对知识图谱基础模型进行个性化调整，参数高效且因果可验证，连接通用知识表示与用户需求。


<details>
  <summary>Details</summary>
Motivation: 知识图谱基础模型在链接预测中能实现群体级表现，但无法捕捉用户个人偏好，存在通用关系推理与个性化排序的脱节。

Method: 提出GatedBias框架，采用结构门控适应，将特定于用户画像的特征与图衍生的二进制门结合，产生可解释的每个实体的偏差，仅需约300个可训练参数。

Result: 在两个基准数据集上评估，显著改善对齐指标，同时保留群体表现；反事实扰动实验验证了因果响应性，特定偏好信号增强时，受益实体排名提升6 - 30倍。

Conclusion: 知识图谱基础模型的个性化调整可以是参数高效且因果可验证的，能连接通用知识表示与用户个人需求。
...
TL;DR: 论文提出了Monadic Context Engineering (MCE) 架构范式，用于设计AI智能体，还扩展到Meta - Agents。


<details>
  <summary>Details</summary>
Motivation: 当前智能体架构常采用命令式、临时模式，导致系统在状态管理、错误处理和并发方面存在问题，需要新的设计范式。

Method: 引入MCE利用Functors、Applicative Functors和Monads的代数结构，将智能体工作流视为计算上下文，处理跨领域问题，并通过Monad、Applicatives和Monad Transformers实现不同功能。

Result: 该分层方法可让开发者用简单、可独立验证的组件构建复杂、有弹性和高效的AI智能体，还将其扩展到Meta - Agents。

Conclusion: MCE为AI智能体设计提供了形式化基础，有助于构建复杂智能体。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全基准难以捕捉构成操纵的细微心理和社会机制，大语言模型的操纵或欺骗行为引发担忧。

Method: 引入DarkPatterns - LLM，采用四层分析管道（MGD、MSIAN、THP、DCRA），数据集含401个精心策划示例及专家注释。

Result: 评估模型存在显著性能差异（65.2% - 89.7%），检测破坏自主性模式有一致弱点。

Conclusion: DarkPatterns - LLM为大语言模型操纵检测建立首个标准化、多维度基准，有助于构建更可信AI系统。
...
TL;DR: 本文利用‘人在回路’闭环框架研究铝纳米颗粒氧化机制，揭示温度调控的双模氧化机制，解决数十年争议，为高能纳米材料设计提供原子尺度框架。


<details>
  <summary>Details</summary>
Motivation: 铝纳米颗粒是高能量密度固体燃料，但从钝化颗粒到爆炸反应物的原子机制不明，现有计算方法存在瓶颈。

Method: 采用‘人在回路’闭环框架，让自审计AI代理验证机器学习势的演化。

Result: 模拟揭示温度调控的双模氧化机制，解决铝纳米颗粒质量传输主导因素的争议。

Conclusion: 研究成果为高能纳米材料设计建立统一原子尺度框架，可通过智能计算设计精确调控点火敏感性和能量释放率。
...
TL;DR: 当前大模型基于最小化下一令牌预测损失，但忽视预测编码模型其他重要组件，文章建议融入行动、组合生成架构和情景记忆，以实现安全、可解释等特性的AI。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型忽略了预测编码模型中的三个重要组件，为实现安全、可解释、节能和类人的AI，提出改进建议。

Method: 提出在基础模型中集成行动、组合生成架构和情景记忆的建议，结合神经科学和认知科学的证据，并与当前趋势对比。

Result: 融入缺失组件可解决基础模型的一些不足，如幻觉、缺乏能动性、安全性和能效问题等。

Conclusion: 重振脑科学与AI间的思想交流将有助于实现安全、可解释的以人为中心的AI。
...
TL;DR: 本文提出用于无线网络的语义感知AgentNet架构SANet，将其优化问题建模为多智能体多目标问题，提出评估指标、MoPS框架和两种优化算法，推导理论边界并构建硬件原型，实验显示性能提升且计算量降低。


<details>
  <summary>Details</summary>
Motivation: AgentNet是去中心化框架，协作智能体可能有不同甚至冲突的目标，需进行去中心化优化。

Method: 将SANet的去中心化优化建模为多智能体多目标问题，寻找帕累托最优解；提出三个评估指标；开发MoPS框架；提出两种去中心化优化算法；推导理论边界；构建硬件原型。

Result: 实验结果表明，所提框架性能提升达14.61%，仅需现有算法44.37%的浮点运算量。

Conclusion: 所提出的SANet架构及相关方法在无线网络管理和优化方面有良好效果，能在提升性能的同时降低计算成本。
...
TL;DR: 提出用于智能生理医疗的Tyee工具包，有三项创新，实验效果好并已开源维护。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在生理信号分析中因数据格式、预处理策略、模型管道和实验设置带来的问题。

Method: 提出Tyee工具包，有统一数据接口和预处理管道、模块化可扩展架构、端到端工作流配置三项创新。

Result: 在所有评估任务中表现出一致的实用性和泛化性，13个数据集中12个达到了最先进的结果。

Conclusion: Tyee工具包能有效推动智能生理医疗领域发展，且已开源并维护。
...
TL;DR: 提出M³ob方法利用多模态时空知识进行位置推荐，实验表明该方法在正常和异常场景均有良好表现。


<details>
  <summary>Details</summary>
Motivation: 现有人类移动性预测方法泛化能力有限，单模态受数据稀疏和偏差限制，多模态难以捕捉移动动态。

Method: 构建统一的时空关系图（STRG）进行多模态表示，设计门控机制融合不同模态的时空图表示，提出STKG引导的跨模态对齐注入时空动态知识。

Result: 在六个公共数据集上的实验显示，该方法在正常场景有持续改进，在异常场景有显著泛化能力。

Conclusion: 所提出的M³ob方法能有效利用多模态时空知识进行位置推荐，具有良好的泛化能力。
...
TL;DR: 研究允许大语言模型互相审查预测能否提高准确性，对四种场景评估，干预在多样模型共享信息场景显著提升准确性，同质组无益处，额外信息未提升准确性。


<details>
  <summary>Details</summary>
Motivation: 受结构化审议可提升人类预测者表现启发，探究类似干预能否提高大语言模型预测准确性。

Method: 使用Metaculus Q2 2025 AI预测锦标赛的202个已解决二元问题，评估四种场景下大语言模型预测的准确性。

Result: 干预在多样模型共享信息场景显著提高准确性，相对降低Log Loss 4%；同质组无益处；额外信息未提升准确性。

Conclusion: 审议可能是提升大语言模型预测的可行策略。
...
TL;DR: 本文介绍了用于评估大语言模型（LLM）代理旅行规划能力的TravelBench基准，收集真实场景请求构建子集，在沙盒环境评估多个LLM并分析其表现。
...
Method: 引入TravelBench基准，收集真实用户请求构建三个子集，搭建含10个旅行领域工具的沙盒环境进行评估。

Result: 对多个LLM在TravelBench上进行评估并分析其行为和表现。

Conclusion: TravelBench为推进旅行规划中的LLM代理提供了实用且可复现的基准。
...
TL;DR: 提出大语言模型智能体持续体验学习理论框架，整合情景记忆与强化学习，证明框架收敛性。


<details>
  <summary>Details</summary>
Motivation: 突破传统训练与部署分离限制，让智能体在无需反向传播或模型微调的情况下通过交互实现自适应。

Method: 引入有状态反思决策过程，模拟与情景记忆的读写交互；用熵正则化策略迭代实例化框架。

Result: 该过程能诱导等效马尔可夫决策过程，结合记忆不断增长能覆盖状态空间使策略收敛到最优解。

Conclusion: 为无参数更新、基于记忆增强和检索的语言模型智能体的持续自适应提供了理论基础。
...
TL;DR: 提出用于多智能体投资组合管理的SAMP - HDRL框架，在多市场制度下回测表现优于传统和DRL基准，有良好特性和解释性。


<details>
  <summary>Details</summary>
Motivation: 非平稳市场中投资组合优化面临制度转变、动态相关性和DRL策略可解释性有限等挑战。

Method: 提出SAMP - HDRL框架，进行动态资产分组，利用上下层智能体决策，采用基于效用的资本分配机制。

Result: 在2019 - 2021年三个市场制度下的回测中，SAMP - HDRL在波动和震荡条件下始终优于多个传统和DRL基准，各项比率至少高2% - 5%，动荡市场收益更大。

Conclusion: SAMP - HDRL将市场结构约束嵌入DRL流程，在复杂金融环境中具有更好的适应性、鲁棒性和可解释性。
...
TL;DR: 介绍了分层基准HiSciBench用于评估基础模型在科学研究中的能力，评估显示领先模型在不同任务上有性能差距，该基准将公开。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准零散，大多聚焦狭窄任务，无法反映真实科学探究的层次和多学科性质。

Method: 引入HiSciBench，包含五个反映完整科学工作流程的层次，涵盖六大学科，支持多模态输入和跨语言评估。

Result: 对领先模型的综合评估显示，在基础素养任务上准确率达69%，在发现级挑战上降至25%。

Conclusion: HiSciBench为评估科学智能建立新标准，为开发更强大可靠的模型提供见解，并将公开促进未来研究。
...
TL;DR: 本文提出了名为Gamma的知识图谱基础模型，引入多头几何注意力机制，在多个知识图谱实验中表现优于Ultra。


<details>
  <summary>Details</summary>
Motivation: 现有方法如Ultra在消息传递中依赖单一关系转换，限制了表达能力且无法捕获多样化的图模式。

Method: 提出Gamma模型，用多种并行关系转换替代单一转换，使用关系条件注意力融合机制，融合不同基于数系的转换，并通过轻量级门控及熵正则化。

Result: 在56个知识图谱的实验中，Gamma在零样本归纳链接预测中始终优于Ultra，在归纳基准上平均倒数排名提高5.5%，在所有基准上提高4.4%。

Conclusion: Gamma模型通过互补的几何表示提高了表达能力，在知识图谱推理中有更好表现。
...
TL;DR: 提出高质量可解释数据集RW - Post和基于代理的多模态事实核查框架AgentFact，提升多模态事实核查的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态事实核查方法因推理能力有限和证据利用不足而效果不佳，缺乏包含推理过程和可验证证据的数据集。

Method: 引入数据集RW - Post，将多模态声明与社交媒体帖子对齐并包含推理和证据；提出AgentFact框架，由五个专门代理协作完成核查子任务，通过迭代工作流程进行证据搜索、过滤和推理。

Result: 实验表明RW - Post和AgentFact协同作用大幅提高了多模态事实核查的准确性和可解释性。

Conclusion: RW - Post数据集和AgentFact框架有效提升了多模态事实核查的性能。
...
TL;DR: 研究对比LLM和DKT模型在评估学习者知识方面表现，发现LLM难以替代成熟智能辅导系统，需混合框架。


<details>
  <summary>Details</summary>
Motivation: K - 12教育中基于LLM的辅导工具盛行，引发其可替代传统学习者建模的误解，且K - 12被欧盟列为高风险领域，需负责任设计。

Method: 使用大型开放数据集，对比DKT模型和广泛使用的LLM（零样本和微调情况）。

Result: DKT在下一步正确性预测上辨别性能最佳（AUC = 0.83），各设置下均优于LLM；微调使LLM的AUC提升约8%，仍低于DKT 6%且早期序列误差更高；DKT掌握度更新稳定正确，LLM有较大时间弱点；微调后LLM多技能掌握度估计轨迹不一致，DKT平滑连贯；微调LLM计算需求远超DKT。

Conclusion: 仅靠LLM难以达到成熟智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架。
...
TL;DR: 研究指出标准奖励模型准确率不适用于部署就绪的个性化对齐选择，引入政策准确率，新基准测试揭示歧视与生成能力脱节，小样本学习在大模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 部署时需推理时自适应而非每个用户策略微调，要求奖励模型不仅准确排名还要有效指导生成决策，但现有研究对这个需求关注不足。

Method: 通过三个数据集系统评估，引入政策准确率指标，提出Pref - LaMP个性化对齐基准测试，对比不同方法性能。

Result: 标准奖励模型准确率作为选择标准失效；政策准确率与奖励模型准确率相关性弱；在Pref - LaMP上歧视与生成能力解耦；大参数模型中小样本学习优于奖励引导方法。

Conclusion: 该领域优化的代理指标无法预测部署性能，不能将偏好转化为实际行为适应。
...
TL;DR: 本文提出ChexReason模型，评估发现GRPO虽提升分布内性能但降低跨数据集迁移能力，存在泛化悖论，指出临床部署中监督微调可能优于强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习在大语言模型推理任务的进展应用于医学影像资源受限，需探索相关方法。

Method: 采用R1风格方法（SFT后接GRPO）训练ChexReason模型，使用少量样本和单块A100 GPU。

Result: GRPO使CheXpert上分布内性能提升23%，但NIH上跨数据集迁移能力下降19%；SFT检查点在优化前能提升NIH表现；结构化推理支架对通用VLM有益，对医学预训练模型增益小。

Conclusion: 对于需要跨多样人群鲁棒性的临床部署，精心策划的监督微调可能优于激进的强化学习。
...
TL;DR: 指出DPO及其变体的局限，提出ISP - O解决问题，实验验证其能提升LLM效果


<details>
  <summary>Details</summary>
Motivation: 发现DPO及其变体存在最优策略依赖任意建模选择和未利用成对数据比较信息的局限，为改进模型对齐效果提出研究

Method: 提出Intrinsic Self - reflective Preference Optimization (ISP - O)，推导出基于上下文和替代响应的全局最优策略

Result: 实验显示在胜率和长度控制指标上有持续改进

Conclusion: 释放模型的自我反思能力可产生更鲁棒、更符合人类偏好的大语言模型
...
TL;DR: 现行评估AI情绪智力的框架需完善，本文回顾理论、评估框架，提出改进策略。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI情绪智力的框架不能充分全面衡量相关方面，且缺乏坚实的情绪本质基础。

Method: 先回顾情绪和一般情绪智力理论，评估其对人工系统的适用性，再批判性评估现有基准框架，最后提出改进策略。

Result: 指出不同基准框架在衡量AI情绪智力时的不足。

Conclusion: 提出了避免AI系统情绪智力评估中缺陷的改进策略选项。
...
TL;DR: 提出SPIRAL框架，将三个LLM代理认知架构嵌入MCTS循环，在数据集上表现优于其他方法，证明结构化LLM推理可产生更优规划器。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂规划任务中易因线性推理难从早期错误恢复，搜索算法在稀疏奖励下效果不佳且未充分利用LLM语义能力。

Method: 引入SPIRAL框架，将Planner、Simulator和Critic三个LLM代理嵌入MCTS循环，形成集成规划管道。

Result: 在DailyLifeAPIs和HuggingFace数据集上，SPIRAL超越默认思维链规划方法和其他先进代理，如在DailyLifeAPIs上准确率达83.6%，提高超16个百分点，且有更好的令牌效率。

Conclusion: 将LLM推理构建为引导、反思和有根据的搜索过程，可产生更强大高效的自主规划器。
...
TL;DR: 本文提出“模型置信度”概念，证明其在提取大语言模型生成数据信息上更高效，通过需求估计研究验证，建议将其作为默认度量。


<details>
  <summary>Details</summary>
Motivation: 现有使用大语言模型生成数据的方法效率低下，未充分利用模型概率特性中的信息。

Method: 引入并形式化“模型置信度”，证明其与模型选择均值的渐近等价性，通过需求估计研究验证性能。

Result: 模型置信度是更高效的估计量，方差更低、收敛更快；在有限运行次数下，能更好解释和预测真实模型选择，减少约20倍计算量。

Conclusion: 支持将模型置信度作为默认度量来提取大语言模型生成数据的更多信息。
...
TL;DR: 提出评估框架 TCEval 评估 AI 认知能力，实验显示其可行性及当前 LLMs 能力情况


<details>
  <summary>Details</summary>
Motivation: 弥补 LLM 任务特定基准缺失的问题，以热舒适场景评估 AI 现实认知能力

Method: 提出 TCEval 框架，为 LLM 代理初始化虚拟人格属性生成反馈，并与数据库对比验证

Result: 代理反馈与人类不完全一致，PMV 分布有差异，离散分类表现接近随机

Conclusion: TCEval 可行，当前 LLMs 有基础跨模态推理能力但缺乏精确因果理解，可补充传统基准
...
TL;DR: 当前通用AI在物理系统控制接口存在局限，提出以物理验证驱动策略优化的特定领域基础模型，在合成反应堆控制场景训练有良好效果且表示可迁移。


<details>
  <summary>Details</summary>
Motivation: 通用AI在物理系统控制接口面临基本障碍，现有前沿模型在基础物理任务上精度低，存在结构局限。

Method: 引入作为Agentic Physical AI的紧凑语言模型，以物理验证驱动策略优化，在合成反应堆控制场景训练3.6亿参数模型，扩大数据集。

Result: 引发通用模型中不存在的相变，大规模模型方差大幅降低，稳定执行行为，模型自主拒绝约70%训练分布，95%运行时执行集中在单库策略，学习表示可跨物理和输入模态迁移。

Conclusion: 基于物理验证的特定领域基础模型为物理系统AI提供了不同路径，具有良好的性能和迁移性。
...
TL;DR: 本文研究了一致性规划和超属性模型检查两个问题的联系，表明二者可相互转化。


<details>
  <summary>Details</summary>
Motivation: 研究规划和验证领域中一致性规划与超属性模型检查两个问题的关联。

Method: 1. 将超属性模型检查实例高效转化为一致性规划实例并证明编码的合理性和完备性；2. 证明每个一致性规划问题本身也是超属性模型检查任务。

Result: 证明了∃^*∀^*超属性的模型检查与计算一致性规划问题密切相关，二者可相互转化。

Conclusion: 一致性规划和超属性模型检查存在紧密联系，可以相互约化。
...
TL;DR: 论文指出大语言模型代理在现实世界部署有空间心智模型的挑战，引入CubeBench评估其能力，实验揭示关键局限并提出诊断框架指导开发。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型代理在物理世界部署中缺乏强大空间心智模型的问题，识别阻碍其部署的核心认知挑战。

Method: 引入以魔方为中心的新基准CubeBench，采用三层诊断框架评估代理能力，还提供外部求解工具隔离认知瓶颈。

Result: 在领先大语言模型上的实验揭示了关键局限，所有长视野任务通过率为0.00%，暴露长期规划的根本失败。

Conclusion: 通过分析失败模式，为开发更具物理基础的智能代理提供关键见解。
...
TL;DR: 介绍了TIR智能体MindWatcher，它集成交错思维和多模态CoT推理，能自主调用工具，通过实验证明其性能优越并发现训练关键见解。


<details>
  <summary>Details</summary>
Motivation: 传统基于工作流的智能体在解决需工具调用的现实问题时智能有限，TIR智能体可用于复杂决策任务。

Method: 引入交错思维和多模态CoT推理，实现自动数据审计和评估管道，构建MWE - Bench基准，配备辅助推理工具和本地图像检索数据库，设计高效训练基础设施。

Result: MindWatcher通过出色的工具调用达到或超越更大或更新模型的性能，发现智能体训练中的遗传继承现象。

Conclusion: MindWatcher是有效的TIR智能体，其设计和训练方法有优势，实验发现对智能体训练有启示。
...
TL;DR: 本文提出计算嵌入视角研究连续学习，引入交互性目标和算法，实验表明深度线性网络在容量增加时能维持更高的交互性。


<details>
  <summary>Details</summary>
Motivation: 现有连续学习中对智能体的显式约束有随意、难融入及限制扩展能力有效性等问题，需新的问题设定。

Method: 引入计算嵌入视角，将嵌入智能体表示为通用计算机内模拟的自动机；提出交互性目标；开发基于模型的强化学习算法并构建合成问题评估。

Result: 深度非线性网络难维持交互性，深度线性网络在容量增加时维持更高交互性。

Conclusion: 可为连续学习提供新的研究视角和评估方法，深度线性网络在连续学习中可能更具优势。
...
TL;DR: 提出AKG kernel agent自动化内核开发，支持多DSL，在GPU和NPU后端加速内核开发。


<details>
  <summary>Details</summary>
Motivation: 现代AI模型对计算内核性能要求高，手动优化难以满足需求，LLM代码生成能力带来自动化开发新可能。

Method: 提出AKG kernel agent多智能体系统，支持多DSL，模块化设计便于集成新DSL和硬件目标。

Result: 在KernelBench上使用Triton DSL，在GPU和NPU后端较PyTorch Eager基线实现平均加速1.46倍。

Conclusion: AKG kernel agent能有效加速现代AI工作负载的内核开发。
...
TL;DR: 提出Hindsight instruction Replay (HiR)框架用于复杂指令跟随任务，用选择重写策略处理失败尝试，实验表明效果好且计算成本低。


<details>
  <summary>Details</summary>
Motivation: 强化学习在大语言模型指令跟随中依赖高质量响应采样，但初始模型能力有限，奖励稀疏或难以区分阻碍学习。

Method: 提出HiR框架，采用选择重写策略将失败尝试作为成功进行重放，在重放样本和原始样本上进行强化学习，理论上构建目标为指令和响应级别的双偏好学习。

Result: 在不同指令跟随任务中取得有前景的结果，且所需计算预算更少。
...
TL;DR: 提出MIRAGE - VC框架用于风险投资预测，解决路径爆炸和异构证据融合问题，提升性能并为其他离图预测任务提供启示。


<details>
  <summary>Details</summary>
Motivation: 传统方法缺乏推理能力，现有图 - 大语言模型方法不适用离图的风险投资预测，需有效方法综合复杂关系证据预测初创企业成功。

Method: 提出多视角检索增强生成框架MIRAGE - VC，使用信息增益驱动的路径检索器筛选邻居，用多智能体架构融合证据。

Result: 在严格防泄漏控制下，MIRAGE - VC的F1值提升5.0%，PrecisionAt5提升16.6%。

Conclusion: MIRAGE - VC能有效解决风险投资预测问题，也为推荐和风险评估等离图预测任务提供思路。
...
TL;DR: 针对大语言模型生成教育问题缺乏多样性的问题，提出CreativeDC方法，评估显示其在多样性、新颖性上表现更好。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成教育问题存在“人工蜂群”效应，导致输出同质化，危害学生思维多样性。

Method: 借鉴Wallas创造力理论和Guilford发散 - 收敛思维框架，提出CreativeDC两阶段提示方法，将创意探索与约束满足分离。

Result: 通过综合指标评估，CreativeDC在多样性和新颖性上显著高于基线方法，同时保持高实用性；抽样分析显示其生成不同问题的有效数量增加更快。

Conclusion: CreativeDC方法能有效提高大语言模型生成教育问题的多样性和新颖性。
...
TL;DR: 提出NeuroSPICE，一种用于器件和电路仿真的PINN框架，虽训练时性能不如SPICE，但有独特优势，可模拟新兴器件。


<details>
  <summary>Details</summary>
Motivation: 开发新的器件和电路仿真框架，解决传统SPICE的局限，满足新兴器件仿真需求。

Method: 利用物理信息神经网络（PINN），通过反向传播最小化电路微分代数方程（DAEs）的残差来求解方程，在时域用解析方程建模波形。

Result: NeuroSPICE在训练时速度和精度不如SPICE，但可作为替代模型用于设计优化和逆问题，能模拟新兴器件。

Conclusion: NeuroSPICE具有灵活性，可用于新兴器件包括强非线性系统的仿真。
...
TL;DR: 本文提出I - PERI算法解决未知客户端干预下的联邦因果发现问题，给出理论保证并进行实证评估。
...
Method: 提出I - PERI算法，先恢复客户端图并集的CPDAG，再利用干预导致的结构差异定向额外边，得到更紧密的Φ - Markov等价类。

Result: 为I - PERI算法的收敛性和隐私保护特性提供理论保证，在合成数据上的实证评估证明算法有效。

Conclusion: I - PERI算法能有效解决未知客户端干预下的联邦因果发现问题。
...
TL;DR: 本文介绍Web World Model（WWM），构建多种基于现实Web堆栈的WWM并提出设计原则，表明Web堆栈可作为世界模型的可扩展基础。


<details>
  <summary>Details</summary>
Motivation: 现有语言智能体持久世界的方法存在极端问题，常规Web框架固定，全生成世界模型缺乏可控性和工程实用性。

Method: 引入WWM，在普通Web代码中实现世界状态和‘物理规则’，在结构化潜在状态之上使用大语言模型生成上下文、叙述和高级决策。

Result: 构建了包括无限旅行地图、银河探索、百科与叙事世界以及模拟和游戏环境等多种WWM系统。

Conclusion: Web堆栈可作为世界模型的可扩展基础，实现可控且开放式的环境。
...
TL;DR: 提出正交圆柱相交体积和表面积的积分表达式及近似函数，经模拟验证准确


<details>
  <summary>Details</summary>
Motivation: 解决正交圆柱部分相交（任意深度比）时需数值方法或近似求解的问题

Method: 推导一般积分表达式，给出经验近似函数

Result: 得到相交体积和表面积与相交深度的显式函数，近似函数相对误差低于15%

Conclusion: 解析解和近似解经模拟验证具有准确性
...
TL;DR: 用无采样侵入式随机伽辽金法求解声波传播问题，处理随机参数对应系统的计算成本，展示了算法的高效扩展性。


<details>
  <summary>Details</summary>
Motivation: 解决声波传播问题在处理具有对数正态随机场近似波速时的计算问题，特别是随着网格大小、时间步长和随机参数数量增加带来的巨大计算成本。

Method: 使用无采样侵入式随机伽辽金法，将随机偏微分方程通过多项式混沌展开转化为确定性偏微分方程和线性方程组，采用基于区域分解的求解器，并应用带两级Neumann - Neumann预处理器的共轭梯度迭代求解器。

Result: 采用的求解器展示了高效的扩展性。

Conclusion: 所采用的方法能够有效应对相关声波传播问题的计算挑战，尤其是处理计算成本高的情况。
...
TL;DR: 研究学习型基数估计技术违背单调性问题，提出MonoM指标和单调训练框架，实验表明该算法提升单调性和估计准确性。


<details>
  <summary>Details</summary>
Motivation: 学习型基数估计技术在生产系统应用中存在违背单调性等基本逻辑原则的问题。

Method: 提出MonoM指标定量衡量基数估计器对单调性的遵循度；提出包含查询生成器和损失函数正则化项的单调训练框架。

Result: 单调训练算法既增强了对单调性的遵循，又提高了基数估计的准确性。

Conclusion: 正则化项减少了过拟合，提高了模型泛化能力，带来了性能提升。
...
TL;DR: 本文首次系统评估大语言模型生成SQL查询的云计算成本，得出推理模型性能更好、执行时间与查询成本相关性弱、模型成本差异大等结论，并给出企业部署建议。


<details>
  <summary>Details</summary>
Motivation: 现有效率指标衡量的是执行时间而非云数据仓库基于消耗的成本，因此需对大语言模型生成SQL查询的云计算成本进行系统评估。

Method: 在Google BigQuery上使用StackOverflow数据集对六个最先进的大语言模型进行180次查询执行，测量处理字节数、插槽利用率和估计成本。

Result: 推理模型比标准模型少处理44.5%的字节、保持等效的正确性；执行时间与查询成本相关性弱；模型成本存在3.4倍差异，标准模型每个查询处理超过36GB的情况有异常值。

Conclusion: 识别出查询的低效模式，为对成本敏感的企业环境提供部署指南。
...
TL;DR: 提出CTA的参数高效框架，通过LoRA在提示增强数据上训练模型，减少对提示变化的敏感性，实验显示该策略有效。


<details>
  <summary>Details</summary>
Motivation: 现有编码器语言模型在跨领域应用有限，提示生成式大模型有对提示敏感和F1分数不高的问题，全量微调大模型成本高且未消除对提示的敏感性。

Method: 提出参数高效框架，通过低秩适应（LoRA）在提示增强数据上训练模型。

Result: 微调模型在推理时能在不同提示模式下保持稳定性能，加权F1分数更高。

Conclusion: 参数高效训练和增强策略在开发实用、适应性强的CTA系统中有效。
...
TL;DR: 提出OrchANN引擎解决十亿级近似最近邻搜索的核外问题，性能优于多个基线系统。


<details>
  <summary>Details</summary>
Motivation: 现有核外系统在偏态语义嵌入下失效，性能受I/O主导。

Method: 采用I/O编排模型统一I/O治理，为每个集群选择异构本地索引，维护查询感知内存导航图，应用多级剪枝。

Result: 在五个标准数据集上，OrchANN在QPS和延迟方面优于四个基线系统，减少SSD访问。

Conclusion: OrchANN在不牺牲准确性的前提下，比竞争系统有更高QPS和更低延迟。
...
TL;DR: 本文针对受公交路网与时刻表约束的路线规划问题，提出改进图结构，先给出暴力算法，后提出EA - Star算法，实验验证了方法有效性。


<details>
  <summary>Details</summary>
Motivation: 解决考虑公交时刻表的路线规划问题，现有方法因公交网络复杂难以有效求解。

Method: 先提出改进图结构表示公交网络，使用基于Dijkstra原理的暴力算法，后引入EA - Star算法计算最短路线，计算时用A*算法缩小搜索空间。

Result: 通过纽约公交网络数据集实验验证了方法的有效性。

Conclusion: 提出的方法能有效在公交网络中规划出访问指定兴趣点的最短时间路线。
...
TL;DR: 本文对四十年间的缓冲区管理研究进行全面综述，分析算法演进、架构差异、生产系统实现和新兴趋势，确定关键模式、权衡和挑战，并给出研究方向。


<details>
  <summary>Details</summary>
Motivation: 缓冲区管理对数据库和操作系统性能至关重要，为填补CPU处理速度和存储访问时间的持久延迟差距，需对其进行全面研究。

Method: 系统分析从基础算法到当代机器学习增强策略和分离式内存架构的演进，研究历史上OS - DBMS架构差异、生产系统实现及新兴趋势，分析超50篇顶级会议论文。

Result: 确定了关键架构模式、性能权衡和开放的研究挑战。

Conclusion: 提出将机器学习与内核可扩展机制集成，为现代数据库系统的异构内存层次结构实现自适应、跨层缓冲区管理的研究方向。
...
TL;DR: 提出ChronoConnect系统用于追踪时间图中的时间路径，支持多种时间遍历算法，利用并行处理，有可视化界面，能有效分析信息传播。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态快照的图分析系统难以在时间维度上有效捕捉信息流，需分析时间图演化中的信息传播模式。

Method: 引入ChronoConnect系统，支持用户配置和执行多种时间遍历算法，利用并行处理演化图的增长问题，提供交互式用户界面。

Result: 通过追踪时间图中高动态顶点路径的算法实现，展示了ChronoConnect的有效性和性能提升。

Conclusion: ChronoConnect有潜力成为用户研究信息在时间图上传播的强大工具。
...
TL;DR: 文章首次探索批量处理多个R$k$NN查询，提出BR$k$NN - Light算法，实验证明其在批量查询处理上的优越性。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视同时处理多个R$k$NN查询，无法共享冗余计算以降低成本，本文旨在解决此问题。

Method: 提出BR$k$NN - Light算法，采用基于几何约束的快速验证和剪枝策略、优化的范围搜索技术，还提出动态距离缓存机制。

Result: 在多个真实道路网络上的实验表明BR$k$NN - Light算法在批量查询处理上表现优越。

Conclusion: BR$k$NN - Light算法能有效处理批量R$k$NN查询，减少不必要计算。
...
TL;DR: 为解决大语言模型在旅游路线规划中无法生成最优方案的问题，本文提出 KATR 查询和探索 - 边界范式，实验表明该方法性能更优。


<details>
  <summary>Details</summary>
Motivation: 大语言模型用于旅游路线规划时无法生成考虑详细用户需求的最优方案，需要更灵活全面的路线规划方法。

Method: 引入 KATR 查询提供灵活全面的语义进行路线规划，提出探索 - 边界范式通过估计分数边界消除冗余候选方案来处理 KATR 查询。

Result: 通过大量实验表明该方法在不同场景下性能优于现有方法。

Conclusion: 所提出的方法能有效解决大语言模型在路线规划中的不足，提供更优的路线规划方案。
...
TL;DR: 提出一种将递增边条件编译进输入图的构造性翻译方法，克服了核心GQL模式匹配的局限性，在Neo4j的Cypher实现中提升了性能


<details>
  <summary>Details</summary>
Motivation: 核心GQL模式匹配无法检查路径上的边值是否递增

Method: 将递增边条件编译进输入图的构造性翻译

Result: 在Neo4j的Cypher概念验证实现中，编译版本运行更快且避免了超时

Conclusion: 理论驱动的翻译方法不仅能弥补表达能力差距，还能带来实际性能提升
...
TL;DR: 本文介绍超图中 s - 可达性和最大可达性查询的概念，提出 HL - 索引解决最大可达性查询问题，开发快速覆盖关系检测方法和轻量级邻居索引以高效构建索引，实验证明该方法的效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 超图可达性对建模复杂的群体交互很重要，现有顶点到顶点和超边到超边的索引技术有局限性，需解决最大可达性查询问题。

Method: 引入 s - 可达性和最大可达性查询概念，提出 HL - 索引，开发快速覆盖关系检测方法，提出轻量级邻居索引。

Result: 在 20 个数据集上的大量实验证明了方法的效率和可扩展性。

Conclusion: 提出的方法能有效且高效地解决超图中的最大可达性查询问题。
...
TL;DR: 为解决文本到SQL系统高质量数据少和复杂场景推理能力弱的问题，提出双中心框架，经实验取得单模型最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前文本到SQL系统发展受高质量训练数据稀缺和模型复杂场景推理能力有限的阻碍。

Method: 从数据中心视角构建迭代数据工厂合成高质量数据，从模型中心视角引入新的代理强化学习框架，含多样性感知冷启动和组相对策略优化。

Result: 在BIRD和Spider基准测试上，此协同方法在单模型方法中取得了最先进的性能。

Conclusion: 提出的双中心整体框架有效改善了文本到SQL系统的问题。
...
TL;DR: 本文研究动态道路网络中考虑旅行成本变化的kNN查询问题，提出分布式算法DkNN，在Storm平台实现，比传统方法更高效有效。


<details>
  <summary>Details</summary>
Motivation: 现有研究在道路网络中以距离为指标进行kNN查询，忽略了旅行成本波动，且基于索引的方法随旅行成本变化而过时。

Method: 放弃索引，在动态道路网络的每个快照上进行增量网络扩展，提出分布式算法DkNN，将道路网络划分为子网络，使用Dijkstra算法并行探索。

Result: DkNN在真实道路网络场景中比传统方法更具效率和有效性。

Conclusion: DkNN能有效解决动态道路网络中kNN查询问题，克服了传统方法的局限。
...
TL;DR: 为解决ER高速流可扩展性问题，提出SPER框架，实验显示有显著加速。


<details>
  <summary>Details</summary>
Motivation: 大数据时代传统批处理ER不可行，现有渐进式方法面对高速流时由于确定性排序导致扩展性差。

Method: 提出SPER框架，将优先级定义为采样问题，用连续随机二分最大化策略替代全局排序。

Result: 在8个真实数据集上，SPER比现有最优基线实现3到6倍加速，同时保持相当的召回率和精确率。

Conclusion: SPER框架解决了现有渐进式方法扩展性问题，在高速流上有更好表现。
...
TL;DR: 提出GPU-Virt-Bench基准框架评估GPU虚拟化系统，展示其在评估软件方案和模拟MIG基线中的作用。


<details>
  <summary>Details</summary>
Motivation: GPU加速工作负载增加，对GPU资源共享需求大，但现有技术有局限且缺乏标准化评估方法。

Method: 构建GPU-Virt-Bench基准框架，涵盖56个性能指标、10个类别，对软件虚拟化方法和理想MIG行为进行对比。

Result: 通过评估HAMi-core、BUD - FCSP和模拟MIG基线，揭示对生产部署决策关键的性能特征。

Conclusion: GPU-Virt-Bench为多租户环境中部署GPU资源提供有价值见解。
...
TL;DR: 提出带性能反馈的端到端LLM框架优化GPU内核，无需完整构建应用，跨平台且成本低。
...
Method: 从独立提取热点内核，自动补全为最小可执行程序，在完整应用外进行多轮迭代优化和评估，集成自动错误修复和性能模式继承，最后将优化变体重新集成到原应用验证。

Result: 在不同平台和应用上取得显著加速比，超过直接LLM优化。

Conclusion: 该方法无需完整源码依赖，具备跨平台可移植性，可实现实用、低成本的GPU内核优化。
...
TL;DR: 论文定义以用户主权为导向交互范式，用SoDA实现，设计握手机制，实验表明可减少消耗和认知负荷，是构建Agentic Web的基础设施。


<details>
  <summary>Details</summary>
Motivation: 互联网从注意力经济向Agentic Web时代转变，现有交互模式无法解决数据锁定和认知过载问题。

Method: 定义用户主权交互范式，通过SoDA实现，采用存储、计算和交互正交解耦设计，设计基于A2A协议的意图 - 权限握手机制。

Result: 在跨平台服务迁移和复杂任务执行时减少约27 - 35%的令牌消耗，在多模态复杂任务编排中，比标准RAG架构减少72%认知负荷，比手动工作流程减少88%，显著提高信息信噪比。
...
TL;DR: 提出有效部署分布式DNN的方法，通过集成结构化模型剪枝和多目标优化适配硬件约束和任务性能，在MVCNN上实验效果良好，该方法为边缘环境部署复杂视觉模型提供途径。


<details>
  <summary>Details</summary>
Motivation: DNNs参数多、计算需求大，在资源受限边缘设备上的部署受阻，需找到一种能兼顾硬件限制和任务性能的部署方法。

Method: 集成结构化模型剪枝和多目标优化，根据异构设备约束调整网络容量，在MVCNN上量化单个视图对分类精度的贡献并分配剪枝预算。

Result: 生成的模型满足用户指定的精度和内存占用界限，在不同硬件平台上推理延迟降低1.2 - 5.0倍。

Conclusion: 性能感知、视图自适应压缩为分布式边缘环境部署复杂视觉模型提供了可行途径。
...
TL;DR: 本文提出局部会合哈希（LRH），在固定拓扑活性变化下有优势，基准测试中降低负载比且速度更快。


<details>
  <summary>Details</summary>
Motivation: 现有环形一致性哈希方案存在高负载比问题，多探针方法有分散内存访问成本，需改进。

Method: 提出LRH，保留令牌环，将最高随机权重（HRW）选择限制在C个不同相邻物理节点的缓存局部窗口。通过一次二分查找定位键，使用预计算的下一个不同偏移量枚举C个不同候选者并选择HRW获胜者。

Result: 在基准测试中，LRH将最大/平均负载从1.2785降至1.0947，达到60.05 Mkeys/s，比8探针多探针一致性哈希快约6.8倍。微观基准测试表明多探针分配受重复环形搜索和内存流量主导。

Conclusion: LRH在负载均衡和性能上表现良好，是一种有效的分布式系统哈希方案。
...
TL;DR: 现有大语言模型在边缘设备实时部署有推理延迟和令牌消耗问题，提出HybridFlow框架解决，评估显示其能减少推理时间和令牌使用并保持精度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在边缘设备实时部署存在推理延迟和令牌消耗大的问题，现有边缘 - 云协作方法采用粗粒度任务分配策略，存在资源利用不高效的问题。

Method: 提出HybridFlow框架，分两个阶段：任务分解与并行执行，将复杂查询动态分解为可并行执行的子任务；资源感知子任务路由，通过学习的路由器根据预测效用增益和实时预算状态将子任务分配到边缘或云模型。

Result: 在GPQA、MMLU - Pro、AIME和LiveBench - Reasoning上的综合评估表明，HybridFlow有效减少了端到端推理时间和整体令牌使用量。

Conclusion: HybridFlow能在保持有竞争力的准确性的同时，有效减少推理时间和令牌使用。
...
TL;DR: 本文探讨将离线性能优化框架应用于微服务应用的DevOps生命周期发布阶段，评估框架并比较优化算法，给出不同目标下的策略。


<details>
  <summary>Details</summary>
Motivation: 多数研究聚焦DevOps周期的Ops阶段微服务智能调度和自动伸缩，发布阶段CPU和内存资源分配配置参数优化问题研究较少，且前期未对资源进行微调会导致容器内存分配不当。

Method: 使用TeaStore微服务应用评估性能优化框架，统计比较不同优化算法。

Result: 前期因子筛选在以可承受采样预算寻找最优资源配置和统计比较算法时有用；若目标是寻找近似最优配置，不进行筛选直接运行贝叶斯优化更好。

Conclusion: 明确了不同目标下，在微服务应用发布阶段使用性能优化框架及选择优化算法的策略。
...
TL;DR: 提出HLS4PC框架加速3D点云模型，优化后获低复杂度模型，FPGA加速有高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 3D点云数据特性致GPU利用率低，影响安全关键应用实时性能。

Method: 提出HLS4PC框架，利用FPGA并行和算法优化，对PointMLP - Elite模型用硬件感知压缩技术。

Result: 得到复杂度低4倍、精度仅降2%的PointMLP - Lite，FPGA加速吞吐量比之前工作高3.56倍，比GPU和CPU分别高2.3倍和22倍。

Conclusion: HLS4PC框架能有效加速3D点云模型，提升吞吐量。
...
TL;DR: 现有基础模型训练生态集中于云数据中心，边缘训练方法有不足。本文提出Cleave范式，实现高效大模型边缘训练，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型训练生态集中，成本高，现有边缘训练方法存在性能、扩展性、内存和通信等问题，需要改进。

Method: 引入Cleave范式，采用选择性混合张量并行方法精细划分训练操作，结合参数服务器中心训练框架，用成本优化模型指导设备选择和工作负载分配。

Result: Cleave能高效扩展到更大模型和数千设备，匹配云GPU训练，支持设备数比基线方法多8倍，每批训练时间比现有方法快10倍，设备故障恢复速度至少快100倍。

Conclusion: Cleave范式有效解决了现有边缘训练方法的问题，实现了高效的大模型边缘训练。
...
TL;DR: 本文提出自适应GPU资源分配框架，在无服务器GPU平台为多智能体系统分配资源，相比轮询调度减少85%延迟，优于静态分配策略。


<details>
  <summary>Details</summary>
Motivation: 在无服务器GPU平台上高效部署多智能体系统面临资源分配挑战，如异构工作负载、不同计算需求和成本效益扩展问题。

Method: 提出自适应GPU资源分配框架，基于工作负载特征、智能体优先级和最小资源需求动态分配GPU资源，采用O(N)复杂度算法实时自适应。

Result: 通过模拟四个异构智能体工作流，表明该框架在延迟、成本和GPU利用率等指标上均优于静态平等和轮询策略。
...
TL;DR: 本文提出TL框架，用于将基于tile的程序编译到空间数据流架构，解决跨核心分布tile实例等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有空间数据流加速器虽有高性能等潜力，但可编程性有限阻碍广泛应用，且工作负载映射影响端到端性能。

Method: 提出TL框架，构建硬件表示，基于MLIR生态系统，定义通用前后端接口。

Result: TL框架能解决跨核心分布tile实例的挑战，可利用片上网络和分布式内存增加数据重用、减少通信。

Conclusion: TL框架有助于提高空间数据流加速器的可编程性，推动其更广泛应用。
...
TL;DR: 介绍AiiDAlab平台发展，从聚焦计算材料科学到跨学科加速科研发现，还介绍其应用和改进举措。


<details>
  <summary>Details</summary>
Motivation: 解决大规模相互依赖模拟执行需专业技术、处理复杂流程的问题，让科学家专注研究。

Method: 开发AiiDAlab平台，通过网页浏览器直观界面实现复杂计算工作流，利用AiiDA引擎追踪模拟来源。

Result: 应用于量子化学、大气建模等多领域，用于教育；简化用户入门、资源访问，支持处理大数据集；与电子实验室笔记本集成。

Conclusion: AiiDAlab平台可让科学家专注研究，确保模拟可重复性，符合FAIR原则，助力生成可重复开放研究数据。
...
TL;DR: 介绍了用于识别Transformer架构中受故障影响区域的BitFlipScope框架，能在有无参考模型的情况下诊断故障并恢复模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在实际和安全关键场景中易受位翻转故障影响，定位故障区域对诊断、修复和恢复模型功能至关重要。

Method: 在有干净参考模型时，对输出、隐藏状态和内部激活进行差异分析；无参考模型时，使用残差路径扰动和损失敏感性分析。

Result: 框架能有效诊断故障，支持轻量级性能恢复，无需微调。

Conclusion: BitFlipScope是在易受硬件故障和对抗环境中实现可信、容错大语言模型部署的重要一步。
...
TL;DR: 本文探讨利用未充分利用的手机资源，以零额外成本提升本地计算系统，展示用分布式管道并行的概念验证，证明手机可助力机器学习。


<details>
  <summary>Details</summary>
Motivation: 大规模机器学习在受限系统环境应用受限，云计算有局限性，需利用本地计算。

Method: 提出利用分布式管道并行的方法，利用iOS设备的功能。

Result: 实现概念验证系统，在小型计算环境中加速模型训练、批量推理和工具使用。

Conclusion: 普通移动设备有潜力为机器学习做出更大贡献。
...
TL;DR: 论文指出大模型生成式AI推理成本和功耗占比超训练，RAG处理长输入时预填充阶段耗能耗时，提出MatKV方案预计算并存储键值向量，实验表明其能降低RAG推理的时间和功耗，还能实现额外优化。


<details>
  <summary>Details</summary>
Motivation: 当前大模型生成式AI中推理成本和功耗占比超训练，且RAG处理长输入时预填充阶段即使使用高端GPU也耗能耗时，需提高RAG推理预填充阶段效率。

Method: 提出MatKV方案，预计算RAG对象的键值向量，将其存储在廉价、快速且节能的闪存中，推理时复用而非用GPU重新计算。

Result: 使用Hugging Face的Transformers库在先进的GPU和闪存SSD上实验，MatKV使RAG工作负载的推理时间和功耗减半，问答任务准确率不受严重影响；还能实现额外优化，如GPU可同时解码和加载键值向量，可使用低端GPU解码。

Conclusion: MatKV有潜力使大规模生成式AI应用更具成本效益、节能，能在更广泛任务和硬件环境中使用。
...
TL;DR: 本文提出针对NVIDIA和AMD GPU的OPENFOAM全GPU移植SPUMA，在欧洲两个集群测试，分析性能、可扩展性和能耗。


<details>
  <summary>Details</summary>
Motivation: 高性能计算在混合集群上用于计算流体动力学有机会，但GPU编程性尤其是在开源环境下是挑战。

Method: 基于可移植编程模型和内存池管理器，对OPENFOAM进行全GPU移植，在两个集群开展数值测试。

Result: GPU利用率影响强可扩展性，弱可扩展性在不同集群有不同结果，一个A100 GPU相当于200 - 300个英特尔至强核心，能耗最多降低82%。

Conclusion: SPUMA实现了OPENFOAM的全GPU移植，在性能、可扩展性和能耗方面表现良好。
...
TL;DR: 介绍Mirage Persistent Kernel (MPK)，自动将多GPU模型推理转换为高性能大内核，性能优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 解决多GPU模型推理性能问题，实现端到端内核融合并保留编程模型灵活性。

Method: 引入SM级图表示，编译器将张量程序转换为优化的SM级任务图并生成CUDA实现，运行时使用跨SM的分散调度在单个大内核中执行任务。

Result: MPK显著优于现有逐算子内核的大语言模型服务系统，端到端推理延迟最多降低1.7倍，提升性能接近硬件极限。

Conclusion: MPK能以最小开发者工作量实现端到端内核融合，具有较高实用性和性能优势。
...
TL;DR: 本文提出用于智能PMU数据处理的可扩展云原生架构，结合AI与边缘和云计算，能实现低延迟处理和高级分析，为下一代智能电网分析提供基础。


<details>
  <summary>Details</summary>
Motivation: 传统集中式处理架构难以应对PMU数据的规模和速度增长，在动态电网环境中面临挑战。

Method: 采用分布式流处理、容器化微服务和弹性资源编排，结合机器学习模型进行时间序列分析，开发分析模型评估系统性能，嵌入安全隐私机制。

Result: 架构可实现亚秒级响应时间，能扩展到大规模PMU部署。

Conclusion: 该方法为下一代智能电网分析提供了稳健且灵活的基础。
...
TL;DR: 介绍Pick and Spin框架用于自托管大语言模型编排，经评估其相比静态部署有显著优势。


<details>
  <summary>Details</summary>
Motivation: 自托管大语言模型有需求，但部署和维护存在GPU利用、工作负载路由和可靠性等挑战。

Method: 基于Kubernetes构建Pick and Spin框架，集成统一的基于Helm的部署系统、自适应缩零自动化和混合路由模块。

Result: 对四个模型在八个公共基准数据集进行评估，涵盖31,019个提示和163,720次推理运行，Pick and Spin比相同模型的静态部署成功率高21.6%、延迟低30%、每次查询的GPU成本低33%。
...
TL;DR: 提出Nightjar算法解决现有推测解码在不同负载下的性能问题，实验显示其效率更高。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在高负载环境性能下降，且使用固定推测长度无法适应动态请求率，存在性能瓶颈。

Method: 提出Nightjar，一种基于学习的自适应推测推理算法，根据请求负载动态选择不同批量大小的最优推测长度，无益处时禁用推测解码。

Result: Nightjar比标准推测解码实现了高达14.8%的吞吐量提升和20.2%的延迟降低。

Conclusion: Nightjar在实时服务中展现出强大的效率。
...
TL;DR: 提出RobustRL系统处理RL后训练GPU错误，提升有效训练时间比。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型容错框架未探索强化学习异步执行的优化潜力，强化学习后训练中训练与推理交错易暴露系统故障。

Method: 采用基于角色的故障隔离策略，将角色分为训练器、滚动执行等子任务；实现角色感知监控检测；针对训练器和滚动执行不同角色进行非中断恢复和隔离机器替换重启；采用基于UCX的动态点对点通信进行权重同步。

Result: 在256-GPU集群Qwen3 - 8B - Math工作负载、10%故障注入频率的强化学习训练任务中，RobustRL有效训练时间比达80%以上（ByteRobust为60%），端到端训练时间快8.4%-17.4%。

Conclusion: RobustRL是首个可处理GPU机器错误、提升强化学习后训练有效训练时间比的综合鲁棒系统。
...
TL;DR: 本文提出OaaS范式解决云计算中无服务器部署问题，通过多项研究验证，做出五项贡献，为云原生平台奠定基础。


<details>
  <summary>Details</summary>
Motivation: 云计算无服务器简化部署的承诺与实际实现存在差距，主要源于函数运行时、状态管理和编排的碎片化。

Method: 通过三项研究（21人从业者访谈、39人开发者体验研究、86个组织101次访谈）进行实证验证和技术创新。

Result: OaaS范式通过Oparaca原型统一管理，无额外开销且可扩展性强；SLA驱动的OaaS可声明式管理非功能需求；OaaS - IoT比传统FaaS有更好表现；验证了商业化路径；建立了基于实践需求的实证方法。

Conclusion: OaaS范式整合碎片化抽象、自动优化性能，为云原生平台奠定基础，让开发者专注创新。
...
TL;DR: 提出RollArc系统提升多任务智能体强化学习在分布式基础设施上的吞吐量，实验证明其有效性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 智能体强化学习工作负载高度异构，高效训练需分布式基础设施，但简单拆分有同步开销和资源利用率低问题。

Method: RollArc基于硬件亲和性工作负载映射、细粒度异步、状态感知计算三个核心原则构建。

Result: RollArc有效提高训练吞吐量，端到端训练时间减少1.35 - 2.05倍，在超3000个GPU的集群上训练模型证明可扩展性和鲁棒性。

Conclusion: RollArc系统对多任务智能体强化学习训练有显著提升效果。
...
TL;DR: 本文研究多模态大语言模型（MLLMs）推理的能源消耗，分析了模态膨胀问题，量化多模态推理额外能耗，揭示不同架构的能源瓶颈，发现GPU利用率问题，还验证了动态电压和频率缩放（DVFS）优化的有效性。


<details>
  <summary>Details</summary>
Motivation: 多数先前工作聚焦文本模型，多模态大语言模型新增模态带来的能源权衡未被充分研究，需了解其能源消耗情况。

Method: 将推理流程拆分为视觉编码、预填充和解码阶段进行详细的阶段级能耗分析，用四个代表性MLLMs在NVIDIA A100 GPU上评估，对比多模态和仅文本推理能耗，检查GPU功率轨迹。

Result: 多模态推理比仅文本推理额外能耗在17% - 94%之间；不同模型架构能源瓶颈差异大；多模态执行时GPU利用率低；输入复杂度使不同模型能源缩放行为不同；阶段式DVFS优化可节能且性能影响小。

Conclusion: 研究为设计更节能的多模态大语言模型服务系统提供实用见解和具体指导。
...
TL;DR: 现有RDMA传输在分布式机器学习中存在尾延迟瓶颈，本文提出OptiNIC传输，评估显示其在多方面有显著提升。


<details>
  <summary>Details</summary>
Motivation: 分布式机器学习集体通信的尾延迟成为瓶颈，现有RDMA传输因严格可靠性和有序交付不适用于ML。

Method: 基于ML对部分或缺失数据的容忍度，消除NIC的重传和有序交付，引入自适应超时，保留标准拥塞控制机制，将损失恢复转移到ML管道。

Result: 在两个公共云环境中，OptiNIC使训练和推理的时间到准确率（TTA）提高2倍、吞吐量提高1.6倍，降低99%分位数延迟3.5倍，减少BRAM使用2.7倍，提高NIC容错能力近一倍。

Conclusion: OptiNIC是为分布式ML工作负载量身打造的具有弹性、尾优化的RDMA传输。
...
TL;DR: 提出首个支持高效任务卸载的Argus框架，实现动态异构环境下大语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型推理解决方案忽略环境动态、随机和异构特性，以及输出令牌长度和设备多样性影响。

Method: 构建Length - Aware Semantics模块预测输出令牌长度，采用Lyapunov - guided Offloading Optimization模块进行长期体验质量优化，引入Iterative Offloading Algorithm with Damping and Congestion Control算法解决整数非线性规划问题。

Result: 通过广泛理论和实证评估，Argus在高度动态、异构环境中实现了稳健性能和卓越效率。

Conclusion: Argus能有效应对动态异构环境下大语言模型推理问题，实现高效推理。
...
TL;DR: 本文探讨为中小企业部署高性能、私有大语言模型推理服务器的可行性，通过对本地量化模型的基准测试，发现本地部署可达到与云服务相当性能，且成本低、保护隐私。


<details>
  <summary>Details</summary>
Motivation: 大语言模型依赖云服务带来数据隐私、运营主权和成本问题，研究为中小企业提供可负担的私有大语言模型推理服务器部署方案。

Method: 对基于Qwen3的本地托管、量化的300亿参数混合专家模型，在配备英伟达下一代GPU的消费级服务器上进行全面基准分析，评估模型能力和服务器负载性能。

Result: 精心配置的本地部署结合新兴消费硬件和量化开源模型，能达到与云服务相当的性能。

Conclusion: 为中小企业提供了一条无需高昂成本和牺牲隐私即可部署强大大语言模型的可行途径。
...
TL;DR: 介绍Bitcoin - IPC软件栈和协议，可扩展比特币，创建基于L1 BTC的PoS Layer - 2子网，降低交易成本、提高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 使比特币成为通用交换媒介，解决比特币交易吞吐量低等扩展性问题。

Method: 设计受SWIFT消息启发，嵌入比特币SegWit机制，创建基于L1 BTC的PoS层2子网，通过比特币L1进行跨L2子网的价值转移。

Result: 将每笔交易的虚拟字节成本降低达23倍，将货币交易吞吐量从7 tps提高到超160 tps，且无需修改比特币L1。

Conclusion: Bitcoin - IPC能有效扩展比特币，使其更接近成为通用交换媒介。
...
TL;DR: 本文围绕Adaptable TeaStore规范，探讨实现自适应的方法，分析不同途径权衡并建议组合成多层架构。


<details>
  <summary>Details</summary>
Motivation: 以Adaptable TeaStore规范为案例，实现基于控制循环的自适应，考虑自适应关键属性。

Method: 研究软件架构方法、云原生Operator模式和传统编程语言技术，将自适应控制逻辑与TeaStore应用解耦。

Result: 分析了不同方法在细粒度表达适应和系统级控制间的权衡，指出适应策略重用最有效的时机。

Conclusion: 这些方法并非相互排斥，可组合成自适应微服务的多层架构。
...
TL;DR: 研究无嫉妒不完全连通公平分配问题，发现按特定参数化时问题计算困难，容忍一点嫉妒时问题可高效解决。


<details>
  <summary>Details</summary>
Motivation: 解决Gahlawat和Zehavi在2023年提出的开放问题，研究无嫉妒不完全连通公平分配问题的计算复杂度。

Method: 针对具有加性估值的代理，分析按p和代理数量参数化时问题的计算复杂度；设计以p和代理类型数量为参数的高效参数化近似方案。

Result: 按p和代理数量参数化时问题计算困难，即使是星型图和一元表示输入也如此；容忍一点嫉妒时，算法在一般图上有效，二进制输入也高效。
...
TL;DR: 提出快速轻量级并行算法构建块树，速度快且内存需求少，还给出Karp - Rabin指纹计算的数据并行算法。


<details>
  <summary>Details</summary>
Motivation: 现有块树构造算法速度慢且需大量工作内存，需改进。

Method: 提出快速轻量级并行算法用于构建块树，给出Karp - Rabin指纹计算的数据并行算法。

Result: 在单核上与当前最快构造算法速度相近，64核时快达4倍，内存需求降低一个数量级。

Conclusion: 所提算法能高效构建块树，解决了现有算法的问题。
...
TL;DR: 提出解决全局最小割和最小 s-t 割问题的高效伪确定性算法，运行时间优于现有算法，并在多种模型中实现。


<details>
  <summary>Details</summary>
Motivation: 找到解决全局最小割和最小 s-t 割问题更高效的方法，在一些尚无高效确定性算法的模型中找到解决方案。

Method: 设计伪确定性算法。

Result: 算法在全局最小割问题上运行时间渐近优于最快的顺序确定性全局最小割算法，并在多种模型中实现。

Conclusion: 所提出的伪确定性算法是解决相关问题的有效方法。
...
TL;DR: 利用PC - 树数据结构获圆图首个线性时间识别算法


<details>
  <summary>Details</summary>
Motivation: 现有最佳圆图识别算法依赖并查集数据结构，未达严格线性时间，需改进算法复杂度

Method: 使用PC - 树数据结构计算圆图分裂分解

Result: 获得圆图首个线性时间的识别算法

Conclusion: PC - 树数据结构可避免使用并查集数据结构计算分裂分解，能实现圆图的线性时间识别
...
TL;DR: 该论文研究最小子图补全问题（MSC）算法复杂度，给出多种情况下的多项式时间算法。


<details>
  <summary>Details</summary>
Motivation: 最小子图补全问题的优化变体的算法复杂度研究尚不充分。

Method: 通过研究，给出多种非平凡场景下的多项式时间算法。

Result: 在图转换为二分图、余二分图、分裂图，二分正则图转换为弦图等情况下能在多项式时间内求解，在特定图类和连通性相关问题上也能在多项式时间解决。

Conclusion: 明确在多种情况下MSC问题可在多项式时间内求解。
...
TL;DR: 论文开发统计框架评估在线梦幻板球平台中技巧对成功的作用，构建多种策略并对比，引入动态锦标赛模型，通过实验证明技巧元素存在。


<details>
  <summary>Details</summary>
Motivation: 在线梦幻板球规模增长，需探究结果是由技巧还是运气驱动。

Method: 开发统计框架，构建基于多种因素的确定性和随机团队选择策略，在两种竞赛结构下评估策略表现，引入动态锦标赛模型，用IPL 2024数据集进行实验。

Result: 结果为在线梦幻板球平台中存在技巧元素提供了定量证据。

Conclusion: 在线梦幻板球平台的结果受技巧因素影响。
...
TL;DR: 本文将两方政策竞争建模为非合作博弈，验证假设、证明均衡存在性，给出算法找近似均衡。


<details>
  <summary>Details</summary>
Motivation: 对Lin等人（2021）的工作进行推广，捕捉政策竞争中的不确定性。

Method: 将政策竞争建模为博弈，通过投票模拟验证假设，证明均衡存在性，构建反例，使用去中心化梯度算法和基于网格的搜索算法。

Result: 验证了等调性假设，证明纯策略纳什均衡存在，去中心化梯度算法能快速收敛到近似均衡，基于网格的搜索算法能在多项式时间内找到ε - 近似均衡。

Conclusion: 所构建的博弈模型和提出的算法可有效处理两方政策竞争问题。
...
TL;DR: 研究单设施选址博弈机制设计，考虑两类个体目标，针对不同类型游戏提出多种策略证明机制并给出近似比和下界。


<details>
  <summary>Details</summary>
Motivation: 设计机制定位一个设施，最大化所有代理满意度总和或最小值，同时激励代理如实报告位置。

Method: 根据不同距离函数考虑两类个体目标，为期望和厌恶设施选址游戏分别提出不同的策略证明机制。

Result: 为期望设施游戏提出近似比为2和5/4的机制，还有近似比为2的同时最大化最小满意度机制；为厌恶设施游戏证明两种机制是最优的，设计4/3近似随机机制并给出下界。

Conclusion: 提出的机制能在不同类型的单设施选址博弈中实现对满意度的优化，且保证了一定的策略证明性。
...
### [108] [Impact of Volatility on Time-Based Transaction Ordering Policies](https://arxiv.org/abs/2512.23386)
*Sunghun Ko,Jinsuk Park*

Main category: cs.GT

TL;DR: 研究Arbitrum的快速通道拍卖，提出优先访问价值折扣假设并验证与模型一致


<details>
  <summary>Details</summary>
Motivation: 研究Arbitrum的快速通道拍卖机制，理解其经济特性

Method: 基于风险规避竞标者的单轮模型提出假设，用拍卖投标记录和高频ETH价格数据验证

Result: 研究结果与模型一致

Conclusion: 优先访问价值因预测短期波动性困难和竞标者风险规避而相对风险中性估值被折扣
...
### [109] [Verifiable Off-Chain Governance](https://arxiv.org/abs/2512.23618)
*Jake Hartnell,Eugenio Battaglia*

Main category: cs.GT

TL;DR: 论文针对DAO治理现状局限，提出可验证的链下计算框架，探索三种治理机制，经实践验证可行。


<details>
  <summary>Details</summary>
Motivation: 当前DAO治理因链上计算限制，组织表达性受限，复杂决策简化为代币加权投票。

Method: 提出可验证的链下计算框架（利用可验证服务、可信执行环境和零知识证明），并探索三种治理机制。

Result: 框架提供了架构规范、安全模型和实施考虑，经实践验证有实际可行性。

Conclusion: 此框架能为DAO带来更高表达性和运营效率。
...
TL;DR: 传统推荐系统多阶段优化目标不一致，现有GR方法有局限，LLMs有推理能力但有应用挑战，本文提出OxygenREC系统解决问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统推荐系统多阶段优化目标不一致，现有GR方法缺乏演绎推理能力，LLMs应用有挑战，现有方案多场景可扩展性差等问题。

Method: 采用快慢思考架构，慢思考用近线LLM管道合成上下文推理指令，快思考用高效编解码器骨干进行实时生成；引入语义对齐机制和Q2I损失；将场景信息转化为可控指令，使用统一奖励映射和SA - GCPO。
...
TL;DR: 提出一个实时从新闻中填充公路 - 铁路平交道口事故数据（Form 57）的演示系统。


<details>
  <summary>Details</summary>
Motivation: 地方铁路委员会在公路 - 铁路平交道口事故后需要及时态势感知，而联邦铁路管理局（FRA）的官方调查需数天到数周。

Method: 设计一个管道，先用视觉语言模型和样本聚合将 Form 57 转换为 JSON 模式，再按表单布局意图进行分组问答以减少歧义；构建评估数据集，将抓取的新闻文章与 FRA 官方记录对齐并标注可检索信息。

Result: 对系统与各种替代方案在信息检索准确性和覆盖率方面进行了评估。

Conclusion: 未明确提及，但展示了系统解决相关问题的能力。
...
### [112] [Pruning Graphs by Adversarial Robustness Evaluation to Strengthen GNN Defenses](https://arxiv.org/abs/2512.22128)
*Yongyu Wang*

Main category: cs.LG

TL;DR: 提出利用对抗鲁棒性评估的剪枝框架增强GNN防御能力


<details>
  <summary>Details</summary>
Motivation: GNN在图结构和特征上的联合建模使其易受对抗攻击和虚假连接影响

Method: 利用对抗鲁棒性评估识别并移除图中脆弱或有害部分，根据鲁棒性分数选择性剪枝边

Result: 在三个代表性GNN架构和基准测试上实验，该方法显著增强GNN在高扰动情况下的防御能力

Conclusion: 所提剪枝框架能有效增强GNN的防御能力
...
### [113] [Towards Unsupervised Causal Representation Learning via Latent Additive Noise Model Causal Autoencoders](https://arxiv.org/abs/2512.22150)
*Hans Jarett J. Ong,Brian Godwin S. Lim,Dominic Dayta,Renzo Roel P. Tan,Kazushi Ikeda*

Main category: cs.LG

TL;DR: 提出Latent Additive Noise Model Causal Autoencoder (LANCA)用于无监督发现，理论上解决组件不确定性，方法上采用确定性Wasserstein Auto - Encoder，实验表现优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 无监督表征学习中标准方法难以捕捉因果依赖，且因果变量解纠缠在无监督等条件下存在可识别性挑战。

Method: 提出LANCA，将Additive Noise Model (ANM)作为强归纳偏置，采用确定性Wasserstein Auto - Encoder (WAE)并结合可微的ANM层。

Result: 在合成物理基准和逼真环境中，LANCA优于现有基线，对复杂背景场景中的虚假相关性有更强鲁棒性。

Conclusion: LANCA能有效解决无监督表征学习中因果依赖捕捉和可识别性问题。
...
### [114] [KernelEvolve: Scaling Agentic Kernel Coding for Heterogeneous AI Accelerators at Meta](https://arxiv.org/abs/2512.23236)
*Gang Liao,Hongsen Qin,Ying Wang,Alicia Golden,Michael Kuchnik,Yavuz Yetim,Jia Jiunn Ang,Chunli Fu,Yihan He,Samuel Hsia,Zewei Jiang,Dianshi Li,Uladzimir Pashkevich,Varna Puvvada,Feng Shi,Matt Steiner,Ruichao Xiao,Nathan Yan,Xiayu Yu,Zhou Fang,Abdul Zainul-Abedin,Ketan Singh,Hongtao Yu,Wenyuan Chi,Barney Huang,Sean Zhang,Noah Weller,Zach Marine,Wyatt Cook,Carole-Jean Wu,Gaoxiang Liu*

Main category: cs.LG

TL;DR: 本文提出KernelEvolve框架解决DLRM中硬件异构问题，实现内核自动生成与优化，减少开发时间并提升性能。


<details>
  <summary>Details</summary>
Motivation: DLRM训练和推理面临模型架构、内核原语和硬件等系统挑战，需解决硬件异构性。

Method: 以内核规格为输入，在多编程抽象层操作，将内核优化描述为基于图的搜索，根据运行时上下文动态调整。

Result: 在KernelBench套件和三个异构硬件平台的160个PyTorch ATen算子上验证，通过率100%；减少开发时间，提升性能。

Conclusion: KernelEvolve减少开发时间，提升性能，降低新AI硬件编程门槛。
...
### [115] [Hierarchical Stacking Optimization Using Dirichlet's Process (SoDip): Towards Accelerated Design for Graft Polymerization](https://arxiv.org/abs/2512.22279)
*Amgad Ahmed Ali Ibrahim,Hein Htet,Ryoji Asahi*

Main category: cs.LG

TL;DR: 本文提出用于辐射诱导接枝（RIG）的分层堆叠优化框架SoDip，整合多种模型，用多样数据集验证，性能优于GPR，为接枝聚合研究奠定基础。


<details>
  <summary>Details</summary>
Motivation: RIG因基膜形态未报告的可变性导致可重复性有限，需解决性能不一致问题。

Method: 提出SoDip框架，整合Transformer、TabNet、XGBoost、GPR和DPMM及贝叶斯优化；用ChemDataExtractor 2.0和WebPlotDigitizer整理多样数据集。

Result: 交叉验证中，SoDip比GPR性能提升约33%，提供校准置信区间，识别低可重复性区域。

Conclusion: SoDip堆叠架构整合不同质量输入，优于先前模型，为接枝聚合研究中可重复、考虑形态的设计奠定基础。
...
### [116] [SoliReward: Mitigating Susceptibility to Reward Hacking and Annotation Noise in Video Generation Reward Models](https://arxiv.org/abs/2512.22170)
*Jiesong Lian,Ruizhe Zhong,Zixiang Zhou,Xiaoyue Mi,Yixue Hao,Yuan Zhou,Qinglin Lu,Long Hu,Junchi Yan*

Main category: cs.LG

TL;DR: 提出SoliReward框架用于视频奖励模型(RM)训练，解决现有方法局限，在基准测试中验证有效性，代码和基准将开源。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型与人类偏好后训练对齐中，奖励模型开发存在数据标注有噪声、架构设计探索不足、易遭受奖励攻击等问题。

Method: 先通过单项二元标注获取高质量低成本数据，用跨提示配对策略构建偏好对；架构上采用分层渐进查询注意力机制增强特征聚合；引入修改的BT损失考虑胜平场景，规范正样本得分分布。

Result: 在评估物理合理性、主体变形和语义对齐的基准测试中，直接RM评估指标和视频生成模型后训练效果都有提升。

Conclusion: SoliReward框架有效，能解决现有视频奖励模型训练存在的问题。
...
### [117] [Wireless Traffic Prediction with Large Language Model](https://arxiv.org/abs/2512.22178)
*Chuanting Zhang,Haixia Zhang,Jingping Qiao,Zongzhang Li,Mohamed-Slim Alouini*

Main category: cs.LG

TL;DR: 提出基于LLM的TIDES框架用于城市无线流量预测，实验显示其在准确性和鲁棒性上优于现有基线，表明将空间感知融入LLM预测器对未来6G系统网络管理很关键。


<details>
  <summary>Details</summary>
Motivation: 下一代无线网络对智能、自适应资源管理需求增长，现有深度学习和基础模型在城市规模流量动态中忽略空间依赖，需要准确且可扩展的无线流量预测方法。

Method: 提出TIDES框架，通过聚类机制识别区域流量模式并训练个性化模型；引入提示工程方案嵌入流量特征；设计DeepSeek模块进行跨域注意力的空间对齐，微调轻量级组件。

Result: 在真实蜂窝流量数据集上的广泛实验表明，TIDES在预测准确性和鲁棒性方面显著优于现有基线。

Conclusion: 将空间感知融入基于LLM的预测器是未来6G系统实现可扩展和智能网络管理的关键。
...
TL;DR: 研究应用机器学习算法自动识别声带并区分正常与麻痹声带图像，模型验证准确率高，显示出提升诊断准确性的潜力。


<details>
  <summary>Details</summary>
Motivation: 声带超声检查准确性依赖操作者，需提升诊断准确性。

Method: 采集30名志愿者的声带超声视频，拆分为帧并裁剪，用健康和模拟麻痹声带图像训练分割和分类模型。

Result: 声带分割模型验证准确率96%，最佳分类模型VIPRnet验证准确率99%。

Conclusion: 机器学习辅助的声带超声分析有望提高诊断准确性，优于依赖操作者的人工判读。
...
TL;DR: 监督深度学习在高维表格领域存在泛化崩溃问题，本文提出Latent Sculpting框架解决，在CIC - IDS - 2017基准测试中表现良好，表明解耦结构学习和密度估计可实现通用异常检测。


<details>
  <summary>Details</summary>
Motivation: 解决监督深度学习在高维表格领域面对分布外数据时的泛化崩溃问题，认为问题源于潜在空间缺乏拓扑约束。

Method: 提出Latent Sculpting分层两阶段表示学习框架，第一阶段用混合1D - CNN和Transformer Encoder结合新型双质心紧凑损失（DCCL）将良性流量塑造成低熵超球形簇；第二阶段在预结构化流形上使用掩码自回归流（MAF）学习精确密度估计。

Result: 在CIC - IDS - 2017基准测试中，框架F1分数达0.87，对‘Infiltration’场景检测率达88.89%，优于监督和无监督基线模型。

Conclusion: 解耦结构学习和密度估计为通用异常检测提供了可扩展途径。
...
TL;DR: 本文提出强化学习框架优化网球策略，训练代理取得高胜率，但存在防守倾向，凸显奖励设计的重要性。


<details>
  <summary>Details</summary>
Motivation: 解决网球策略优化这一具有挑战性的序贯决策问题。

Method: 将自定义网球模拟环境与使用课程学习训练的Dueling Double Deep Q-Network (DDQN) 集成。

Result: 训练代理对阵平衡对手胜率达98 - 100%，发球和回球效率较高；消融研究表明决斗架构和课程学习对稳定收敛必要，标准DQN基线无法学习有效策略。

Conclusion: 简化体育模拟中胜率驱动优化有局限，奖励设计对现实体育强化学习很重要。
...
TL;DR: 本文是系列第二篇，聚焦将物理知识和不确定性融入学习过程，介绍PINNs应用，提出贝叶斯PINNs量化不确定性并做预测，还给出新兴研究方向。


<details>
  <summary>Details</summary>
Motivation: 将基于物理的知识与机器学习模型集成，以更好地进行电力变压器监测、诊断和预测。

Method: 先介绍物理信息神经网络PINNs用于时空热建模和固体绝缘老化，再提出贝叶斯PINNs作为量化认知不确定性的框架。

Result: 提出贝叶斯PINNs可在稀疏数据下进行鲁棒预测。

Conclusion: 指出物理感知和可信机器学习对关键电力资产有潜力，给出新兴研究方向。
...
TL;DR: 本文聚焦电力变压器状态监测与健康管理，分析神经网络及其扩展，首篇介绍概念、探讨CNN应用及RL决策，给出研究方向展望。


<details>
  <summary>Details</summary>
Motivation: 传统变压器状态监测方法应对不确定性、有限数据及复杂工况存在困难，机器学习可提供补充和扩展。

Method: 介绍神经网络基本概念，探索卷积神经网络在状态监测中的应用，讨论神经网络在强化学习范式中的集成。

Result: 在论文中探讨了相关技术在变压器状态监测与健康管理中的作用。

Conclusion: 最后提出了新兴研究方向的观点。
...
TL;DR: 研究现代CNN光谱偏差，提出评估正则化器方法，发现L2正则化抑制高频能量积累，揭示准确率 - 鲁棒性权衡，从信号处理视角证明正则化偏向低频结构。


<details>
  <summary>Details</summary>
Motivation: 现有正则化技术在特征频率选择的物理机制不明，研究现代CNN光谱偏差。

Method: 引入视觉诊断框架追踪权重频率动态变化，提出光谱抑制比量化正则化器“低通滤波”强度，通过离散径向剖析解决小内核混叠问题。

Result: L2正则化比无正则化基线多抑制超3倍高频能量积累；L2模型对宽带高斯噪声敏感，但在高频信息损失场景中鲁棒性超基线6%以上。

Conclusion: 从信号处理视角证实正则化对低频结构有强烈光谱归纳偏差。
...
TL;DR: 现代AI决策方法依赖外部奖励函数，在现实环境中脆弱，本文提出受生物情感启发的EILS框架，可动态调节优化，有望提升样本效率和非平稳适应性。


<details>
  <summary>Details</summary>
Motivation: 现代AI依赖外部奖励函数的方法在开放现实环境中使智能体表现脆弱，缺乏内部自主性，需要解决这些问题以实现鲁棒自主性。

Method: 引入Emotion - Inspired Learning Signals (EILS)框架，将情感建模为连续的稳态评估信号，如好奇心、压力和信心，将这些信号形式化为基于交互历史的向量值内部状态，实时动态调节智能体的优化格局。

Result: 文中未提及具体实验结果，仅假设EILS智能体在样本效率和非平稳适应方面能超越标准基线。

Conclusion: 提出的EILS框架有望通过闭环稳态调节，使智能体在样本效率和非平稳适应方面优于标准方法。
...
TL;DR: 提出单头动态值注意力（DVA）方法，可减少训练时间并提升学习能力


<details>
  <summary>Details</summary>
Motivation: 现有Transformer优化工作未解决每个头中每个查询使用相同静态值的固有局限，且多头数量受复杂度限制

Method: 提出为每个查询动态决定值的方法，削减冗余头，仅保留一个头并可完全削减前馈网络

Result: 实验显示DVA比原始Transformer节省37.6%训练时间，同时提升学习能力

Conclusion: Transformer中仅需单头动态值注意力（DVA）
...
TL;DR: 提出基于组合推理的半监督节点分类框架，在异质性和过渡性基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在异质性图上表现不佳，需要新方法。

Method: 采用置信度排序贪心算法，结合多种因素打分，使用验证门控混合策略。

Result: 在异质性和过渡性基准测试中与现代GNNs竞争，有更好的可解释性，可调节性和计算效率。

Conclusion: 所提出的框架是一种有效且具有优势的半监督节点分类方法。
...
TL;DR: 提出Müntz - Szász Networks (MSN)架构，用可学习分数幂基替换固定激活函数，证明其通用逼近性和新逼近率，在奇异函数逼近任务上优于标准MLP和PINN。


<details>
  <summary>Details</summary>
Motivation: 标准神经网络激活函数不适合逼近物理中常见的具有奇异或分数幂行为的函数。

Method: 引入MSN架构，其边计算使用可学习指数的分数幂基，指数和系数一同学习。

Result: MSN继承通用逼近性，有新逼近率；在监督回归中比MLP误差低5 - 8倍且参数少10倍；在PINN基准测试中提升3 - 6倍并能学习到与已知解结构匹配的可解释指数。

Conclusion: 理论引导的架构设计能显著提升科学相关函数类的逼近效果。
...
TL;DR: 研究用神经网络和迁移学习模型预测爱尔兰燕麦作物霉菌毒素污染，评估五种建模方法，TabPFN表现最佳，90天收获前天气模式和种子含水量是重要预测因子。


<details>
  <summary>Details</summary>
Motivation: 霉菌毒素污染对谷物质量、食品安全和农业生产力构成风险，准确预测霉菌毒素水平可支持早期干预并减少经济损失。

Method: 使用包含环境、农艺和地理预测因子的爱尔兰燕麦样本数据集，评估五种建模方法（基线多层感知器、预训练MLP和三种迁移学习模型），用回归和分类指标评估模型性能，进行基于排列的变量重要性分析。

Result: 迁移学习方法TabPFN总体表现最佳，其次是基线MLP。

Conclusion: 90天收获前的天气历史模式和种子含水量是最重要的预测因子。
...
TL;DR: 提出ReGAIN框架用于网络流量分析，结合多种技术，在真实数据集上表现良好且有解释性。


<details>
  <summary>Details</summary>
Motivation: 传统网络流量分析系统存在高误报和缺乏可解释性问题，限制分析师信任。

Method: 提出多阶段框架ReGAIN，结合流量总结、检索增强生成和大语言模型推理，使用分层检索管道并采用多种机制减少幻觉。

Result: 在真实流量数据集上评估，不同攻击类型和基准测试中准确率达95.95% - 98.82%，结果经数据集真实值和专家评估验证。

Conclusion: ReGAIN性能优于基线方法，且提供可信赖、可验证的解释。
...
TL;DR: 本文扩展了神经网络超参数转移工作，提出Complete$^{(d)}$ Parameterisation，研究模块级超参数优化与转移，实验表明转移模块级超参数可提升大语言模型训练速度。


<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型训练稳定性和最终性能有重大影响，现有工作在参数化和超参数转移方面有一定进展，但有可拓展空间。

Method: 提出Complete$^{(d)}$ Parameterisation统一多个维度的缩放，研究模块级超参数优化与转移，给出应对高维超参数优化问题的实用指南。

Result: 实验证明在正确的参数化下，超参数转移在模块级超参数体系中依然成立，转移模块级超参数可显著提升大语言模型训练速度。

Conclusion: 通过合适的参数化，超参数转移在模块级超参数场景也适用，能有效提升大语言模型训练效率。
...
TL;DR: 提出DiRL后训练框架和DiPO方法，在数学数据上训练模型取得SOTA数学性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLMs）后训练发展不足，现有方法存在计算低效和训练推理目标不匹配问题，限制复杂推理任务表现。

Method: 引入DiRL框架，集成FlexAttention加速分块训练和LMDeploy优化推理，实现高效两阶段后训练；提出DiPO方法。

Result: 在高质量数学数据上训练DiRL - 8B - Instruct模型，在dLLMs中取得SOTA数学性能，在多个基准测试中超越Qwen2.5系列可比模型。

Conclusion: 所提出的DiRL框架和DiPO方法有效提升了dLLMs在数学任务等复杂推理任务上的性能。
...
TL;DR: 为解决GNN应用于大图的计算成本问题，提出BLISS方法，能动态选节点，与GCN、GAT集成，实验显示其表现佳。


<details>
  <summary>Details</summary>
Motivation: 图神经网络应用于大图时，因处理节点邻居产生内存和计算瓶颈，需要解决计算成本问题。

Method: 提出BLISS，使用多臂老虎机在每层动态选择最具信息的节点，平衡探索与利用，能适应不同聚合机制。

Result: BLISS与GCN和GAT集成，实验显示其能维持或超越全批量训练的准确度。

Conclusion: BLISS可动态选择节点，适应节点重要性变化，选择更合理，性能更优。
...
TL;DR: 大尺寸视觉语言模型部署不便，提出Masters框架解决知识蒸馏难题，结合离线RL提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型尺寸大难部署，小模型从大模型蒸馏知识因尺寸差距面临挑战，需解决学习不稳定和性能下降问题。

Method: 提出Masters框架，先掩码教师模型非主导权重，训练中逐步恢复其容量；集成离线RL阶段，设置准确性奖励和蒸馏奖励，利用掩码教师预生成响应指导学生模型。

Result: 文中未明确提及具体实验结果，但表明该方法能让学生模型平滑稳定学习，无需高计算成本的在线思考-回答过程，实现较好性能。

Conclusion: Masters框架有效解决了从大教师模型到小学生模型知识蒸馏的难题，通过掩码和离线RL策略，提升了学生模型学习效果。
...
TL;DR: 提出CRC框架解决现代多元预测器的安全问题，实验表明其能提升准确率并确保高非退化率。


<details>
  <summary>Details</summary>
Motivation: 现代多元预测器有系统误差且缺乏性能不退化保证，现有事后残差校正方法有缺陷。

Method: 提出CRC框架，采用因果启发编码器解耦动态，混合校正器建模残差，有四重安全机制。

Result: 在多数据集和预测骨干网络实验中提升准确率，消融研究证实有高非退化率。
...
TL;DR: 介绍用于衡量重复训练中模型解释稳定性的EvoXplain框架，评估发现模型解释常具多模态性。


<details>
  <summary>Details</summary>
Motivation: 探究两个高准确率模型是否依赖相同内部逻辑，质疑高准确率模型解释就一定正确可信的假设。

Method: 引入EvoXplain框架，将解释视为随机优化过程样本进行分析。在乳腺癌和COMPAS数据集上，用逻辑回归和随机森林两类模型评估框架。

Result: 所有模型虽准确率高，但解释常具多模态性，即使逻辑回归这类通常被认为稳定的模型也会产生多个分离解释区域，差异并非由超参数变化或简单性能权衡导致。

Conclusion: EvoXplain不选'正确'解释，让解释不稳定性可见且可量化，揭示单实例或平均解释会掩盖潜在多种机制，将可解释性视为模型类重复实例化的属性。
...
TL;DR: 本文构建‘贝叶斯风洞’验证Transformer执行贝叶斯推理能力，发现其通过一致几何机制实现推理，揭示层次化注意力的设计原理及风洞的应用价值。


<details>
  <summary>Details</summary>
Motivation: 自然数据缺乏解析后验，大模型混淆推理与记忆，难以严格验证Transformer执行贝叶斯推理的能力。

Method: 构建‘贝叶斯风洞’，在可控环境下测试小Transformer和容量匹配的MLP，并开展双任务实验，运用几何诊断方法分析。

Result: 小Transformer能高精度重现贝叶斯后验，MLP表现差；发现Transformer通过特定几何机制实现贝叶斯推理，有相关几何特征和训练现象。

Conclusion: 层次化注意力通过几何设计实现贝叶斯推理，解释了注意力的必要性和扁平架构的失败；贝叶斯风洞为连接小验证系统和大语言模型推理现象提供基础。
...
TL;DR: 提出基于元学习的跨数据集知识转移模型预测L - DED熔道几何形状，两种元学习算法表现优于传统前馈神经网络。


<details>
  <summary>Details</summary>
Motivation: 解决L - DED中因实验数据集稀缺和异质性导致难以准确预测熔道几何形状的问题。

Method: 研究两种基于梯度的元学习算法MAML和Reptile，利用多个实验数据集进行框架验证。

Result: MAML和Reptile用3 - 9个训练样本就能准确预测熔道高度，优于传统神经网络，元学习模型泛化性能强。

Conclusion: 元学习模型在异质L - DED环境下能有效进行知识转移。
...
TL;DR: 文章指出常用的基于排名的输入归一化算子不稳定，提出三个公理来规范其设计，构建满足条件的算子并验证约束非平凡。


<details>
  <summary>Details</summary>
Motivation: 现有基于排名的归一化算子在不变性下的结构条件未明确，且常用算子不稳定。

Method: 提出三个公理，证明满足公理的算子形式，构建满足条件的最小算子并进行实验。

Result: 构建的算子满足条件，且在现实场景中约束非平凡。

Conclusion: 明确了有效基于排名的归一化算子的设计空间，与现有方法区分开来。
...
TL;DR: 研究评估肺癌筛查深度学习风险评估模型性能差异与公平性，发现模型在不同人群有显著差异，强调改进与监测欠代表性亚组模型性能及算法公平性研究的重要性。


<details>
  <summary>Details</summary>
Motivation: 肺癌年度低剂量CT筛查可能使放射科人力紧张，AI模型虽有潜力但不同人群性能待研究，因此评估模型性能差异和公平性。

Method: 基于JustEFAB框架评估两个深度学习风险评估模型和逻辑回归模型，在NLST验证集评估，分析不同人群的AUROC、敏感性和特异性，探索临床风险因素混杂。

Result: Sybil模型在女性和男性中的AUROC有显著差异；Venkadesh21模型对黑人参与者的敏感性低于白人，且这些差异不能用临床混杂因素解释。

Conclusion: 强调在肺癌筛查中改进和监测欠代表性亚组模型性能以及开展算法公平性研究的重要性。
...
TL;DR: 研究深度学习模型超参数跨规模迁移策略，提出概念框架，分析Maximal Update Parameterization的迁移机制并提出假设。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模增大使标准超参数优化成本过高，需理解超参数跨规模迁移策略原理。

Method: 开发概念框架，分析理论性质，通过合成设置和实验验证假设。

Result: 证明快速迁移对计算最优网格搜索等价于有用迁移，发现迁移性质依赖问题结构，给出假设的实证证据。

Conclusion: 提出的框架和假设有助于理解超参数跨规模迁移，对大规模模型超参数优化有指导意义。
...
TL;DR: 为LLM法官高效获取校准良好的不确定性估计很关键，本文引入线性探针，在多任务上表现优于现有方法，虽有局限性但为生产提供实用方案。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的评判器用于行业应用，高效获取校准良好的不确定性估计对生产部署至关重要，但现有技术存在校准差或计算成本高的问题。

Method: 引入用基于Brier分数损失训练的线性探针，从推理评判器的隐藏状态提供校准的不确定性估计，无需额外模型训练。

Result: 探针在计算上节省约10倍成本，校准效果优于现有方法，能稳健泛化到未见评估领域，高置信度预测准确率更高，但在简单数据集上表现不佳。

Conclusion: 基于可解释性的不确定性估计为生产中的大语言模型评判器提供了实用且可扩展的即插即用解决方案。
...
TL;DR: 本文指出梯度下降中数学理想与有效激活更新存在系统不匹配，提出新的归一化理解和替代方法，得到新函数并经验验证。


<details>
  <summary>Details</summary>
Motivation: 解决梯度下降中激活更新未采取最优最陡下降步骤的问题，重新审视归一化的作用。

Method: 分析激活更新的问题，从第一性原理推导归一化，提出替代方法如PatchNorm，将归一化器分解为带参数缩放的激活函数映射。

Result: 得到新的归一化理解和替代方法，新函数在多个测试中优于传统归一化器。

Conclusion: 提出理论原则性方法，得到新函数并验证，对模型创建的仿射+非线性方法提出疑问。
...
TL;DR: 本文深入研究目标损失估计误差与替代损失估计误差，得出H - 一致性边界，并在二分类、多分类等场景下展示其应用，开发推导框架并分析边界增长率。


<details>
  <summary>Details</summary>
Motivation: 机器学习中训练优化的损失函数常与定义任务性能的目标损失不同，因此研究目标损失估计误差与替代损失估计误差。

Method: 先从二分类开始建立边界，再扩展到多分类，为不同损失函数推导边界，开发综合框架推导边界，分析边界增长率与最小化差距。

Result: 得出H - 一致性边界，在某些情况下无法得到非平凡的H - 一致性边界，为不同损失函数推导了边界，确定了平滑替代损失的通用平方根增长率。

Conclusion: H - 一致性边界比贝叶斯一致性等更具优势，能为替代损失选择提供指导。
...
TL;DR: 提出基于模拟的摊销推理方法，用合成飞行数据训练神经网络预测模型火箭飞行性能，在零真实训练样本下实现从模拟到真实的转移，减少预测误差。


<details>
  <summary>Details</summary>
Motivation: 传统方法预测模型火箭飞行性能存在局限，计算流体动力学或经验关联方法有不足，数据驱动方法需大量昂贵且耗时收集的真实飞行数据。

Method: 基于模拟的摊销推理方法，在物理模拟器生成的合成飞行数据上训练神经网络，直接从单次顶点测量结合发动机和配置特征预测阻力系数和推力修正因子。

Result: 在10000个合成飞行数据上训练，8个真实飞行数据上评估，顶点预测平均绝对误差12.3米；与OpenRocket基线预测相比，减少了顶点预测误差。

Conclusion: 该方法在零真实训练样本下实现了有前景的从模拟到真实的转移；分析发现预测存在系统性正偏差；公开代码以支持业余火箭社区的可重复性和应用。
...
TL;DR: 论文研究大语言模型面临的幻觉和高推理成本问题，探讨多专家延迟学习及弃权学习，提出新代理损失并证明一致性，通过实验验证算法有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉和高推理成本问题，利用多专家延迟可提高可靠性和效率，从而引出多专家延迟学习问题。

Method: 分析多分类中弃权学习的基于分数和预测 - 拒绝器公式，设计单阶段和两阶段场景的新代理损失；为回归问题引入新框架和代理损失。

Result: 解决弃权学习两个开放性问题，证明代理损失的一致性，实验表明算法性能优越。

Conclusion: 提出的方法能有效解决多专家延迟学习及相关问题，具有较强的一致性和实用性。
...
TL;DR: 本文利用大语言模型提取人体运动序列的时间视觉语义信息，改进子空间聚类方法以实现无监督人体运动分割，在四个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统无监督人体运动分割方法忽视时间语义探索，本文旨在利用大语言模型提取时间视觉语义提升子空间聚类性能。

Method: 通过大语言模型从连续帧提取文本运动信息，融入子空间聚类框架；判断连续帧是否为同一运动，学习时间邻域信息；开发集成时间视觉语义的子空间聚类方法，引入时间约束和反馈框架持续优化。

Result: 在四个基准人体运动数据集上，该方法优于现有最先进方法。

Conclusion: 利用时间视觉语义改进的子空间聚类方法能有效提升无监督人体运动分割效果。
...
TL;DR: 本文构建融合生物医学图，用图注意力网络预测药物 - 细胞对的基因表达变化，表现优于基线，为药物机制建模提供新思路。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型未应用于基因扰动任务，需构建新方法揭示药物转录组学机制。

Method: 构建融合PrimeKG++和LINCS L1000节点的生物医学图，用图注意力网络训练，预测978个地标基因的表达变化。

Result: 框架在支架和随机拆分下优于多层感知机基线，消融实验表明生物医学知识图谱的边能增强扰动水平预测。

Conclusion: 框架为药物机制建模提供方向，从二元药物 - 疾病关联任务转向治疗干预的转录效应建模。
...
TL;DR: 论文针对大语言模型策略梯度方法中的离策略不匹配问题，推导更紧误差界，提出Trust Region Masking方法提供长序列任务单调改进保证。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型强化学习中离策略不匹配问题不可避免，经典信任区域误差界对长序列任务无效。

Method: 推导Pinsker - Marginal界（$O(T^{3/2})$）和Mixed界（$O(T)$），提出Trust Region Masking方法，若序列中任一token违反信任区域则排除该序列进行梯度计算。

Result: 得到更紧的误差界，且依赖于序列级别的最大token - level KL散度。

Conclusion: Trust Region Masking方法为长序列大语言模型强化学习提供了首个非空的单调改进保证。
...
TL;DR: 提出Graph Attention Adaptive Transfer Network (GAATNet)解决图神经网络链接预测问题，实验显示其达最优性能


<details>
  <summary>Details</summary>
Motivation: 现有链接预测方法在处理大规模稀疏图及不同数据集对齐时有挑战，且以往研究忽视迁移学习跨图数据集的潜力

Method: 提出GAATNet结合预训练和微调；设计两种策略，在自注意力模块引入远距离邻居嵌入作为偏差 ，微调时引入轻量级自适配器模块

Result: 在七个公开数据集的综合实验中，GAATNet在链接预测任务中达到最优性能

Conclusion: 本研究为链接预测任务提供通用可扩展的解决方案，有效结合图神经网络和迁移学习
...
TL;DR: 研究大语言模型强化学习中训练推理不匹配问题，提出在动态修剪的“安全”词汇表上约束RL目标的方法。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型强化学习中高吞吐量推理引擎和精确训练系统产生的训练推理不匹配问题。

Method: 将强化学习目标约束到动态修剪的“安全”词汇表，排除极端尾部低概率标记。

Result: 方法实现了稳定训练，理论上界定了词汇修剪引入的优化偏差。

Conclusion: 通过修剪标记，用有界优化偏差替代系统偏差大的不匹配，实现稳定训练。
...
TL;DR: 研究基于PCI患者的真实和合成数据开发预测心脏死亡风险的模型，应用机器学习模型预测PCI后三年死亡率，发现简单扩充数据可改善不平衡临床预测。


<details>
  <summary>Details</summary>
Motivation: 开发基于PCI患者真实和合成数据的心脏死亡风险预测模型，识别对死亡率影响最大的因素。

Method: 分析2044名PCI患者数据，应用多种机器学习模型预测死亡率，生成500个合成样本处理类别不平衡，用排列特征重要性评估特征贡献，进行去除非信息特征的实验。

Result: 未过采样时模型整体准确率高但忽略少数类，数据扩充提高少数类召回率，减少AUROC损失，改善概率质量，得出四个最具影响力的特征。

Conclusion: 简单扩充现实和极端案例可减少不平衡临床预测的脆弱性，建议常规报告概率质量和压力测试。
...
TL;DR: 研究现代生成和视觉语言模型（VLMs）在有限样本下预测均匀准确性和校准性的条件，给出有限样本一致收敛界和依赖于特征谱的界，并分析对数据有限生物医学建模的影响。


<details>
  <summary>Details</summary>
Motivation: 现代VLMs用于科学和医学决策支持时，其预测概率需准确且校准良好，但不清楚预测何时能在输入、类别或子群体中均匀推广，在生物医学中这是关键问题。

Method: 从有限样本角度研究，聚焦于在受限表示空间内通过改变提示或语义嵌入得到的分类器族，采用经典的一致收敛工具。

Result: 给出了VLM诱导分类器在关于提示嵌入的Lipschitz稳定性下，准确性和校准功能的有限样本一致收敛界，样本复杂度取决于内在/有效维度，得到依赖于特征谱的界。

Conclusion: 分析了对数据有限生物医学建模的影响，包括当前数据集大小何时能支持均匀可靠预测，以及平均校准指标可能忽略最坏情况校准误差的原因。
...
TL;DR: 本文对物理信息光栅耦合器频谱生成器进行消融研究，揭示物理约束悖论、常见陷阱，发现生成器高速且有物理可学习性权衡，为数据集设计提供指导。


<details>
  <summary>Details</summary>
Motivation: 科学领域真实数据稀缺时，现有物理约束数据生成方法常过度约束模型且未识别必要物理组件，需进行研究。

Method: 对物理信息光栅耦合器频谱生成器进行系统消融研究，选择性去除特定物理约束。

Result: 发现能量守恒约束冗余；去除法布里 - 珀罗振荡使带宽展宽大幅降低；噪声处理引入非物理值；生成器每秒可产生 200 个样本，比文献中全波求解器快。下游机器学习评估显示有物理可学习性权衡。

Conclusion: 研究结果为物理信息数据集设计提供可行指导，可用机器学习性能评估约束相关性。
...
TL;DR: 论文针对优化广义指标面临的挑战，提出原则性算法METRO，有理论性能保证，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 在类别不平衡或成本不对称应用中，优化Fβ - 度量等指标存在计算和统计挑战，现有方法有局限性。

Method: 将指标优化重新表述为广义成本敏感学习问题，设计有H - 一致性保证的替代损失函数，开发METRO算法。

Result: 实验表明所提方法相比之前基线更有效。

Conclusion: 所提出的原则性算法能有效优化广义指标，且有理论性能保证。
...
TL;DR: 提出确定性AI内存基板Valori，用定点运算取代浮点运算，保证跨平台内存状态和搜索结果相同，表明确定性内存对可信AI系统必要。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统基于浮点运算的向量嵌入存储和搜索存在非确定性，导致无法重放和安全部署，影响事后验证和审计。

Method: 提出Valori，用定点算术（Q16.16）取代浮点内存操作，将内存建模为可重放状态机。

Result: 展示非确定性在索引或检索前就已出现，Valori能在内存边界强制执行确定性。
...
TL;DR: 现有PNN仿真工具训练大规模PNN时扩展性差，引入切片方法集成到LuxIA框架，降低内存和时间消耗，实验证明其速度和扩展性优于现有工具，推动PNN仿真发展。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的PNN仿真工具在训练大规模PNN时面临显著的扩展性挑战，由于传输矩阵计算的计算需求，导致高内存和高时间消耗。

Method: 引入与反向传播兼容的切片方法，并将其集成到统一的仿真和训练框架LuxIA中。

Result: 实验评估表明，在各种光子架构和标准数据集上，LuxIA在速度和可扩展性方面始终超过现有工具。

Conclusion: 该研究为更高效和可扩展的光子神经网络研发铺平了道路，有助于通过光子技术加速人工智能硬件的创新。
...
TL;DR: 本文系统研究大语言模型在时间 motif 相关任务上的表现，提出基准 LLMTM，开发工具增强的 LLM 代理，还提出结构感知调度器平衡准确性和成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型广泛应用，而利用其进行动态图的时间 motif 分析研究较少，有必要系统研究其在相关任务上的表现。

Method: 提出基准 LLMTM，包含六个任务和九种时间 motif 类型；开展大量实验分析不同提示技术和 LLM 对性能的影响；开发工具增强的 LLM 代理；提出结构感知调度器。

Result: 工具增强的 LLM 代理能高精度解决任务，但成本高；结构感知调度器能有效在降低成本的同时保持高精度。

Conclusion: 结构感知调度器可有效平衡大语言模型在时间 motif 相关任务中的准确性和成本。
...
TL;DR: 本文在LMMSE框架下研究盲逆问题学习，提供理论分析、闭式解、收敛结果和有限样本误差界，并进行数值实验验证。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法解决盲逆问题缺乏可解释性和严格理论保障，限制其在应用领域的可靠性。

Method: 在LMMSE框架下进行理论分析，推导最优估计器的闭式表达式，建立与Tikhonov正则化的等价关系，证明收敛结果和推导有限样本误差界。

Result: 得出闭式解、建立等价关系、证明收敛性、得到有限样本误差界，通过数值实验验证了理论预测的收敛行为。

Conclusion: 在LMMSE框架下对盲逆问题学习的理论分析有效，可为解决实际应用问题提供可靠理论保障。
...
TL;DR: 本文提出DBAW - PIKAN网络解决PINNs在处理多尺度或高频特征问题时的梯度和表示瓶颈，经数值测试有显著优势。


<details>
  <summary>Details</summary>
Motivation: PINNs在处理多尺度或高频特征问题时存在梯度流刚度和频谱偏差问题，限制其预测能力。

Method: 提出DBAW - PIKAN网络，结合基于可学习B样条的Kolmogorov - Arnold网络架构和带动态衰减上限的自适应加权策略。

Result: 相比基线模型，加速收敛过程，至少提高一个数量级的解精度，且不增加计算复杂度。

Conclusion: DBAW - PIKAN在提高精度和泛化性能方面有显著优势。
...
TL;DR: 为解决现有GAN方法在合成负载模式的局限性，提出Cluster Aggregated GAN框架，实验显示该框架性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有GAN方法将所有设备统一处理，忽略间歇性和连续性电器行为差异，导致训练不稳定和输出保真度有限，需新方法解决。

Method: 提出Cluster Aggregated GAN框架，根据电器行为特征将其分配到专门分支，对间歇性电器使用聚类模块，连续性电器采用LSTM生成器。

Result: 在UVIC数据集上实验表明，该框架在真实性、多样性和训练稳定性指标上优于基线方法，聚类可提高可解释性和可扩展性。

Conclusion: 该框架是无创负荷监测研究中合成负载生成的有效方法。
...
TL;DR: 本文介绍Co - GRPO，将MDM生成重新表述为统一的马尔可夫决策过程，协同优化模型和推理计划参数，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前Masked Diffusion Models (MDMs) 训练和推理程序存在差异，推理计划在训练中未得到优化。

Method: 引入Co - GRPO，将MDM生成重新表述为统一的马尔可夫决策过程 (MDP)，应用Group Relative Policy Optimization协同优化模型参数和计划参数。

Result: 在四个基准测试（ImageReward、HPS、GenEval和DPG - Bench）上的实验结果证明了方法的有效性。

Conclusion: Co - GRPO可以更彻底地使训练与推理保持一致，大幅提高生成质量。
...
TL;DR: 研究组织在用户效用、部署成本和合规要求共同影响下如何选择AI模型，提出ML Compass框架并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有能力排行榜不能直接用于模型部署决策，存在能力 - 部署差距，需从系统层面解决模型选择问题。

Method: 开发ML Compass框架，将模型选择视为能力 - 成本前沿上的约束优化问题，理论上分析最优模型配置，实践中提出实施流程。

Result: 通过两个案例研究验证，框架产生的建议和排行榜与仅基于能力的排名有显著差异。

Conclusion: ML Compass框架能弥合能力 - 部署差距，明确能力、成本和安全之间的权衡对最优模型选择的影响。
...
TL;DR: 本文介绍了自监督分布式学习范式Osmotic Learning (OSM - L)，它能从分布式数据中挖掘潜在知识，实验验证了其能力。


<details>
  <summary>Details</summary>
Motivation: 分布式系统中相互依赖的数据源存在隐藏关系和潜在结构，有必要挖掘其中有价值的信息。

Method: 提出OSM - L范式，核心是渗透过程，通过提取上下文信息合成紧凑表示，迭代对齐本地数据表示，还能识别相关数据组。

Result: 实验表明OSM - L在结构化数据集上有收敛和表示能力，本地信息对齐准确率超0.99且保留上下文完整性。

Conclusion: OSM - L能有效从分布式数据中挖掘更高层次的潜在知识。
...
TL;DR: 研究使用双机器学习框架分析零工数据，发现人力资源实践与工作绩效关系是非单调的，部分定义的控制会造成混乱，清晰规则可使强监督可行。


<details>
  <summary>Details</summary>
Motivation: 探究在算法担任管理角色时以人为本的管理能否生存，标准工具常因工人对算法系统的响应非线性而无法捕捉情况。

Method: 使用双机器学习框架估计调节中介模型，不施加限制性函数形式，分析464名零工的调查数据。

Result: 发现明显的非单调模式，支持性人力资源实践能改善员工幸福感，但在算法监督存在却难以解释时，其与绩效的联系减弱；监督透明可解释时，关系又加强。

Conclusion: 简单线性规范会遗漏模式甚至得出相反结论；平台设计中部分定义的控制会造成混乱，清晰规则和可靠追索能使强监督可行；双机器学习可用于组织研究中估计条件间接效应而无需强制数据呈线性。
...
TL;DR: 机器学习是AI基础，虽模型能力强但分布偏移是弱点，研究聚焦分布偏移下可信机器学习，提出解决方案。


<details>
  <summary>Details</summary>
Motivation: 分布偏移限制机器学习系统可靠性和实用性，引发信任问题，需提升AI鲁棒性等。

Method: 研究三种常见分布偏移（扰动、领域、模态），从鲁棒性、可解释性、适应性三方面研究可信度。

Result: 提出有效解决方案和基础见解。

Conclusion: 研究有助于增强机器学习的效率、适应性和安全性等关键问题。
...
TL;DR: 本文揭示不平衡重叠子图的不公平问题，提出FairGFL算法解决该问题，实验表明其在模型效用和公平性上优于基线算法。


<details>
  <summary>Details</summary>
Motivation: 此前研究未探索图数据中客户端间不平衡重叠数据的负面影响，本文旨在解决此带来的不公平问题。

Method: 提出FairGFL算法，采用可解释加权聚合方法，利用隐私保护估计重叠率，在联邦复合损失函数中加入正则化器。

Result: 在四个基准图数据集上的实验显示，FairGFL在模型效用和公平性上优于四个代表性基线算法。

Conclusion: FairGFL能在保护隐私的同时，提升跨客户端公平性并维持模型效用。
...
TL;DR: 针对现有图异常检测方法不足，提出MHSA - GNN，经实验验证效果良好。


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测方法处理复杂异常模式有挑战，光谱图神经网络存在使用固定全局共享滤波器易过平滑、缺乏适应性的问题。

Method: 提出MHSA - GNN，设计轻量级超网络根据光谱指纹动态生成切比雪夫滤波器参数，引入结合TSC和BTD的双正则化策略。

Result: 在四个真实数据集上的实验表明，能有效保留高频异常信号，显著优于现有先进方法，在高度异构数据集上有出色鲁棒性。

Conclusion: MHSA - GNN能有效解决现有图异常检测方法的问题，具有良好性能和鲁棒性。
...
TL;DR: 提出Splitwise框架用于大语言模型在边缘和云环境的细粒度、自适应分区，实验显示其能降低延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 边缘设备部署大语言模型因资源有限有挑战，云推理有高延迟和成本问题，静态边缘云分区有局限性，需新的分区方案。

Method: 提出基于Lyapunov辅助的深度强化学习框架Splitwise，将transformer层分解，采用分层DRL策略，有分区检查点确保鲁棒性。

Result: 在多种设备和模型上实验表明，相比现有分区器，Splitwise降低端到端延迟1.4 - 2.8倍，最多削减能耗41%；相比仅云执行，降低95%分位延迟53 - 61%，且保持准确性和适度内存需求。

Conclusion: Splitwise能有效解决大语言模型在边缘和云环境的分区问题，实现低延迟、低能耗和高准确性。
...
TL;DR: 训练数据中的警告框架内容无法让语言模型避免不良行为，原因与特征正交化失败有关，训练时特征消融可解决该问题。


<details>
  <summary>Details</summary>
Motivation: 探究训练数据中的警告框架内容为何无法让语言模型避免警告行为。

Method: 进行实验对比模型表现，用稀疏自动编码器分析特征激活情况并研究‘隐形滑移’现象。

Result: 接触警告的模型和直接接收内容的模型重现标记内容的比率无显著差异；‘描述X’和‘执行X’激活重叠潜在特征；存在‘隐形滑移’现象；提示和推理时引导无法解决问题，训练时特征消融可以。

Conclusion: 当前架构中统计共现主导，模型学习跟随上下文的内容而非出现原因。
...
TL;DR: 传统无监督域适应方法在处理信息不平等的领域时存在缺陷，本文提出基于Le Cam理论的决策理论框架，用Le Cam Distortion实现无源域退化的迁移学习，在多实验中表现良好，为负迁移不可接受的领域提供风险可控的迁移学习框架。


<details>
  <summary>Details</summary>
Motivation: 指出传统无监督域适应（UDA）方法在处理信息不平等的领域时，严格的特征不变性会导致信息破坏和负迁移，在安全关键应用中可能造成灾难性后果，因此需要新的方法。

Method: 提出基于Le Cam理论的决策理论框架，用建设性近似将对称不变性替换为方向可模拟性，引入Le Cam Distortion作为迁移风险的严格上界，通过学习核函数从源域模拟目标域。

Result: 在基因组学、视觉、强化学习五个实验中，Le Cam Distortion实现了HLA基因组近乎完美的频率估计、CIFAR - 10图像分类零源效用损失、强化学习控制中安全的策略迁移。

Conclusion: Le Cam Distortion为负迁移不可接受的领域（如医学成像、自主系统和精准医学）提供了首个有原则的风险可控迁移学习框架。
...
TL;DR: 本文提出混合量子 - 经典专家混合体（QMoE）架构，验证干涉假设，实验显示量子路由器优势，分析其抗噪声能力并探讨实际应用。


<details>
  <summary>Details</summary>
Motivation: 传统专家混合体（MoE）架构存在专家不平衡和经典路由机制计算复杂的问题，研究量子机器学习解决这些局限的潜力。

Method: 采用量子门控网络（路由器）与经典专家结合进行消融研究，利用量子特征映射和波干涉。

Result: 验证干涉假设，量子路由器在非线性可分数据上有拓扑优势，分析其抗量子噪声的鲁棒性。

Conclusion: QMoE架构可行，适用于近期中等规模量子（NISQ）硬件，在联邦学习等领域有应用潜力。
...
TL;DR: 研究整合多种技术分析美国158号公路5年交通事故数据，识别时空事故模式，随机森林分类器预测伤害严重程度表现良好，结果支持针对性干预以提升交通安全。


<details>
  <summary>Details</summary>
Motivation: 为美国158号公路提供改善安全的可行见解，通过方法改进促进对农村公路安全分析的更广泛理解。

Method: 整合高级统计分析、机器学习和空间建模技术，应用核密度估计、负二项回归、随机森林分类和公路安全手册安全性能函数比较。

Result: 随机森林分类器预测伤害严重程度准确率达67%，优于HSM SPF；莫兰指数检验证实空间聚类，KDE分析揭示主要交叉口附近的热点。

Conclusion: 研究结果支持对该重要运输通道采取针对性干预措施以改善交通安全。
...
TL;DR: 提出结合随机特征与受控微分方程的时间序列学习框架，有两种变体，理论上有对应核，实验表现好。
...
Method: 结合随机特征与CDEs，提出RF - CDEs和R - RDEs两种变体。

Result: 模型在时间序列基准测试中有竞争力或达到了当前最优性能。

Conclusion: 方法是显式签名计算的实用替代方案，兼具归纳偏置和随机特征的效率。
...
TL;DR: 本文提出自适应MLOps驱动的信用风险预测决策系统PDx，解决传统模型问题，验证其有效性和适应性。


<details>
  <summary>Details</summary>
Motivation: 传统PD模型忽视借款人行为变化的持续适应，金融机构难将ML模型从开发环境过渡到生产并维护，因此开发PDx解决这些局限。

Method: 采用动态端到端模型生命周期管理方法，通过强大的MLOps管道集成持续模型监控、再训练和验证；引入动态冠军 - 挑战者框架更新基线模型。

Result: 决策树集成模型分类违约者表现好但需频繁更新，线性模型和神经网络性能退化明显；PDx可减少数字贷款人价值侵蚀。

Conclusion: PDx能有效解决传统模型问题，在不同贷款数据集上验证了其可扩展性和适应性，适用于现代信用风险预测。
...
TL;DR: 提出LLMBoost集成微调框架，利用大语言模型中间状态提升性能，实验证明可提高准确率并降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型集成学习方法常将模型视为黑盒，忽略内部表示和交互，本文旨在打破这一限制。

Method: 引入LLMBoost框架，包含跨模型注意力机制、链式训练范式和近并行推理范式，并建立理论基础。

Result: 在常识推理和算术推理任务实验中，LLMBoost持续提高准确率并降低推理延迟。

Conclusion: LLMBoost框架有效提升了大语言模型集成学习的性能。
...
TL;DR: 研究在人口统计均等和服务率约束下，从多臂老虎机反馈中学习一维阈值策略，提出OFS方法并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统在公平性和服务约束下运行会产生反馈效应，需研究在线学习策略。

Method: 提出Optimistic Feasible Search (OFS)，基于网格的方法，维护候选阈值的奖励和约束残差的置信区间，选择可行且乐观奖励最大的阈值。

Result: 在多个基准测试中，OFS比无约束和原始对偶多臂老虎机基线获得更高奖励和更小累积约束违反，接近最优可行固定阈值。
...
TL;DR: 提出语言感知多模态临近预报框架LangPrecip，引入大规模多模态数据集LangPrecip - 160k，实验显示优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 短期降水临近预报存在不确定性和约束不足问题，现有生成方法对未来运动约束弱且模糊。

Method: 提出LangPrecip框架，将临近预报表述为整流流范式下的语义约束轨迹生成问题，实现文本和雷达信息在潜空间的高效物理一致集成，引入LangPrecip - 160k数据集。

Result: 在瑞典和MRMS数据集实验中，相比现有方法有持续改进，在80分钟提前时间的强降雨CSI指标上分别提升超60%和19%。

Conclusion: LangPrecip框架和LangPrecip - 160k数据集有效提升短期降水临近预报效果。
...
TL;DR: 本文指出概率知识图谱嵌入的实体方差与关系无关的局限性，提出不确定性分解方法，CAGP方法在多个基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 发现概率知识图谱嵌入中实体不确定性与关系无关的问题，该问题混淆了新兴实体和新关系上下文两种不同的分布外现象。

Method: 将不确定性分解为语义不确定性和结构不确定性，CAGP方法通过学习权重结合二者。

Result: CAGP方法在多个基准的时间分布外检测中达到0.94 - 0.99 AUROC，相对提升60 - 80%，在选择性预测中85%回答率下减少43%错误。

Conclusion: 提出的不确定性分解信号非冗余，结合二者的方法效果显著优于关系无关的基线方法。
...
TL;DR: 本文将全球M2流动性作为外生变量，用TimeXer架构构建预测模型，实验表明该模型显著提升比特币长周期价格预测准确性。


<details>
  <summary>Details</summary>
Motivation: 比特币价格预测具有高波动性和非平稳性，传统单变量时间序列模型难以进行长周期预测，存在研究缺口。

Method: 将18个主要经济体的全球M2流动性作为有12周滞后结构的外生变量，使用TimeXer架构构建流动性条件预测模型（TimeXer - Exog），并与LSTM、N - BEATS等基准模型对比。

Result: 在2020年1月至2025年8月的比特币日价格数据实验中，70天预测期时，TimeXer - Exog模型均方误差为1.08e8，比单变量TimeXer基线模型性能提升超89%。

Conclusion: 基于全球流动性对深度学习模型进行条件设定，可大幅改善比特币长周期价格预测。
...
TL;DR: 参数高效微调方法LoRA在指令微调时可能导致模型能力下降，但对训练过程做小调整可解决该问题。


<details>
  <summary>Details</summary>
Motivation: 研究参数高效微调方法LoRA在指令微调时导致模型能力灾难性下降的问题及解决办法。

Method: 采用正则化近似重放方法，惩罚与初始模型的KL散度，并穿插来自不同但相似开放语料库的下一个标记预测数据。

Result: 应用于Qwen指令调优模型时，该方法在增加少量计算开销的情况下，能保留模型的通用知识且不影响对新任务的适应性。

Conclusion: 对训练过程做小调整可解决LoRA指令微调时模型能力下降的问题。
...
TL;DR: 提出AFA - LoRA训练策略，通过退火激活函数使LoRA有非线性表达能力，减少与全参数训练性能差距。


<details>
  <summary>Details</summary>
Motivation: LoRA的线性适应过程限制其表达能力，存在线性训练和非线性训练表达能力的差距，需缩小这一差距。

Method: 提出AFA - LoRA，采用退火激活函数，在训练中从非线性过渡到线性变换，并在监督微调、强化学习和推测解码中实现该方法。

Result: AFA - LoRA减少了LoRA和全参数训练之间的性能差距。

Conclusion: 该工作使参数高效适应有了更强大和实用的范式。
...
TL;DR: 本文提出AMBIT灰色框框架，结合物理移动基线和可解释树模型进行OD流预测，展示了方法优势并提供相关分析工具。


<details>
  <summary>Details</summary>
Motivation: 解决OD流预测中高精度和可解释性的矛盾需求。

Method: 先对经典空间交互模型在纽约出租车OD数据集上审计，然后在物理基线基础上用梯度提升树和SHAP分析构建残差学习器。

Result: 基于物理的残差接近强树基预测器的精度且保留可解释结构，基于POI的残差在空间泛化下有竞争力和鲁棒性。

Conclusion: 提供了适用于城市决策的可复现流程、丰富诊断和空间误差分析。
...
TL;DR: 针对多预训练专家模型共存时新目标域扩展问题，提出无梯度的GLUE方法，在多数据集和架构实验中表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有新目标域扩展方法中启发式混合准确率低，基于目标损失学习系数计算开销大。

Method: 提出GLUE方法，将目标模型初始化为固定专家的凸组合，通过无梯度两点（SPSA）更新学习混合系数。

Result: 在三个数据集和三种网络架构实验中，GLUE生成的单一先验可有效微调，相比数据大小加权最高提升8.5%准确率，相比代理指标选择最高提升9.1%，与基于反向传播的全梯度混合相当或更优。

Conclusion: GLUE方法在新目标域扩展中有效，能提升测试准确率。
...
TL;DR: 提出多类不平衡学习的协作优化Boosting模型，在20个公开数据集上表现优于8个基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有研究未探索不平衡学习与模型训练的协作优化，限制了性能提升。

Method: 提出协作优化Boosting模型，集成密度和置信因子，设计抗噪权重更新机制和动态采样策略，模块紧密集成。

Result: 在20个公开不平衡数据集上，模型显著优于8个最先进的基线模型。

Conclusion: 所提模型实现了不平衡学习和模型训练的协作优化，性能良好。
...
TL;DR: 提出自适应物联网威胁检测框架，训练分类器一次，用对齐模型处理输入流量，结合图神经网络分类，在概念漂移下检测性能稳健。


<details>
  <summary>Details</summary>
Motivation: 现有基于AI的物联网威胁检测模型依赖固定数据集，无法适应概念漂移，现有解决方案需周期性重训练分类器，计算开销大且有灾难性遗忘风险。

Method: 对历史流量的潜在空间表示训练一次分类器，用对齐模型将输入流量映射到历史潜在空间，将低维潜在表示转换为图结构格式并用图神经网络分类。
...
TL;DR: 研究彩票假说是否适用于低秩自适应（LoRA）方法，发现成立并提出Partial - LoRA，实验证明其可减少参数并维持或提高准确率。


<details>
  <summary>Details</summary>
Motivation: 随着对预训练模型微调的依赖增加，探究彩票假说是否适用于参数高效微调（PEFT），特别是LoRA方法。

Method: 研究LoRA中彩票假说是否成立，提出Partial - LoRA方法来识别子网络并训练稀疏低秩适配器。

Result: 实验表明在8个视觉和12个语言任务中，Partial - LoRA最多可减少87%的可训练参数，同时维持或提高准确率。

Conclusion: 结果加深了对迁移学习、预训练和微调相互作用的理论理解，为开发更高效适配策略开辟新途径。
...
TL;DR: 研究在多选口腔修复学考试中预测大语言模型回答正确性的可行性，利用元数据和幻觉信号构建预测器，发现能提升准确率，提示策略影响模型行为，但现有方法用于高风险场景尚不够稳健。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在高风险领域应用时生成错误信息风险大，预测其回答正确性是关键且待探索的问题。

Method: 分析通用模型GPT - 4o和推理中心模型OSS - 120B，利用三种提示策略下的元数据和幻觉信号为每个（模型，提示）对构建正确性预测器。

Result: 基于元数据的方法可将准确率提高达7.14%，精度达83.12%；实际幻觉是回答错误的强指标，元数据信号单独不能可靠预测幻觉；提示策略不影响整体准确率，但显著改变模型内部行为和元数据预测效用。

Conclusion: 为开发大语言模型可靠性信号提供了有前景的方向，但本文探索的方法用于关键高风险部署还不够稳健。
...
TL;DR: 本文提出任务向量分解方法，分离出共享和独特信息，在三个领域验证其有效性，为控制任务向量运算提供新框架。


<details>
  <summary>Details</summary>
Motivation: 现有任务向量算术运算中存在概念重叠干扰问题，需要更精确控制概念操作的方法。

Method: 提出分解方法，将任务向量分为共享知识和特定任务独特信息两部分，通过识别投影中的不变子空间实现。

Result: 在图像分类、扩散模型和语言模型三个领域验证了方法有效性，如提升多任务合并效果、实现风格混合、降低语言模型毒性。
...
TL;DR: 本文提出评估SNN对抗鲁棒性的可靠框架，提升攻击成功率，揭示当前SNN鲁棒性被高估。


<details>
  <summary>Details</summary>
Motivation: SNN的脉冲激活的二进制和不连续特性导致梯度消失，现有替代梯度方法在强对抗攻击下的有效性不明确。

Method: 理论分析替代梯度的梯度消失程度，引入自适应锐度替代梯度（ASSG）；设计带自适应步长的对抗攻击（SA - PGD）。

Result: 在多种对抗训练方案、SNN架构和神经元模型上显著提高攻击成功率。

Conclusion: 当前SNN的鲁棒性被显著高估，需要更可靠的对抗训练方法。
...
TL;DR: 本文提出统一编码 - 解码预测框架 TimePerceiver，结合有效训练策略，实验表明其性能优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测工作主要关注编码器设计，忽略预测和训练的整体考虑，本文欲提出综合解决方案。

Method: 将预测任务泛化，设计新的编码 - 解码架构，编码引入潜在瓶颈表征，解码使用可学习查询。

Result: 在多个基准数据集上，TimePerceiver 持续且显著优于先前的先进基线。

Conclusion: TimePerceiver 是一个有效且实用的时间序列预测框架，代码已开源。
...
TL;DR: 本文分析超2500条美国AV碰撞记录，用两阶段数据挖掘框架揭示碰撞动态，为相关方提供策略指导。


<details>
  <summary>Details</summary>
Motivation: AV在混合交通环境中的安全和可靠性存疑，以往研究数据有限、对各SAE自动化等级碰撞趋势理解不足。

Method: 开发两阶段数据挖掘框架，先用K - means聚类按时空和环境因素将碰撞记录分为4类，再用关联规则挖掘提取各簇中碰撞模式与影响因素的关系。

Result: 揭示了AV潜在的碰撞动态，提取了碰撞模式与影响因素的多元关系。

Conclusion: 研究成果可为AV开发者、安全监管者和政策制定者制定部署策略和降低碰撞风险提供可行指导。
...
TL;DR: 提出EnFlow框架解决分子构象生成和基态识别难题，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于物理的管道生成低能构象集和识别基态构象计算成本高，现有学习方法范式有缺陷。

Method: 将流匹配（FM）与显式学习的能量模型通过沿非高斯FM路径的能量引导采样方案耦合。

Result: 在GEOM - QM9和GEOM - Drugs上的实验表明，EnFlow仅用1 - 2个ODE步骤就改善生成指标，降低基态预测误差。

Conclusion: EnFlow能同时提高生成指标和降低基态预测误差，优于现有方法。
...
TL;DR: 本文提出用于加密货币价格预测的Parallel Gated Recurrent Units (PGRU) 模型，实验表明该模型比现有方法更准确高效。


<details>
  <summary>Details</summary>
Motivation: 加密货币价格波动大，投资者需预测价格方法，当前领域有此需求。

Method: 提出PGRU模型，用循环神经网络并行独立预测价格，各网络用不同价格相关特征输入，最后由神经网络组合输出预测未来价格。

Result: 模型在窗口长度20和15时，平均绝对百分比误差分别为3.243%和2.641%。

Conclusion: 该方法相比现有方法，用更少输入数据和更低计算成本实现更高准确性和效率。
...
TL;DR: 本文提出基于人工智能的黄金价格预测算法，结合LSTM和MLP网络，优化神经元数量，在实际交易中有高回报，预测误差小。


<details>
  <summary>Details</summary>
Motivation: 金融市场准确预测有挑战，需提供能准确预测市场未来的模型。

Method: 用两个LSTM网络分别进行日、月预测，结果集成到MLP网络得出次日价格预测；用GWO算法优化网络内部神经元数量；收集2010 - 2021年宏观经济等数据。

Result: 模型在三个月交易中回报率达171%；预测黄金日收盘价MAE为0.21美元，下月价格MAE为22.23美元。

Conclusion: 所提出的LSTM - MLP模型能较好地预测黄金价格，可用于实际市场交易。
...
TL;DR: 论文提出无客户端状态或控制变量的偏置压缩框架CAFe和CAFe - S，分析证明其优势，实验验证优于现有压缩方案。


<details>
  <summary>Details</summary>
Motivation: 分布式学习尤其是联邦学习通信成本高，现有带误差反馈的偏置压缩技术需客户端控制变量，侵犯隐私且不适用于无状态客户端。

Method: 提出CAFe框架，用前一轮全局聚合更新作共享控制变量；提出CAFe - S框架，在服务器有私有数据集时生成服务器引导的候选更新作更准确预测器。以DGD为代表算法进行分析证明。

Result: 分析证明CAFe在非凸有界梯度差异情况下优于DCGD，证明CAFe - S收敛到驻点且收敛速率随服务器数据代表性增强而提高，实验验证所提方法优势。

Conclusion: 提出的无客户端状态或控制变量的偏置压缩框架有效，优于现有压缩方案。
...
TL;DR: 提出Selective TTS框架，用于多阶段管道推理扩展，在数据科学管道任务中提升洞察力质量。


<details>
  <summary>Details</summary>
Motivation: 现实中多阶段管道任务最终结果缺乏可验证奖励或数据训练奖励模型，基于判断的细化易累积误差。

Method: 提出Selective TTS，跨多智能体管道不同阶段扩展推理，用特定阶段判断器分配计算资源和提前修剪低质量分支。

Result: 构建端到端多智能体管道，设计可靠判断模型，在固定计算预算下提升洞察力质量，平均分从61.64增至65.86且方差降低。

Conclusion: 研究为扩展无验证奖励的复杂开放性任务迈出第一步。
...
TL;DR: 本文首次将量子生成模型应用于计算流体动力学（CFD）数据的潜在空间表示，对比量子和经典生成方法，量子模型表现更好，并提供开源管道和研究基础。


<details>
  <summary>Details</summary>
Motivation: 此前量子模型用于学习流体系统统计特性，但结合离散潜在空间压缩和量子生成采样用于CFD尚未探索。

Method: 开发GPU加速的格子玻尔兹曼方法（LBM）模拟器生成流体涡度场，用向量量化变分自编码器（VQ - VAE）将其压缩到7维离散潜在空间，对比量子电路玻恩机（QCBM）、量子生成对抗网络（QGAN）和经典长短期记忆网络（LSTM）。

Result: 在实验条件下，两个量子模型生成样本与真实分布的平均最小距离小于LSTM，QCBM指标最优。

Conclusion: 提供连接CFD模拟和量子机器学习的开源管道，首次对物理模拟压缩潜在表示进行量子生成建模的实证研究，为该交叉领域未来研究奠定基础。
...
TL;DR: 研究低秩结构的去中心化多任务表示学习，提出新算法，分析复杂度并验证性能。


<details>
  <summary>Details</summary>
Motivation: 中心化方法研究充分，去中心化方法研究不足，需探索去中心化多任务表示学习。

Method: 提出交替投影梯度和最小化算法。

Result: 给出时间、通信和样本复杂度的全面刻画，通信复杂度与目标精度无关，减少通信成本；数值模拟验证理论分析，显示去中心化学习在某些情况下优于中心化联邦方法。

Conclusion: 新算法在去中心化多任务表示学习中有良好性能，能降低通信成本。
...
TL;DR: 本文研究了用p进数空间替代实数空间用于机器学习的可能性，建立了相关学习模型和算法，还举例说明p进数的优势并探讨未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习框架基于实数空间，作者探讨是否有其他可选空间，p进数的层次结构和对其作为无限字符串的解释使其成为编码理论和层次表示学习的有吸引力的工具。

Method: 开展探索性理论研究，建立基于p进数的分类、回归和表示学习的基本框架，提供学习模型和算法。

Result: 建立了基于p进数的学习模型和算法，展示了简单Quillian语义网络可表示为紧凑的p进数线性网络，而实数空间无法实现。

Conclusion: 提出了新的机器学习框架，讨论了该框架下的开放问题和未来研究机会。
...
TL;DR: 提出基于学习的框架预测极端事件停电，整合多源数据，评估四个机器学习模型，LSTM误差最低，经济和基建好停电少。


<details>
  <summary>Details</summary>
Motivation: 预测极端事件导致的停电，针对低概率高后果的停电场景，了解社区停电风险。

Method: 整合EAGLE - I停电记录与天气、社会经济等多源数据，采用随机森林、支持向量机、自适应提升和长短期记忆网络四个机器学习模型。

Result: 在密歇根州下半岛县的大规模数据集上实验，LSTM网络预测误差最低，经济条件好和基础设施发达地区停电少。

Conclusion: 提出的基于学习的框架可用于预测极端事件停电，LSTM模型效果好，经济和基建影响停电发生。
...
TL;DR: 本文探讨深度学习时间序列预测架构设计空间，分析设计维度与权衡，指出架构设计应基于预测原则，还提及被忽视的实现细节影响结果，提议用辅助预测模型卡刻画架构。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列中广泛应用，但新架构多且实证结果矛盾，难以评估各组件对性能的贡献，需理清设计空间。

Method: 讨论时间序列预测模型设计的维度和权衡，评估局部性和全局性概念在预测架构中的应用，分析现有架构中被忽视的实现细节。

Result: 考虑某些方面比采用特定序列建模层更能实现准确结果，简单设计良好的架构可媲美现有最优；部分实现细节会改变预测方法类别和影响实证结果。

Conclusion: 应反思当前错误的基准测试实践，设计架构时关注预测问题的基础方面，提出用辅助预测模型卡刻画架构。
...
TL;DR: 论文指出长时强化学习中现有上下文折叠方法存在问题，提出FoldAct框架解决这些问题，提升训练稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有长时强化学习的上下文折叠方法将摘要动作视为标准动作，忽略其对观测空间的影响，违反RL假设，带来梯度稀释、自调节和计算成本高等问题。

Method: 提出FoldAct框架，通过分离损失计算、全上下文一致性损失和选择性片段训练三个创新点解决问题。

Result: 该方法能实现具有上下文折叠的长时搜索代理的稳定训练，解决非平稳观测问题，训练效率提升5.19倍。

Conclusion: FoldAct框架有效解决长时强化学习上下文折叠的问题，提升训练稳定性与效率。
...
TL;DR: 用54028个合金样本同时预测电学电阻率、维氏硬度和非晶形成能力，对比单任务模型和MTL，发现MTL使回归性能下降但提升分类效果，建议回归用独立模型，分类用MTL。


<details>
  <summary>Details</summary>
Motivation: 验证多任务学习（MTL）假设，即相关材料属性共享潜在物理规律可用于更好预测。

Method: 使用54028个合金样本同时预测电学电阻率、维氏硬度和非晶形成能力，对比单任务模型、标准和结构化MTL。

Result: MTL显著降低回归性能，但提高分类效果；任务间权重接近零，表明属性独立；回归失败归因于严重数据不平衡导致的负迁移。

Conclusion: 精确回归使用独立模型，召回率关键的分类任务使用MTL。
...
TL;DR: 本文提出HEROSQL用于Text - to - SQL语义验证，用层次化表示和特定网络提升性能，实验显示优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Text - to - SQL系统SQL验证多关注语法正确性，语义验证面临捕捉信息和构建注释两大挑战。

Method: 引入HEROSQL，结合逻辑计划和抽象语法树；使用嵌套消息传递神经网络；提出基于抽象语法树的子SQL增强策略。

Result: 在Text - to - SQL验证基准测试中，平均AUPRC提升9.40%，AUROC提升12.35%，能检测细粒度语义错误。

Conclusion: 该方法可提升数据查询平台的可靠性和可解释性。
...
TL;DR: 研究第三方平台面对策略型代理的定价行为，开发算法解决需求学习问题，揭示供应端噪声影响需求可学习性，通过模拟和真实数据验证方法实用性。


<details>
  <summary>Details</summary>
Motivation: 研究第三方平台在面对策略型代理时的定价行为，解决一般需求学习问题。

Method: 开发具有最优遗憾的算法，利用非独立同分布行动作为工具变量，提出新颖的同胚构造。

Result: 揭示供应端噪声对需求可学习性的影响，导致遗憾的相变；为深度神经网络学习需求提供效率保证。

Conclusion: 所提方法具有实际应用价值，通过模拟和真实数据得到验证。
...
TL;DR: 本文引入微观 - 宏观机器学习框架，结合多尺度数据预测儿童肥胖风险，展示了整合多尺度数据识别肥胖风险环境差异的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统流行病学研究独立分析各层面因素，限制了对结构环境条件与个体特征如何相互作用影响健康结果的理解，因此开展此研究。

Method: 引入微观 - 宏观机器学习框架，整合个体层面和宏观层面数据，用四种机器学习模型预测肥胖，构建环境脆弱性综合指数。

Result: XGBoost 模型表现最佳，州层面环境负担高的地区与全国预测的微观层面肥胖风险分布有很强的地理相似性。

Conclusion: 整合多尺度数据集识别肥胖风险的环境差异是可行的，该建模流程有扩展到因果建模、干预规划和实时分析的潜力。
...
TL;DR: 现有TGNN可解释性方法存在通用性差、计算成本高、用户友好性低的问题，本文提出GRExplainer方法解决这些问题，实验显示其在多方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前TGNN缺乏可解释性限制其实践应用，且现有可解释性研究存在通用性、计算成本和用户友好性方面的问题。

Method: 提出GRExplainer方法，提取节点序列作为统一特征表示，利用广度优先搜索和时间信息构建节点序列，设计基于RNN的生成模型。

Result: 在六个真实数据集和三个目标TGNN上的实验表明，GRExplainer在通用性、效率和用户友好性方面优于现有基线方法。

Conclusion: GRExplainer是一种通用、高效且用户友好的TGNN解释方法。
...
TL;DR: 介绍受量子力学启发的Schrödinger AI统一机器学习框架，含三个组件，有良好实证结果，指明新方向。


<details>
  <summary>Details</summary>
Motivation: 提出替代传统交叉熵训练和Transformer注意力的物理驱动机器学习框架。

Method: 构建由时间无关波能量求解器、时间相关动态求解器和低秩算子演算三个紧密耦合组件构成的系统。

Result: 展示出无监督反映人类概念类关系的语义流形、适应环境变化的动态推理和模运算任务的精确算子泛化。

Conclusion: 为机器学习指明将学习视为发现和探索潜在语义能量景观的新基础方向。
...
TL;DR: 基于因果可迁移性理论设计零样本组合泛化算法，还设计无显式因果结构的监督域适应方案，有理论结果和模拟验证。


<details>
  <summary>Details</summary>
Motivation: 解决跨领域泛化问题，需要对目标域和源域进行结构约束。

Method: 基于因果可迁移性理论，设计零样本组合泛化算法Circuit - TR学习模块并迁移组合；设计无显式因果结构的监督域适应方案。

Result: 理论结果用图形电路可迁移性标准表征少样本可学习任务类别，将少样本泛化性与电路规模复杂性联系起来，模拟验证了理论结果。

Conclusion: 提出的算法和方案有助于解决跨领域泛化问题，理论结果得到模拟验证。
...
TL;DR: 通过对公共卫生干预的回顾性分析，研究中国新冠病毒传播模式，发现地区差异、就医时间和感染源变化情况。


<details>
  <summary>Details</summary>
Motivation: 通过综合回顾性分析公共卫生干预措施，找出最能缓解新冠疫情的机制。

Method: 利用公开追踪数据研究中国新冠传播模式，收集案例报告，应用NLP和人工整理构建传播/追踪链，结合人口流动数据量化和可视化传播动态。

Result: 地区差异大，大城市感染多，多数有症状者五天内就医，感染源随时间改变。

Conclusion: 公共卫生干预措施的效果有地区差异，感染源受不同时期活动影响。
...
TL;DR: 提出SNM - Net用于开放集气体识别，核心是几何解耦机制和引入马氏距离，实验表现优于现有方法，解决了精度与稳定性权衡问题。
...
Method: 提出SNM - Net框架，采用级联批量归一化和L2归一化的几何解耦机制，引入马氏距离作为评分机制，且与多种骨干网络兼容。

Result: Transformer + SNM配置在Vergara数据集上表现优异，AUROC达0.9977，未知气体检测率达99.57%，优于现有方法，框架在不同传感器位置稳定性好。

Conclusion: 该工作有效解决了精度与稳定性的权衡问题，为工业电子鼻部署提供技术基础。
...
TL;DR: 提出基于强化学习的扩散模型蒸馏框架，在推理步数和计算资源上性能更优


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型采样慢的问题

Method: 将蒸馏过程视为策略优化问题，用奖励信号训练学生模型，动态引导其探索多去噪路径

Result: 相比现有蒸馏技术，用更少推理步骤和计算资源取得更优性能

Conclusion: 该框架与模型无关，为高效扩散学习提供通用优化范式
...
TL;DR: 本文介绍Mixture - of - Representations (MoR)量化框架，基于此提出动态选择FP8和BF16表示的算法，初测98.38%的张量量化到FP8可达到SOTA结果，表明动态感知属性量化有潜力。


<details>
  <summary>Details</summary>
Motivation: 混合精度训练很重要，但需找到合适训练方法组合，探索可提升模型量化训练效果的新方法。

Method: 提出Mixture - of - Representations (MoR)量化框架，基于此提出动态选择FP8和BF16表示的具体算法，对不同粒度进行量化。

Result: 该方法能实现98.38%的张量量化到FP8格式并达到SOTA结果。

Conclusion: 该动态感知属性量化方法有潜力在保持模型质量的同时提升低精度训练鲁棒性，也可与其他方法结合利用更低精度格式。
...
TL;DR: 提出长程蒸馏方法，用教师模型生成大量合成数据训练学生模型进行长程天气预报，在实验和现实中均有良好表现，证明AI生成合成数据可提升长程预报技能。


<details>
  <summary>Details</summary>
Motivation: 现有AI长程天气预报模型因误差累积和训练数据有限面临挑战，长时步概率模型训练易过拟合。

Method: 引入长程蒸馏方法，用短步自回归教师模型生成大量合成训练数据，训练长时步概率学生模型直接进行长程预报。

Result: 在完美模型实验中，蒸馏模型表现优于气候学，接近教师模型；在现实中，经ERA5微调后S2S预报技能与ECMWF集合预报相当，且技能随合成训练数据增加而提升。

Conclusion: 首次证明AI生成的合成训练数据可用于提升长程预报技能。
...
TL;DR: 提出带时间方差驱动课程的师生学习范式加速目标条件强化学习，在多任务中效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在单目标任务成功，但多目标场景中统一目标选择导致样本低效，需更高效方法。

Method: 提出师生学习范式，教师模块动态优先处理策略置信分数时间方差最高的目标，建立Q值时间方差与策略演变的理论联系，且与现有框架集成。

Result: 在11个不同的机器人操作和迷宫导航任务评估中，相比现有课程学习和目标选择方法有显著且一致的改进。

Conclusion: 提出的方法算法无关，能有效加速目标条件强化学习，在多目标场景表现出色。
...
TL;DR: 本文提出 Federated Multi - Task Clustering (FMTC) 框架，解决现有聚类算法在去中心化环境的不足，实验表明该框架性能优于其他算法。


<details>
  <summary>Details</summary>
Motivation: 现有谱聚类算法多用于集中式环境，在去中心化环境不适用，且当前联邦学习方法泛化能力差、无法捕捉客户端潜在关联。

Method: 提出 FMTC 框架，含客户端个性化聚类模块和服务器端张量相关模块；基于交替方向乘子法推导分布式算法来解决联合优化问题。

Result: 在多个真实数据集上的实验表明，FMTC 框架显著优于各种基线和最先进的联邦聚类算法。

Conclusion: FMTC 框架能为异构客户端学习个性化聚类模型，同时以隐私保护的方式利用共享结构，有效解决了现有算法的局限。
...
TL;DR: 本文提出基于动态图的GraphLogDebugger框架用于调试表格日志，简单动态GNN比大语言模型效果更好。


<details>
  <summary>Details</summary>
Motivation: 现有处理富文本表格日志数据的方法过度依赖大语言模型等重载模型，灵活性和可扩展性有限。

Method: 提出GraphLogDebugger框架，通过为对象和事件构建异构节点并连接边，将表格日志背后的系统恢复为动态图，使用简单动态GNN进行调试。

Result: 在计算机系统和学术论文的真实日志数据集实验中，简单动态GNN在调试表格日志方面优于大语言模型。

Conclusion: 基于动态图的GraphLogDebugger框架在调试表格日志上比依赖大语言模型的方法更有效。
...
TL;DR: 本文提出元学习框架MetaCD解决数据长尾分布和动态变化问题，实验表明其准确性和泛化性优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有认知诊断方法因数据长尾分布和动态变化问题，性能受限。

Method: 提出基于持续学习的元学习框架MetaCD，用元学习解决长尾问题，用参数保护机制的持续学习方法适应数据动态变化。

Result: 在五个真实数据集上的综合实验表明，MetaCD在准确性和泛化性上优于其他基线。

Conclusion: MetaCD既提高模型在单个任务上的可塑性，又保证在顺序任务中的稳定性和泛化性。
...
TL;DR: 提出Sat - EnQ两阶段框架解决深度Q学习算法不稳定问题，理论证明其优势，实验显示有良好效果。


<details>
  <summary>Details</summary>
Motivation: 深度Q学习算法不稳定，尤其是早期训练时最大化算子会放大估计误差。

Method: 引入Sat - EnQ两阶段框架，第一阶段在满意目标下训练轻量级Q网络集成，用动态基线限制早期价值增长；第二阶段将集成提炼到更大网络并用标准Double DQN微调。

Result: 实现3.8倍方差减少，消除灾难性失败，在环境噪声下保持79%性能，所需计算量比自举集成少2.5倍。

Conclusion: 在优化前采用满意策略是实现稳健强化学习的可行途径。
...
TL;DR: 提出Multiple Token Divergence (MTD)衡量语言模型计算工作量，引入Divergence Steering解码方法，MTD比先前方法更有效。


<details>
  <summary>Details</summary>
Motivation: 现有衡量语言模型上下文计算工作量的指标无法捕捉推理复杂性，先前基于潜在状态可压缩性的方法有侵入性且不稳定。

Method: 提出MTD，基于预训练多预测头模型直接计算；引入Divergence Steering解码方法。

Result: MTD在区分复杂和简单任务上比先前方法更有效，在数学推理基准上与问题难度正相关，较低MTD与更准确推理相关。

Conclusion: MTD为分析和引导语言模型计算动态提供实用轻量级工具。
...
TL;DR: 介绍Alpha - Divergence Preference Optimization (APO)框架，用Csiszar alpha - divergence在同一锚定几何中插值，实验表明其有竞争力且训练稳定。


<details>
  <summary>Details</summary>
Motivation: 现有现代对齐实践的两种散度机制各有优缺点，锚定方法通常只采用单一散度，需要改进。

Method: 引入APO框架，使用Csiszar alpha - divergence在同一锚定几何中连续插值，推导统一梯度动力学，提出奖励和置信度保护的alpha调度。

Result: 在Qwen3 - 1.7B和math - level3实验中，APO与GRPO和GSPO基线相比有竞争力且训练稳定。

Conclusion: APO是一种有效的方法，能在保证训练稳定性的同时实现较好性能。
...
TL;DR: 因隐私等限制，工作 - 生活平衡和幸福感的纵向数据有限，本文引入合成纵向数据集FLOW及可配置数据生成工具，用于相关研究。


<details>
  <summary>Details</summary>
Motivation: 解决因隐私、伦理和后勤限制导致的纵向个体层面工作 - 生活平衡和幸福感数据获取受限问题，以支持可重复性研究、方法基准测试和教育。

Method: 使用基于规则、反馈驱动的模拟生成FLOW数据集，模拟1000人两年的每日数据，并开发可配置的数据生成工具。

Result: 生成了FLOW合成纵向数据集并公开，开发了可配置数据生成工具。
...
TL;DR: 本文提出单通道EEG睡眠分期的上下文感知和可解释框架，结合多尺度特征提取与时间建模，处理数据不平衡问题，在SleepEDF数据集上取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 自动睡眠分期重要，现有方法存在类别不平衡、感受野建模有限和可解释性不足等问题，需改进N1阶段检测。

Method: 结合紧凑多尺度特征提取与时间建模，应用类加权损失函数和数据增强，将EEG信号分段，平均软最大概率得到最终预测。

Result: 整体准确率89.72%，宏平均F1分数85.46%，N1阶段F1分数61.7%，优于先前方法。
...
TL;DR: 对19种多模态学习方法进行大规模实证研究，提出简单基线模型SimBaMM，发现复杂架构未必优于它，呼吁关注方法严谨性。


<details>
  <summary>Details</summary>
Motivation: 挑战多模态特定方法能提升性能的假设，探究复杂架构的实际效果。

Method: 在标准化条件下重新实现19种方法，在9个数据集上评估，测试泛化能力，提出SimBaMM模型。

Result: 复杂架构在严格调参下未必优于SimBaMM，常无法可靠超越单模态基线。

Conclusion: 应从追求架构新颖性转向注重方法严谨性。
...
TL;DR: 提出一种新的持续学习方法，将LoRA更新正交初始化并顺序合并为统一LoRA，能保持恒定内存复杂度、减少任务干扰并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有低秩适应（LoRA）持续学习技术存在计算内存增长、存储受限和任务干扰等问题。

Method: 正交初始化并顺序合并LoRA更新为单一统一LoRA，利用正交基提取初始化新任务学习，使用时间感知缩放机制平衡新旧知识。

Result: 在多个持续学习基准测试中，使用不同Llama模型进行大量实验，证明了方法的有效性和效率。

Conclusion: 该方法能保持恒定内存复杂度，通过正交基初始化减少新旧任务干扰，通过自适应缩放提升非对称LoRA合并性能。
...
TL;DR: 本文通过机制可解释性研究FedAvg算法，发现非IID数据分布导致局部电路结构差异，使全局模型中电路退化，将统计漂移问题转化为机制保存问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非IID数据条件下性能显著下降，但其内部机制原因不明，需探究。

Method: 通过机制可解释性视角研究FedAvg算法，训练可解释的权重稀疏神经网络，用IoU量化电路保存情况。

Result: 发现非IID数据分布使结构不同的局部电路发散，导致全局模型中电路退化。

Conclusion: 将联邦学习中的统计漂移问题重新定义为机制保存的具体可观察的失败问题，为更有针对性的解决方案铺平道路。
...
TL;DR: 提出物理感知多模态基础模型（PI - MFM）框架用于PDE求解，在多方面表现优于纯数据驱动模型，还能实现零样本微调。


<details>
  <summary>Details</summary>
Motivation: 现有多算子学习方法数据需求大且训练时忽略物理信息，需改进。

Method: 提出PI - MFM框架，以符号表示PDE为输入，通过向量化导数计算自动组装PDE残差损失，以统一物理感知目标进行训练和调整。

Result: 在13个一维时变参数PDE族基准测试中表现更优，物理损失增强抗噪性，重采样策略提高精度，零样本微调效果好。

Conclusion: PI - MFM为数据高效、可迁移的PDE求解器提供了实用且可扩展的途径。
...
TL;DR: 提出Phase Gradient Flow (PGF)框架解决选择性状态空间模型梯度分析的内存瓶颈问题，实现低内存复杂度和高吞吐量，可用于染色体尺度敏感性分析。


<details>
  <summary>Details</summary>
Motivation: 选择性状态空间模型基于梯度的敏感性分析在反向传播时存在内存瓶颈，无法在消费级硬件上进行基因组尺度建模。

Method: 引入PGF框架，直接在状态空间流形中计算精确解析导数，将SSM动态重构为Tiled Operator - Space Evolution (TOSE)。

Result: 实现了与序列长度相关的O(1)内存复杂度，峰值VRAM减少94%，吞吐量提高23倍，在极端序列中保持近机器精度。

Conclusion: PGF能使单GPU进行染色体尺度敏感性分析，弥合理论无限上下文模型和实际硬件限制之间的差距。
...
TL;DR: 现有MoE模型在联邦学习中部署存在两大挑战，提出FLEX - MoE框架应对，实验证明其性能优越且能保持专家负载平衡。


<details>
  <summary>Details</summary>
Motivation: 解决混合专家（MoE）模型在联邦学习部署中面临的边缘设备无法存储全量专家集和非IID数据分布导致的专家负载不平衡问题。

Method: 提出FLEX - MoE框架，引入客户端 - 专家适应度分数量化专家对本地数据集的适用性，采用基于优化的算法来最大化客户端 - 专家专业化并确保系统范围内专家的平衡利用。

Result: 在三个不同数据集上的综合实验表明，FLEX - MoE具有优越性能，能在不同资源受限场景中保持专家利用平衡。

Conclusion: FLEX - MoE框架能有效解决MoE模型在联邦学习中的两大关键挑战，优于现有的贪婪方法。
...
TL;DR: 本文从结构重参数化角度重新思考视觉 - 语言模型（VLMs）微调，应用掩码微调（MFT）方法，实验表明 MFT 表现优于 LoRA 变体和全量微调。


<details>
  <summary>Details</summary>
Motivation: 现有 VLMs 微调方法多依赖显式权重更新，忽略了预训练模型中已编码的大量表征结构，而 MFT 在语言模型中展现出强大高效性，因此想将其应用于 VLMs 微调。

Method: 将 MFT 应用于不同语言骨干的 VLMs 的语言和投影组件，并与强大的参数高效微调（PEFT）基线进行比较。

Result: MFT 始终超越 LoRA 变体，甚至超越全量微调，在不改变冻结骨干的情况下实现高性能。

Conclusion: 有效的模型适配不仅可以通过更新权重实现，还可以通过重新建立模型现有知识之间的连接来实现。
...
TL;DR: 现有自监督学习方法在情感计算的多模态表征学习中存在不足，本文提出MFMC框架，实验显示其性能出色且稳健。


<details>
  <summary>Details</summary>
Motivation: 情感计算中多模态表征学习面临挑战，且情感标注稀缺主观，现有自监督学习方法难以处理多模态依赖和高阶交互。

Method: 提出Multimodal Functional Maximum Correlation (MFMC)框架，通过Dual Total Correlation (DTC)目标最大化高阶多模态依赖，利用基于FMCA的迹替代优化。

Result: 在三个公开情感计算基准上，MFMC在主体依赖和主体独立评估协议下表现出色，如提升CEAP - 360VR的准确率。

Conclusion: MFMC框架有效，能直接捕捉联合多模态交互，对主体间变异性具有鲁棒性。
...
TL;DR: 提出整合模仿学习和强化学习的大语言模型微调统一框架，将梯度分解为两部分。


<details>
  <summary>Details</summary>
Motivation: 为大语言模型微调提供更有效的方法。

Method: 分析结合轨迹级KL散度与任务奖励的复合目标梯度，将其分解为可计算的密集梯度和蒙特卡罗估计的稀疏梯度。

Result: 得出两种梯度，且密集梯度有封闭形式的对数级公式，便于GPU高效实现。

Conclusion: 所提出的框架在大语言模型微调方面有一定优势和可行性。
...
TL;DR: 本文提出SE - MLP架构快速预测侵彻加速度特征值，经对比实验验证其性能优越，数值模拟和测试表明该方法可行且具工程适用性。


<details>
  <summary>Details</summary>
Motivation: 传统获取侵彻加速度特征值依赖长模拟周期和高计算成本，需新方法克服此局限。

Method: 提出融合通道注意力机制和残差连接的SE - MLP架构，以不同工况物理参数为输入，输出层间加速度特征，建立物理参数与侵彻特性的非线性映射。

Result: 对比实验显示SE - MLP在预测精度、泛化性和稳定性上更优；消融研究表明通道注意力模块和残差结构对性能提升贡献大；数值模拟和测试中，预测与实测的加速度峰值和脉宽差异在工程公差内。

Conclusion: 所提方法可行且具工程适用性，为侵彻引信快速生成先验特征值提供实践依据。
...
TL;DR: 引入带变压器融合的时间感知图神经网络模型(GNN - TF)，结合多种数据预测青少年未来烟草使用情况，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 整合非欧几里得脑成像数据和欧几里得表格数据，特别是在纵向研究中预测未来结果存在挑战。

Method: 引入GNN - TF模型，灵活整合表格数据和动态脑连接数据，利用纵向数据集进行全面分析。

Result: 对比分析显示GNN - TF在预测未来烟草使用上优于现有机器学习和深度学习模型，有更高预测精度。

Conclusion: GNN - TF的端到端时间感知结构可融合多数据模态、利用时间动态，是临床结果预测功能脑成像研究的有价值工具。
...
TL;DR: 本文构建首个弱信号特征学习专用数据集，提出PDVFN模型，实验表明其在处理弱信号等情况时准确性和鲁棒性更高，为弱信号学习研究奠定基础。


<details>
  <summary>Details</summary>
Motivation: 弱信号学习缺少专用数据集制约研究，在有强信号的任务中有效提取弱信号也是提升模型性能的关键。

Method: 构建含13158个光谱样本的数据集，提出双视图表示和PDVFN模型，并行提取局部序列特征和全局频域结构。

Result: 方法在处理弱信号、高噪声和极端类别不平衡情况，尤其低信噪比和不平衡场景下，准确性和鲁棒性更高。

Conclusion: 本研究提供了专用数据集、基线模型，为未来弱信号学习研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有表示学习研究中，分散式方法探索不足，需开发相关算法。

Method: 提出交替投影梯度下降和最小化算法，以扩散式分散和联邦方式恢复低秩特征矩阵。

Result: 获得算法样本复杂度下界和迭代复杂度上界，证明算法快速且通信高效，数值模拟验证性能。

Conclusion: 所提分散投影梯度下降算法适用于多任务表示学习，有良好性能和复杂度表现。
...
TL;DR: 文章系统评估了在RLVR范式下的PEFT方法，发现结构变体如DoRA等优于LoRA，还揭示了一些方法的问题并提供了参数高效的RL方法探索指南。


<details>
  <summary>Details</summary>
Motivation: RLVR激励语言模型提高推理能力，但最适合RLVR的PEFT架构未知，需进行评估。

Method: 对DeepSeek - R1 - Distill系列上的12种以上PEFT方法在数学推理基准上进行综合评估，还进行了消融研究和扩展实验。

Result: 结构变体DoRA等始终优于LoRA；SVD初始化策略存在谱塌缩现象；极端参数减少严重限制推理能力。

Conclusion: 为参数高效的RL方法的更多探索提供明确指导。
...
TL;DR: 提出基于HELM符号的首个编码器肽语言模型HELM - BERT，在下游任务中表现优于现有模型，显示出HELM表示法的优势。


<details>
  <summary>Details</summary>
Motivation: 现有分子语言模型无法准确捕捉治疗性肽的理化性质复杂性，需要更好的表示方法。

Method: 提出基于DeBERTa的HELM - BERT模型，在39,079个化学多样的肽语料库上预训练。

Result: HELM - BERT在下游任务中显著优于基于SMILES的语言模型。

Conclusion: HELM的表示法在治疗性肽建模上有数据效率优势，弥合了小分子和蛋白质语言模型的差距。
...
TL;DR: 提出Physics - Geometry Operator Transformer (PGOT)处理PDEs建模难题，在多基准和工业任务表现佳


<details>
  <summary>Details</summary>
Motivation: 现有高效架构处理大规模非结构化网格时会因特征降维导致几何混叠，丢失物理边界信息

Method: 提出PGOT并设计Spectrum - Preserving Geometric Attention，采用“物理切片 - 几何注入”机制，能根据空间坐标调整计算路径

Result: PGOT在四个标准基准测试中达到了先进水平，在机翼和汽车设计等大规模工业任务中表现出色

Conclusion: PGOT能够解决现有架构问题，实现空间自适应和高精度的物理场建模
...
TL;DR: 本文提出FedOLF框架，结合有序层冻结和张量运算近似，在非独立同分布数据上比现有方法精度更高，且能效高、内存占用低。


<details>
  <summary>Details</summary>
Motivation: 物联网边缘设备计算、内存和带宽有限，现有联邦学习框架在减少开销时牺牲精度或忽略内存限制。

Method: 提出FedOLF，训练前按预定义顺序冻结层，引入张量运算近似（TOA）。

Result: 在非独立同分布数据上，FedOLF在多个数据集和模型上比现有工作精度分别至少高0.3%、6.4%、5.81%、4.4%、6.27%和1.29%，能效更高、内存占用更低。

Conclusion: FedOLF能有效解决物联网边缘设备联邦学习的效率和可扩展性问题，在精度、能效和内存占用方面表现良好。
...
TL;DR: 传统统计方法处理FAERS含偏数据预测药物不良反应有误差，本文提出基于联邦学习的PFed - signal模型，利用欧氏距离处理偏置数据，经实验模型性能优于基线。


<details>
  <summary>Details</summary>
Motivation: 传统基于统计的方法不能消除FAERS含偏数据，导致药物不良反应信号预测不准确。

Method: 提出Pfed - Split将原始数据集按ADR拆分；提出ADR - signal，含基于联邦学习的偏置数据识别法与基于Transformer的ADR预测模型，前者用欧氏距离识别并删除偏置数据，后者在无偏数据集训练。

Result: 干净数据集上ROR和PRR优于传统方法，PFed - Signal准确率0.887、F1分数0.890等，高于基线。

Conclusion: 基于联邦学习的PFed - signal模型能提高药物不良反应预测准确性。
...
TL;DR: 研究具有有限指数矩分布间的流匹配逆问题，在一维和高斯情形下确定解的唯一性，多维问题待研究。


<details>
  <summary>Details</summary>
Motivation: 现代生成式AI应用，如流匹配模型蒸馏。

Method: 未提及

Result: 在一维和高斯情形下建立了解的唯一性。
...
TL;DR: 本文研究了HC - PINNs训练动力学的理论机制，建立NTK框架，揭示边界函数作用，指出有效秩可预测训练收敛，还发现常见边界函数问题并提供设计新思路。


<details>
  <summary>Details</summary>
Motivation: HC - PINNs虽能严格执行边界条件，但训练动力学的理论机制未被探索。

Method: 建立严格的Neural Tangent Kernel (NTK)框架，进行谱分析。

Result: 发现边界函数作为光谱滤波器重塑神经网络本征核的特征谱；有效秩可作为训练收敛的确定性预测指标；常见边界函数可能导致光谱崩溃。

Conclusion: 该框架将边界函数设计转化为有原则的光谱优化问题，为科学机器学习中的几何硬约束提供理论基础。
...
TL;DR: 本文延续之前CNN反向传播矢量化推导工作，将轻量级无索引方法应用于基于Transformer的下一个token预测架构，给出多种层的梯度表达式及极简GPT网络的PyTorch实现。


<details>
  <summary>Details</summary>
Motivation: 手动进行反向传播，能让我们在尝试对损失函数求导时发现前向传播理解上的差距，加深对各操作如何影响最终输出的直觉。

Method: 将轻量级无索引方法论应用于嵌入层、多头自注意力层、层归一化层等新类型层，并给出LoRA层的梯度表达式。

Result: 给出了多种层的梯度表达式，完成了极简GPT网络所有梯度更新的解析表达式，提供了完整的PyTorch实现。

Conclusion: 手动反向传播有助于深入理解模型中各操作对最终输出的影响。
...
TL;DR: 本文提出多模型协作定律，通过实验揭示多模型系统的性能缩放规律，表明模型协作可拓展大语言模型智能边界。


<details>
  <summary>Details</summary>
Motivation: 现有单一大语言模型能力有上限，多模型集成技术缺乏统一性能缩放理论框架。

Method: 提出多模型协作定律，采用与方法无关的公式，假设理想化集成预言机确定样本总交叉熵损失。

Result: 多模型系统随总参数数量呈幂律缩放，比单模型缩放有更显著提升趋势和更低理论损失下限，异构模型家族集成性能缩放更佳。

Conclusion: 模型协作是拓展大语言模型智能边界的关键途径。
...
TL;DR: 提出ECG - RAMBA框架用于心电图分类，分离并融合形态和节律特征，在多数据集表现良好，消融实验表明各部分对跨域鲁棒性有不同作用。


<details>
  <summary>Details</summary>
Motivation: 深度学习在心电图分类跨采集设置泛化性差，现有模型架构存在形态和节律特征纠缠问题，阻碍临床应用和纵向监测。

Method: 提出ECG - RAMBA框架，结合MiniRocket提取形态特征、计算心率变异性的节律描述符、用双向Mamba骨干进行长程上下文建模，引入Power Mean池化算子；采用受试者层面交叉验证等评估方案。

Result: 在Chapman - Shaoxing数据集上macro ROC - AUC约0.85；零样本迁移中在CPSC - 2021数据集上房颤检测PR - AUC = 0.708，优于对比基线；在PTB - XL数据集上跨数据集性能稳定。

Conclusion: 确定性形态特征是基础，显式节律建模和长程上下文是跨域鲁棒性的关键驱动因素。
...
TL;DR: 介绍Isometric Policy Optimization (ISOPO) 方法，实现单步近似自然策略梯度，计算开销小。


<details>
  <summary>Details</summary>
Motivation: 现有近端策略方法如GRPO或CISPO需多步梯度近似自然梯度，希望提出单步有效近似方法。

Method: 最简单形式在与优势函数收缩前用Fisher度量归一化每个序列的对数概率梯度；另一种变体基于每层的神经正切核转换微批次优势函数，逐层应用转换。

Result: ISOPO可在单步反向传播中完成操作，与普通REINFORCE相比，计算开销可忽略不计。

Conclusion: ISOPO是一种高效的能在单步近似自然策略梯度的方法。
...
TL;DR: 本文针对openPangu-Embedded模型在Ascend NPUs上推理时的计算约束，采用低比特量化方法，提出统一低比特推理框架，评估显示该方法能有效促进高效推理并保持高模型保真度。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型的CoT模式在Ascend NPUs上部署时存在内存和延迟开销大的计算约束问题。

Method: 利用低比特量化将FP16计算转换为整数运算，引入支持INT8和W4A8量化的统一低比特推理框架。

Result: INT8量化能保持超90%的FP16基线准确率，在Atlas A2上实现1.5倍预填充加速；W4A8量化显著降低内存消耗，但精度有一定损失。
...
TL;DR: 本文提出结合生成模型与神经算子的框架高效构建高分辨率速度模型，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统速度模型构建方法计算成本高、耗时长，深度学习方法可解决传统方法局限。

Method: 提出结合生成模型与神经算子的框架，用神经算子作正向映射算子生成图像，通过自动微分更新迁移速度，嵌入生成模型作正则化器。

Result: 合成和实地数据实验表明该方法有效。

Conclusion: 所提出的基于生成神经算子的速度模型构建方法是有效的。
...
TL;DR: 提出 CoLog 框架用于日志异常检测，结合多模态分析，实验显示优于现有方法，适用于多领域。


<details>
  <summary>Details</summary>
Motivation: 单模态方法忽略日志数据不同模态信息，多模态方法难以处理模态间交互，需更好的日志异常检测方法。

Method: 将多模态情感分析应用于日志异常检测，提出 CoLog 框架，利用协作变压器和多头注意学习模态间交互，加入模态适应层处理异质性。

Result: 在七个日志异常检测基准数据集上，检测点和集体异常的平均精度 99.63%，平均召回率 99.59%，平均 F1 分数 99.61%。

Conclusion: CoLog 是日志异常检测的重大进展，通过统一框架有效解决了点和集体异常检测问题，为日志数据分析提供解决方案。
...
TL;DR: 提出频率引导图结构学习（FgGSL）框架，结合频率信息与监督拓扑推理，在六个异质性基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在学习异质图的判别性节点表示上存在困难，连接节点标签往往不同，特征相似性提供的结构线索较弱。

Method: 提出端到端的图推理框架FgGSL，联合学习同质和异质图结构与谱编码器，用可学习的掩码函数推断互补图，用高低通图滤波器组处理，用基于标签的结构损失促进边的恢复。

Result: 在六个异质性基准测试中，FgGSL始终优于最先进的图神经网络和图重连方法。

Conclusion: 结合频率信息和监督拓扑推理对图结构学习有益。
...
TL;DR: 文章将大语言模型训练的神经缩放定律扩展到“一次训练，多种模型”的范式，引入粒度（G）作为新变量，通过实验得到粒度惩罚遵循乘幂律，验证“一次训练，多次部署”范式。


<details>
  <summary>Details</summary>
Motivation: 现有的神经缩放定律通常假设单一密集模型输出，忽略了对跨异质设备 - 边缘 - 云层次实现普适智能至关重要的家族模型范式，因此需扩展缩放定律。

Method: 引入粒度（G）作为基本缩放变量，提出统一函数形式L(N, D, G)并通过大规模实验进行参数化，采用严格的IsoFLOP实验设计，系统扫描不同模型大小（N）和粒度（G）并动态调整训练令牌（D）。

Result: 结果显示粒度惩罚遵循具有极小指数的乘幂律。

Conclusion: 理论上连接了固定计算训练与动态架构，实践中验证了“一次训练，多次部署”范式，证明可在不影响密集基线计算最优性的前提下实现部署灵活性。
...
TL;DR: 提出绕过优化瓶颈的方法，可大幅压缩分类头且性能损失小，引出子空间原生蒸馏范式。


<details>
  <summary>Details</summary>
Motivation: 当前先进模型虽有低内在维度，但仍依赖高维宽度解决非凸优化搜索问题，需绕过此优化瓶颈。

Method: 提出一种构造性方法，将解的几何结构与环境搜索空间解耦。

Result: 在ResNet - 50、ViT和BERT上，分类头可压缩达16倍且性能损失可忽略不计。

Conclusion: 提出子空间原生蒸馏新范式，为学生模型提供稳定几何坐标系，有望实现大训练小部署。
...
TL;DR: 提出STAMP框架，可编码时间信息，在多数据集上评估，在疾病进展预测中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法如MAE缺乏时间感知能力，无法捕捉纵向医疗数据集中疾病进展。

Method: 提出STAMP，即随机时间自编码器与掩码预训练框架，通过随机过程编码时间信息，将MAE重建损失重构为条件变分推理目标。

Result: 在两个OCT和一个MRI数据集上评估，STAMP预训练的ViT模型在不同晚期年龄相关性黄斑变性和阿尔茨海默病进展预测中优于现有时间MAE方法和基础模型。

Conclusion: STAMP能有效学习疾病潜在的非确定性时间动态，在疾病进展预测方面有优势。
...
TL;DR: 本文针对Mixture of Experts模型的问题提出Dynamic Subspace Composition（DSC）框架，降低了参数复杂度和内存流量，并给出动态更新的最坏情况边界，代码开源。


<details>
  <summary>Details</summary>
Motivation: 解决Mixture of Experts（MoE）模型存在的表示坍缩和梯度不稳定问题。

Method: 提出DSC框架，通过共享基库的状态相关稀疏扩展近似上下文相关权重，将权重更新建模为星形域内的残差轨迹，采用幅度门控单纯形插值确保恒等连续性，由解耦的单位范数基向量构建组合秩K近似。

Result: 将参数复杂度降低到O(M d)，内存流量降低到O(Kd)，并通过框架理论正则化和谱约束给出动态更新的严格最坏情况边界。

Conclusion: DSC框架在解决MoE模型问题上有较好效果，减少了参数和内存开销，并能保证一定的更新边界。
...
TL;DR: 现有奖励模型（RM）训练数据质量低、有归纳偏差，本文提出信息论去偏方法DIR，实验验证其能缓解偏差、提升性能且泛化性好。


<details>
  <summary>Details</summary>
Motivation: RM训练数据质量差，存在归纳偏差易致过拟合和奖励破解，现有去偏方法有局限，需缓解更复杂多样的归纳偏差。

Method: 引入信息论去偏方法DIR，受信息瓶颈启发，最大化RM分数与人类偏好对的互信息，最小化RM输出与偏好输入偏差属性的互信息。

Result: 通过实验验证DIR对响应长度、谄媚和格式三种归纳偏差有效，能缓解目标归纳偏差，提升不同基准上的RLHF性能，有更好的泛化能力。

Conclusion: DIR能处理具有非线性相关性的复杂偏差，扩展了RM去偏方法在现实中的应用场景。
...
TL;DR: 提出新微调方法FRoD，结合分层联合分解和旋转自由度，提升效率和表达能力，在多基准测试中表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法如LoRA存在收敛慢和适应能力有限的问题，无法很好捕捉复杂模式。

Method: 提出FRoD方法，结合分层联合分解与旋转自由度，提取全局共享基并注入稀疏可学习扰动。

Result: 在20个涵盖视觉、推理和语言理解的基准测试中，FRoD准确率与全模型微调相当，仅使用1.72%的可训练参数。

Conclusion: FRoD能增强表达能力和效率，实现更快更稳健的收敛。
...
TL;DR: 针对IIoT网络多设备动态URLLC在CSI不完善情形，提出BO驱动的TD3方法并设计训练机制，仿真显示效果好。


<details>
  <summary>Details</summary>
Motivation: 在IIoT网络中CSI不完善，且URLLC网络存在误差样本不平衡及TD3算法参数敏感问题，影响收敛速度和可靠性，需提出优化方法。

Method: 提出BO驱动的TD3方法，根据不完善CSI自适应确定设备服务顺序和MCS；还提出基于BO的训练机制解决误差样本不平衡等问题。

Result: 通过大量仿真表明，所提算法比现有方案收敛更快、和速率性能更高。

Conclusion: 所提的基于BO的TD3方法及训练机制能在CSI不完善的IIoT网络中有效提高多设备动态URLLC性能。
...
TL;DR: 提出 EGDA 框架解决脑电情感识别跨会话差异问题，在 SEED - IV 数据集表现良好，并分析出关键频段和脑区。


<details>
  <summary>Details</summary>
Motivation: 脑电情感识别中跨会话差异影响模型泛化。

Method: 提出 EGDA 框架，联合对齐全局和特定类分布，用图正则化保留脑电数据内在结构。

Result: 在 SEED - IV 数据集三个迁移任务上准确率分别达 81.22%、80.15% 和 83.27%，超多个基线方法。

Conclusion: EGDA 框架能实现稳健跨会话表现，Gamma 频段最具区分性，中央 - 顶叶和前额叶脑区对情感识别至关重要。
...
TL;DR: 提出VL - RouterBench基准评估VLM路由系统，评估10种路由方法和基线，发现有改进空间并将开源工具链。


<details>
  <summary>Details</summary>
Motivation: 现有工作缺乏系统、可复现的基准来评估视觉语言模型（VLMs）的多模型路由。

Method: 基于VLMs的原始推理和评分日志构建基准，覆盖多数据集和模型，评估协议联合测量平均准确率、平均成本和吞吐量并构建排名分数。

Result: 评估10种路由方法和基线有显著路由增益，当前最佳路由器与理想Oracle有差距。

Conclusion: VLM路由系统的路由器架构有改进空间，将开源工具链促进多模态路由研究的可比性、可复现性和实际部署。
...
TL;DR: 文章介绍混合框架结合共形预测增强传统统计过程控制（SPC），提出两个新应用，为质量控制提供更稳健严谨方法。


<details>
  <summary>Details</summary>
Motivation: 传统SPC依赖统计假设，在现代复杂制造环境监测不可靠，需改进。

Method: 引入混合框架，结合共形预测的分布自由和模型无关特性，提出共形增强控制图和共形增强过程监测两个新应用。

Result: 框架能提供更稳健和统计严谨的质量控制方法，且保持经典方法的可解释性和易用性。

Conclusion: 该框架是一种有效的质量控制方案，可解决传统SPC的局限性。
...
TL;DR: 大语言模型在软件工程问题泛化能力不足，本文提出BOAD框架构建分层多智能体系统，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理现实世界软件工程问题时泛化能力差，现有系统的单智能体设计存在不足，受人类工程师解决复杂问题方式启发。

Method: 将层级发现问题建模为多臂老虎机问题，提出Bandit Optimization for Agent Design (BOAD) 框架，高效探索子智能体设计。

Result: 在SWE - bench - Verified上，BOAD优于单智能体和手动设计的多智能体系统；在SWE - bench - Live上，36B系统排名第二，超过GPT - 4和Claude等大模型。

Conclusion: 自动发现的分层多智能体系统显著提高了具有挑战性的长期软件工程任务的泛化能力。
...
TL;DR: 将长上下文语言建模视为持续学习问题，用标准Transformer架构，通过测试时学习和元学习改进，实验显示TTT - E2E在长上下文表现优且推理快。


<details>
  <summary>Details</summary>
Motivation: 以新视角解决长上下文语言建模问题，避免单纯依赖架构设计。

Method: 将长上下文语言建模作为持续学习问题，用带滑动窗口注意力的Transformer架构，测试时持续学习，训练时用元学习改进初始化。

Result: TTT - E2E在3B模型和164B标记训练下，上下文长度扩展表现与全注意力Transformer相同，推理延迟恒定，比全注意力快2.7倍。

Conclusion: 提出的TTT - E2E方法在长上下文语言建模中有效且高效，代码公开可复用。
...
TL;DR: 研究利用现有论文语料训练语言模型生成更好研究计划，构建训练语料并通过强化学习训练，经多方面验证有效。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型难以生成符合所有约束和隐含要求的研究计划，需提升AI协同科学家生成研究计划的能力。

Method: 自动从多领域论文中提取研究目标和评分准则构建训练语料，通过自评分强化学习训练模型。

Result: 人类专家对微调后模型生成的研究计划偏好度达70%，认可84%自动提取的评分准则，在多领域有12 - 22%相对提升和跨领域泛化能力。

Conclusion: 可扩展的自动化训练方法对提升通用AI协同科学家有潜力。
...
TL;DR: 提出CosineGate用于残差网络动态路由，在CIFAR - 10上取得计算量减少且准确率匹配或超越ResNet - 20的效果。


<details>
  <summary>Details</summary>
Motivation: 现代深度残差网络对每个输入评估所有残差块存在大量冗余计算，即使恒等映射就足够。

Method: 引入CosineGate，用恒等和残差特征表示之间的余弦不兼容性作为自监督跳过信号，通过余弦不兼容率（CIR）衡量语义冗余，使用Gumbel - Softmax松弛在训练时实现逐样本、逐块门控，用渐进FLOPs正则化项控制平均计算用量。

Result: 在CIFAR - 10上，不同配置在减少计算量的同时达到不同准确率，匹配或超越ResNet - 20。

Conclusion: 特征不兼容性的简单几何度量为动态残差路由提供了有原则且有效的信号。
...
TL;DR: 研究小transformer模型修复真实Java漏洞及语法正确性是否代表语义正确性，模型语法正确性高但精确匹配修复为零。


<details>
  <summary>Details</summary>
Motivation: 探究小transformer模型能否有效修复真实Java漏洞，以及语法正确性是否可作为语义正确性的可靠指标。

Method: 在CodeXGLUE的52,364个Java漏洞修复对上微调CodeT5-small模型，使用AST解析评估token级性能和语法有效性。

Result: 模型收敛良好，语法正确性高，约94%生成语法有效Java代码，但精确匹配评估中修复为零，约80%复制错误输入。

Conclusion: 仅依靠小transformer模型在精确修复真实Java漏洞上存在明显不足，语法正确性不能可靠代表语义正确性。
...
TL;DR: 本文对高速公路场景下基于目标的激光雷达攻击中纵向安全控制器进行系统性故障分析，揭示自动驾驶系统感知与控制安全的差距，为设计安全机制和控制策略提供见解。


<details>
  <summary>Details</summary>
Motivation: 以往研究虽表明激光雷达感知易受攻击，但此类攻击对车辆安全控制器的影响尚不明确。

Method: 使用集成激光雷达感知、目标跟踪和闭环车辆控制的高保真仿真框架，聚焦现实的切入和跟车场景，评估虚假和错位目标检测在感知、规划和控制流程中的传播。

Result: 短时间激光雷达引发的目标幻觉可导致不安全制动、对真实危险响应延迟和控制不稳定，切入场景中不安全减速事件和碰撞时间违规增加，控制器故障受伪造目标时间一致性影响更大。

Conclusion: 自动驾驶系统感知鲁棒性和控制层面安全保障存在关键差距，研究为依赖激光雷达的自动驾驶车辆设计安全机制和控制策略提供实用见解。
...
TL;DR: 针对大语言模型在Text - to - SQL生成中产生幻觉的问题，提出不依赖标准答案的SQLHD幻觉检测方法，实验证明其性能优越。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在Text - to - SQL生成中会产生幻觉，影响下游应用，现有检测方法在应用于大语言模型时因缺乏真实数据而受限。

Method: 提出基于变形测试的SQLHD方法，将检测任务分为两个阶段，通过多种变形关系检测幻觉，在各阶段分别调用大语言模型生成结果并进行交叉检查。

Result: 实验中F1分数在69.36% - 82.76%，性能优于大语言模型自我评估方法，能有效识别幻觉。

Conclusion: SQLHD方法在检测Text - to - SQL任务中的幻觉方面表现出色。
...
TL;DR: 本文对基于大语言模型的智能体软件问题解决研究进行系统调查，介绍任务流程、建立分类，指出智能体强化学习带来范式转变，总结挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 现实软件问题解决复杂，需超越传统单步方法的智能体能力，LLM 智能体系统成主流，故开展相关研究。

Method: 对 126 项前沿研究进行系统调查，按基准、技术、实证研究三个维度建立分类。

Result: 明确任务一般流程，指出智能体强化学习带来软件工程中智能体系统设计与训练范式转变。

Conclusion: 总结关键挑战并给出未来研究有前景的方向。
...
TL;DR: 研究大语言模型生成代码在干净环境中的可执行性，发现只有68.3%项目可直接运行，不同语言差异大且存在大量隐藏依赖。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为编码代理虽有望加速软件开发，但对生成代码的可重复性影响未充分研究，故开展此研究。

Method: 对Claude Code、OpenAI Codex和Gemini三个大语言模型编码代理，用100个标准化提示生成300个Python、JavaScript和Java项目，引入三层依赖框架量化执行可重复性。

Result: 只有68.3%的项目可直接运行，不同语言差异大（Python 89.2%，Java 44.0%），声明到实际运行依赖平均膨胀13.5倍，存在大量隐藏依赖。

Conclusion: 大语言模型生成代码的可执行性存在问题，不同语言表现不同且有大量隐藏依赖。
...
TL;DR: 通过对20个氛围编码视频研究，揭示氛围编码行为、挑战及心理模型影响，为软件工程研究和实践提供方向。


<details>
  <summary>Details</summary>
Motivation: 了解开发者如何定义和参与基于大语言模型的“氛围编码”实践。

Method: 对20个氛围编码视频进行扎根理论研究，包括活动时长和提示意图分析。

Result: 发现氛围编码行为谱，存在不同使用AI方式；都需应对生成随机性；不同心理模型影响策略、评估和信任。

Conclusion: 研究结果为软件工程研究指明新方向，为工具设计和教育提供实践机会。
...
TL;DR: 提出GraphLocator方法解决软件问题定位中症状与原因、一对多不匹配问题，实验显示其效果好。


<details>
  <summary>Details</summary>
Motivation: 解决软件问题定位中，问题描述与源代码实现间语义差距导致的症状与原因、一对多不匹配问题。

Method: 提出GraphLocator方法，构建因果问题图（CIG），分症状顶点定位和动态CIG发现两阶段工作。

Result: 在三个真实数据集实验中，GraphLocator在函数级召回率和精度平均提升+19.49%和+11.89%；在两类不匹配场景下召回率和精度均有提升；CIG让下游解决任务性能提升28.74%。

Conclusion: GraphLocator能有效解决软件问题定位中的两类不匹配问题，提升定位准确性。
...
TL;DR: 提出MultiConf方法自动隔离编译器故障，通过构建多对对抗编译配置和SBFL公式定位故障，在GCC编译器bug基准测试中效果和效率优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现代编译器基础设施复杂，定位编译器故障到特定源文件极具挑战，需有效方法。

Method: 构建多对对抗编译配置（包含失败和通过配置），用SBFL公式对编译器源文件可疑度排名，通过加权投票汇总排名。

Result: 在60个真实GCC编译器bug的基准测试中，MultiConf在效果和效率上显著优于现有技术，前1位文件级别成功定位27个bug，优于Odfl(20)和Basic(21)。

Conclusion: MultiConf方法能更准确、稳健地进行编译器故障定位。
...
TL;DR: 研究基于学习的自动漏洞修复（AVR）技术，指出当前模型和评估指标问题，并提出三种改进方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于微调语言模型的AVR技术在修复未见漏洞能力存疑，模型过拟合且评估集不互斥，匹配指标有局限。

Method: 一是对测试集应用语义保留转换；二是重新划分训练、验证和测试集；三是引入基于测试的基准L - AVRBench。
...
TL;DR: 提出CFIghter系统，在GNU项目评估中解决构建错误，修复大量违规，证明自动兼容性修复让严格编译器CFI可实际部署。


<details>
  <summary>Details</summary>
Motivation: 编译器基控制流完整性（CFI）在大型C/C++软件中部署因可见性不匹配等问题存在挑战。

Method: CFIghter集成全程序分析和引导式运行时监控，迭代应用最小必要调整，直至测试通过或失败无法解决。

Result: 在四个GNU项目评估中，解决可见性构建错误，自动修复95.8%违规，多数代码保持严格类型CFI，无需手动改源码。

Conclusion: 自动兼容性修复使严格编译器CFI可在成熟模块化C软件中实际部署。
...
TL;DR: 论文指出大语言模型使非程序员也能创建应用，但破坏了软件工程安全假设，提出RSA策略利用大语言模型生成漏洞利用代码，测试成功率100%，表明软件工程安全原则需重新设计。


<details>
  <summary>Details</summary>
Motivation: 探讨公开可用的大语言模型如何被社会工程化，使新手成为攻击者，挑战传统漏洞利用需技术专长的原则。

Method: 提出RSA（角色分配、场景借口、行动请求）策略，对Odoo平台测试五个主流大语言模型。

Result: 对五个主流大语言模型测试成功率达100%，每个测试的CVE在3 - 4轮提示内至少产生一个可用的漏洞利用代码，且消除了人工操作的开销。

Conclusion: 传统软件工程安全原则失效，需为只需编写提示而非理解代码就能进行攻击的时代重新设计安全实践。
...
TL;DR: 传统代码优化方法存在局限，机器学习方法成本高难扩展，本文提出低成本高效框架FasterPy优化Python代码，实验显示其性能优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 传统规则式代码优化方法劳动密集、适用性有限，机器学习方法开发成本高难扩展，需新方法，而大语言模型为代码优化带来新途径。

Method: 提出FasterPy框架，结合基于知识库的检索增强生成（RAG）和低秩自适应（LoRA）来优化Python代码执行效率。

Result: 在PIE基准测试中，该方法在多个指标上优于现有模型。

Conclusion: FasterPy是一种低成本、高效的优化Python代码的方法，可有效提升代码执行效率。
...
TL;DR: 本文介绍团队幸福感分析项目结果及项目内开发的原型，指出团队成员幸福感受认可程度影响。


<details>
  <summary>Details</summary>
Motivation: 软件开发团队成员幸福感对工作效率、绩效、身体健康和个人生活都很关键，且成员被认可程度是影响幸福感的重要因素。

Method: 未提及

Result: 呈现了团队幸福感分析项目的结果以及开发的原型。
...
TL;DR: 提出集成混合深度学习模型MobResTaNet的AI诊断软件，可直接从超声图像对胆囊疾病分类，能实时预测，准确率高，已部署为应用。


<details>
  <summary>Details</summary>
Motivation: 早期准确检测胆囊疾病很关键，但超声解读有挑战，需要有效方法辅助诊断。

Method: 使用混合深度学习模型MobResTaNet进行分类，结合可解释AI（XAI）可视化技术。

Result: 系统达到99.85%的准确率，仅2.24M参数，已部署为网页和移动应用。

Conclusion: 该软件能提供高效、可及且值得信赖的即时诊断支持。
...
TL;DR: 提出GLiSE工具解决软件工程灰色文献大规模收集和评估难题，介绍工具、提供数据集并进行可用性实证研究。


<details>
  <summary>Details</summary>
Motivation: 软件工程灰色文献对研究重要，但因来源、格式和API异构，大规模收集和评估困难。

Method: 开发GLiSE工具，将研究主题提示转换为特定平台查询，从常见软件工程网络源收集结果，用基于嵌入的语义分类器过滤和排序。

Result: 提供软件工程灰色文献搜索结果的精选数据集，按语义相关性分类。

Conclusion: 未明确提及，但通过实证研究可评估GLiSE工具的可用性。
...
TL;DR: 研究GenAI在软件工程（SE）中应用现状，发现其广泛应用且有诸多好处，但存在挑战，从业者预计角色将被重新定义。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏GenAI在SE实践中应用的实证，该研究旨在了解其在SE中的采用状况、好处、挑战及影响。

Method: 未提及

Result: GenAI工具被广泛应用并深度融入日常SE工作，从业者认为有诸多益处，但客观衡量生产力和质量有限，存在输出不可靠、提示工程困难等挑战，工具和技术制度化常见但有差异，从业者预计角色被重新定义且有一定担忧。

Conclusion: GenAI在SE中应用广泛但面临挑战，将对从业者角色产生影响而非替代。
...
TL;DR: 文章通过对Hugging Face和GitHub的讨论进行实证研究，构建识别安全讨论的管道，得出312,868个安全讨论数据集，分析出32个安全问题和24个解决方案，为AI供应链安全提供指导。


<details>
  <summary>Details</summary>
Motivation: 人工智能安全格局复杂，开发者面临传统和新的特定安全威胁，且缺乏对常见安全问题及解决方式的了解，阻碍了有效安全措施的开发。

Method: 基于Hugging Face和GitHub的讨论，开发结合关键词匹配与微调distilBERT分类器的管道来识别安全相关讨论，对数据集抽样进行主题分析。

Result: 得到312,868个安全讨论数据集，分析出包含32个安全问题和24个解决方案的细粒度分类法，发现很多安全问题源于AI组件的复杂依赖和黑盒特性，模型和数据相关挑战常缺乏具体解决方案。

Conclusion: 研究结果能为开发者和研究人员应对AI供应链的实际安全威胁提供基于证据的指导。
...
TL;DR: 本文探讨用AIOps原理增强Kubernetes自动伸缩，提出多信号自动伸缩框架，实验显示该方法可减少SLO违规、提升伸缩响应并降低成本，提高云平台可靠性、效率与可信任度。


<details>
  <summary>Details</summary>
Motivation: Kubernetes现有自动伸缩机制在生产环境常出现服务级别目标（SLO）违规和成本低效问题，原因是反应式伸缩、应用层信号利用有限和控制逻辑不透明。

Method: 对现有自动伸缩方法进行差距驱动分析，提出安全且可解释的多信号自动伸缩框架，集成SLO感知和成本意识控制以及轻量级需求预测。

Result: 使用代表性微服务和事件驱动工作负载的实验表明，与默认和调优的Kubernetes自动伸缩基线相比，该方法可将SLO违规持续时间减少31%，伸缩响应时间提高24%，基础设施成本降低18%，并保持稳定和可审核的控制行为。

Conclusion: AIOps驱动、以SLO为先的自动伸缩可显著提高基于Kubernetes的云平台的可靠性、效率和运营可信任度。
...
TL;DR: 论文分享十余年软件工程课程中教授软件质量方面的经验，并给出在敏捷/Scrum项目式学习中嵌入质量保证主题的建议。


<details>
  <summary>Details</summary>
Motivation: 总结在软件工程课程（聚焦敏捷/Scrum场景）中教授软件质量方面的经验，为教学提供参考。

Method: 回顾十余年教学经历，总结经验。

Result: 得到了在软件工程教学中的经验教训。

Conclusion: 给出在敏捷/Scrum项目式学习中嵌入质量保证主题的若干建议。
...
TL;DR: 本文介绍EnCoMSAS工具用于监测自适应软件系统能耗，通过实验验证其有效性及对整体能耗影响较小。


<details>
  <summary>Details</summary>
Motivation: 全球能耗增加，数据中心是主要贡献者，需降低软件应用能耗，但缺乏有效能耗监测工具。

Method: 引入EnCoMSAS工具，通过Adaptable TeaStore案例进行实证研究，改变工作负载条件，使用Grid5000测试平台。

Result: EnCoMSAS能有效收集能耗数据，CPU使用与能耗有关联，能耗受算法复杂度和部署环境影响，对整体能耗影响较小。

Conclusion: EnCoMSAS工具可用于收集软件应用能耗，支持运行时动态自适应，且对整体能耗影响不大。
...
TL;DR: 本文提出AdaptiFlow框架，利用自主计算原理实现微服务自适应，通过基准测试验证其在不同场景的实用性，还探讨了未来工作。
...
Method: 提出AdaptiFlow框架，包含指标收集器、自适应动作和轻量级事件驱动及基于规则的机制，通过增强的可适应茶店基准进行验证。

Result: 验证了三个自适应场景（自我修复、自我保护和自我优化），只需对每个服务进行最小的代码修改，证明了去中心化自适应可从局部决策产生。

Conclusion: 该工作将自主计算理论与云原生实践相结合，为构建弹性分布式系统提供概念框架和具体工具，未来将集成形式协调模型和应用AI代理进行主动适应。
...
TL;DR: 现有方法难检测大语言模型多步推理中复杂逻辑缺陷，本文提出MATP评估框架，经评估表现出色，能提升推理可信度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理存在逻辑错误，现有方法难以检测多步推理中的复杂逻辑缺陷，有必要提出新方法。

Method: 提出MATP评估框架，将自然语言推理转化为一阶逻辑，用自动定理证明器评估每一步逻辑有效性。

Result: 在包含10830个推理实例的基准测试中，MATP在推理步骤验证上比基于提示的基线方法高出超42个百分点，还揭示了模型级差异。
...
TL;DR: 本文提出利用基于模型开发（MBD）流程的自动驾驶软件并行化方法，可缩短执行时间，适合开发自动驾驶软件。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶软件需考虑多种功能和复杂环境，要求有实时性能。

Method: 提出一种基于MBD流程的并行化方法，扩展现有的基于模型并行化器（MBP）方法以实现复杂处理。

Result: 减少了执行时间。

Conclusion: 所提方法适用于自动驾驶软件开发，尤其在实现实时性能方面。
...
TL;DR: 本文提出MBD框架解决传统手动程序并行化和MBD集成现代框架的问题，达成ROS 2模型多输入并行代码生成，评估显示并行化有效减少执行时间。
...
Method: 提出MBD框架，将兼容ROS 2的Simulink模型分为事件驱动和定时器驱动类型进行针对性并行化。

Result: 扩展了MBD传统并行化，支持ROS 2多输入模型的并行代码生成，所有模式执行时间减少。

Conclusion: 所提框架的并行化方法有效。
...
TL;DR: 构建连续伊斯兰合规指数（CSCI），研究其在美股表现，发现与二元伊斯兰基准风险调整后表现相似，收紧门槛有负面影响，该框架实用。


<details>
  <summary>Details</summary>
Motivation: 二元伊斯兰筛选标准存在差异且有硬阈值，分类不连续，需更好的方法。

Method: 将标准筛选比率映射到保守和宽松界限间的平滑分数，结合行业活动因子构建CSCI，用CRSP/Compustat美股数据研究。

Result: 基于CSCI的多头组合历史风险调整后表现与二元伊斯兰基准相似，收紧最低合规门槛会减少可投资范围、降低分散性和夏普比率。

Conclusion: 该框架产生实用合规梯度，支持投资组合构建、约束设计和跨标准比较，无需依赖通过/失败筛选。
...
TL;DR: 本文重新审视Gerber的信息质量（IQ）框架，解决其在投资组合优化应用中的两个障碍，提出原子IQ参数化方法并开发分析特征值下限，回测显示其表现更优。


<details>
  <summary>Details</summary>
Motivation: 解决Gerber的IQ框架在投资组合优化中保证正半定性（PSD）和控制谱条件数的两个障碍。

Method: 引入挤压恒等式将IQ估计量表示为结构化通道矩阵的类凸组合，提出原子IQ参数化方法；开发分析特征值下限以调节条件数。

Result: 在考虑交易成本的仅多切点回测中，原子IQ提高了样本外夏普比率，风险状况更稳定。

Conclusion: 原子IQ方法在投资组合优化中优于一系列标准协方差估计量。
...
TL;DR: 股票收益分布的负偏态和正均值主要源于随机波动率在收益和损失上的不对称，用修正分布反映该不对称并应用于标普500日收益分布分析。


<details>
  <summary>Details</summary>
Motivation: 探究股票收益分布的负偏态和正均值的成因。

Method: 从股票收益和随机波动率的随机微分方程出发，将股票收益分布按收益和损失拆分，采用修正的Jones - Faddy偏态t分布反映不对称性。

Result: 修正的分布能有效反映股票收益分布的不对称性。

Conclusion: 股票收益分布的负偏态和正均值很大程度上是由随机波动率在收益和损失上的对称性被打破导致的，修正分布可有效捕捉这种不对称。
...
TL;DR: 加密货币永续期货回测易因忽略微观结构摩擦和重复使用评估窗口而失真，引入AutoQuant框架进行策略配置选择，研究表明考虑成本的回测与不考虑成本的差异大，且有过拟合风险。


<details>
  <summary>Details</summary>
Motivation: 解决加密货币永续期货回测中忽略微观结构摩擦和重复使用评估窗口导致回测结果不可靠的问题。

Method: 研究四种流动性好的永续合约，引入AutoQuant框架，采用贝叶斯优化，应用两阶段双筛选协议。

Result: 仅考虑费用和零成本回测会高估年化收益率，双筛选可减少回撤，CSCV/PBO诊断显示有过拟合风险。

Conclusion: AutoQuant可作为验证和治理基础设施，而不能保证持续的阿尔法收益。
...
TL;DR: 提出用于上下文感知alpha筛选的推理模型Alpha - R1，实证表明其优于基准策略且更抗衰减，代码开源。


<details>
  <summary>Details</summary>
Motivation: 非平稳市场中信号衰减和制度转变给数据驱动投资策略带来挑战，传统方法难泛化，大语言模型在定量因子筛选的经济推理潜力待挖掘，现有因子法忽视语义原理。

Method: 提出8B参数的推理模型Alpha - R1，通过强化学习训练，结合因子逻辑和实时新闻判断因子在不同市场条件下的相关性。

Result: 在多个资产池中的实证结果显示，Alpha - R1持续优于基准策略，且对alpha衰减有更好的鲁棒性。

Conclusion: Alpha - R1是解决非平稳市场投资策略问题的有效方案，可推广应用。
...
TL;DR: 本文强调五种流行模型的相似性，聚焦可识别性问题，证明部分模型解的唯一性与 NMF 等价，还回顾算法、用实例说明并讨论相关模型。


<details>
  <summary>Details</summary>
Motivation: 不同领域中相似的矩阵分解模型有不同名称，其相似性未被广泛认知，需进行研究梳理。

Method: 分析五种模型的可识别性，证明部分模型解的唯一性与 NMF 解的唯一性的等价关系。

Result: 证明 LBA、EMA、LCA、PLSA 的解唯一当且仅当 NMF 的解唯一。

Conclusion: 明确了五种模型的相似性及可识别性的关系，还对模型算法做了简要回顾，并讨论了相关模型。
...
TL;DR: 本文引入斐波那契集成框架用于集成学习，通过两种公式实现方差减少和增强表示深度，实验表明该框架在集成学习理论中有自然且可解释的优势。


<details>
  <summary>Details</summary>
Motivation: 受自然界斐波那契序列体现的平衡规律启发，希望为集成学习引入新的框架，补充和扩展经典聚合方案。

Method: 提出两种相互交织的公式，一是使用归一化斐波那契权重并通过正交化和Rao - Blackwell优化减少基学习器的方差；二是二阶递归集成动态，模仿斐波那契流增强表示深度。

Result: 通过一维回归实验，发现斐波那契加权在某些情况下可媲美或优于均匀平均，且与正交Rao - Blackwell化有规律的交互。

Conclusion: 斐波那契集成在集成学习的更广泛理论中是一个自然且可解释的设计点。
...
TL;DR: 本文提出超越经典方差减少理论的集成学习加权理论，解释低方差基学习器集成依然有效的原因


<details>
  <summary>Details</summary>
Motivation: 经典方差减少理论无法解释由稳定估计器构建的集成学习效果，因此需要新理论

Method: 将集成表示为作用于假设空间的线性算子，对加权序列空间施加几何和谱约束，推导细化的偏差 - 方差近似分解

Result: 给出结构化加权优于均匀集成的条件，最优权重是约束二次规划的解，经典方法是统一理论的特例

Conclusion: 为结构驱动的集成学习奠定理论基础，解释低方差基学习器中集成学习有效性并为后续工作做铺垫
...
TL;DR: 本文对Transformer注意力头中交叉熵训练如何重塑注意力分数和值向量进行一阶分析，得到路由定律和值更新规则，揭示其类似EM过程的特点，证明梯度动态能塑造贝叶斯推理所需的几何结构。


<details>
  <summary>Details</summary>
Motivation: 探究基于梯度的学习如何在Transformer中创造所需的内部几何结构。

Method: 对Transformer注意力头进行一阶分析，通过受控模拟对比不同更新方法。

Result: 得出基于优势的路由定律和基于责任的价值更新规则，发现其类似EM过程，且梯度动态能塑造贝叶斯推理所需的低维流形。

Conclusion: 优化（梯度流）产生几何结构（贝叶斯流形），进而支持上下文概率推理功能。
...
TL;DR: 本文提出似然保持嵌入理论，引入似然比失真度量，证明相关定理，给出神经网络构造框架，实验验证理论并展示应用。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习嵌入通常破坏经典基于似然的统计推断所需的几何结构，需要开发似然保持嵌入理论。

Method: 引入似然比失真度量$Δ_n$，证明铰链定理，给出用神经网络作为近似充分统计量的构造框架。

Result: 控制$Δ_n$对保持推断必要且充分；证明通用似然保持需基本可逆嵌入；实验验证指数族理论预测的相变，展示分布式临床推断应用。

Conclusion: 提出的似然保持嵌入理论有效，能在似然工作流程中用学习表示替代原始数据而不改变推断结论。
...
TL;DR: 提出JADAI框架联合优化贝叶斯自适应设计与推理，在标准基准测试中表现出色。
...
Method: 引入JADAI框架，端到端训练策略、历史网络和推理网络，最小化通用损失，推理网络采用基于扩散的后验估计器。

Result: 在标准自适应设计基准测试中，JADAI取得了优越或有竞争力的性能。

Conclusion: JADAI框架在参数估计的自适应设计问题上有良好效果。
...
TL;DR: 提出带L0约束的联邦学习方法，在多数据集实验中证明其通信高效且统计性能好。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中数据和模型固有稀疏性未处理的问题，避免模型过密和泛化性差。

Method: 对非零参数密度施加L0约束，通过概率门重参数化及连续松弛实现，基于联邦随机梯度下降提出分布式学习算法。

Result: 在多种模型和数据集上能实现目标参数密度，与基于幅度剪枝的阈值算法相比，通信高效且统计性能更好。

Conclusion: 所提带L0约束的联邦学习方法有效，具有良好的通信效率和统计性能。
...
TL;DR: 本文表明可在概率建模和推理范围内回答因果推断问题，无需特定因果工具或符号，并重新解读因果工具。


<details>
  <summary>Details</summary>
Motivation: 机器学习中因果推断研究工具使用存在混淆，常见认为需定制因果框架或符号来回答因果问题。

Method: 通过具体例子说明将所有事物的概率写下来可以解决因果问题。

Result: 展示了在概率建模和推理范畴内可回答因果推断问题。

Conclusion: 重新解读因果工具，阐明其必要性和实用性。
...
TL;DR: 研究非平稳环境下股票收益预测的机器学习模型，提出新模型选择方法，在实证中表现优于基准。


<details>
  <summary>Details</summary>
Motivation: 解决非平稳环境下股票收益预测中模型复杂度与非平稳性之间的权衡问题。

Method: 采用锦标赛程序的新模型选择方法，联合优化模型类和训练窗口大小。

Result: 应用于17个行业投资组合收益，平均提高样本外$R^2$ 14 - 23%，衰退期表现更佳，交易策略累计收益高31%。

Conclusion: 该方法能平衡误设误差、估计方差和非平稳性，接近事后最佳模型。
...
TL;DR: 提出在线方法MultiQT保证多水平分位数预测校准，实验显示提升了预测校准效果。


<details>
  <summary>Details</summary>
Motivation: 解决多水平分位数预测同时保证校准的问题，应对分布漂移。

Method: 引入Multi - Level Quantile Tracker (MultiQT)方法，包装现有预测器生成校正预测，保证校准和预测有序，并有无遗憾保证。

Result: MultiQT显著提升了疫情和能源预测问题中真实预测器的校准效果。

Conclusion: MultiQT是一种有效的多水平分位数预测校准方法，不会渐近恶化现有预测器性能。
...
TL;DR: 提出Iterated Bellman Calibration用于校准无限期马尔可夫决策过程的离策略价值预测，有有限样本保证。


<details>
  <summary>Details</summary>
Motivation: 校准无限期马尔可夫决策过程的离策略价值预测。

Method: 将经典直方图和保序校准方法应用于动态反事实场景，用双重稳健伪结果处理离策略数据，反复将拟合的贝尔曼目标回归到模型预测上。

Result: 得到一维拟合值迭代方案，可应用于任何价值估计器。

Conclusion: 在弱假设下为校准和预测提供有限样本保证，且无需贝尔曼完备性或可实现性。
...
TL;DR: 介绍屏蔽朗之万蒙特卡罗（LMC）采样器，可从穿孔支撑上的未归一化目标分布采样，实验显示其有优势


<details>
  <summary>Details</summary>
Motivation: 解决从非凸空间（凸集带凸孔）的未归一化目标分布采样这一挑战性问题

Method: 结合空间自适应温度和排斥漂移，确保样本在可行区域内

Result: 在二维高斯混合和多输入多输出（MIMO）符号检测实验中，屏蔽LMC相比无约束情况展示出优势

Conclusion: 屏蔽LMC是一种有效的从穿孔支撑上的未归一化目标分布采样的方法
...
### [309] [Uncertainty-Aware Flow Field Reconstruction Using SVGP Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.22426)
*Y. Sungtaek Ju*

Main category: physics.flu-dyn

TL;DR: 提出SVGP - KAN机器学习框架用于不确定度感知的流场重建，与经典方法对比，结果显示该方法有良好性能和实用指导意义。


<details>
  <summary>Details</summary>
Motivation: 从时间稀疏测速测量中重建时间分辨流场对表征复杂热流体系统至关重要，需一种能进行不确定度量化的流场重建方法。

Method: 引入基于Kolmogorov - Arnold网络拓扑的稀疏变分高斯过程（SVGP - KAN）机器学习框架，与经典重建方法和卡尔曼滤波进行系统比较，用脉冲冲击射流的合成数据评估，采用多种评估指标。

Result: SVGP - KAN方法达到与现有方法相当的重建精度，同时提供校准良好的不确定度估计，可靠指示预测退化的时间和位置。

Conclusion: 该框架是一种稳健的数据驱动流场重建方法，能进行有意义的不确定度量化，为周期性流动实验设计提供实用指导。
...
### [310] [From geometry to dynamics: Learning overdamped Langevin dynamics from sparse observations with geometric constraints](https://arxiv.org/abs/2512.23566)
*Dimitra Maoutsa*

Main category: math.DS

TL;DR: 提出新框架解决随机系统稀疏采样轨迹下动力学规律学习问题，在欠采样数据上表现优于现有方法，证明几何归纳偏置有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在随机系统轨迹稀疏采样时存在局限，要么需高频观测，要么仅适用于保守系统。

Method: 将推理重新表述为随机控制问题，使用几何驱动的路径增强，利用系统不变密度的几何信息重建可能轨迹并推断动力学，不假设特定参数模型。
...
### [311] [Synthesis of signal processing algorithms with constraints on minimal parallelism and memory space](https://arxiv.org/abs/2512.22676)
*Sergey Salishev*

Main category: eess.SP

TL;DR: 本文在最小并行性和内存空间约束下开发信号处理算法与实现方案以提高低功耗计算硬件能效，提出多个方法并给出结果。
...
Method: 提出支持选择最优并行性的时钟CMOS逻辑功耗/能耗模型；用整数友好的近似方法减少查找表大小；给出混合基数流式FFT在多存储体和单端口内存上无冲突的数据放置和执行顺序；对用于超快速Toeplitz系统求解的快速Schur算法进行并行性/内存分析。

Result: 提供建设性定理、调度方案和设计权衡，可实现高效的专用加速器。

Conclusion: 所提方法和结果有助于在约束条件下开发高效的信号处理算法和实现方案，提升低功耗计算硬件能效。
...
### [312] [UniFi: Combining Irregularly Sampled CSI from Diverse Communication Packets and Frequency Bands for Wi-Fi Sensing](https://arxiv.org/abs/2512.22143)
*Gaofeng Dong,Kang Yang,Mani Srivastava*

Main category: eess.SP

TL;DR: 提出首个基于Wi-Fi的ISAC框架UniFi，消除了侵入性数据包注入，评估显示其精度高、模型小且不影响通信吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有Wi-Fi传感系统依赖注入高速率探测包影响通信且可部署性差，现有ISAC解决方案仍依赖辅助包注入。

Method: 直接利用多频段通信包的不规则采样CSI，集成CSI清理管道，结合时间感知注意力模型，引入CommCSI - HAR数据集。

Result: 在数据集和四个公共基准测试中，UniFi以紧凑模型大小达到了先进精度，同时完全保留通信吞吐量。

Conclusion: UniFi是一种有效且实用的基于Wi-Fi的ISAC解决方案。
...
### [313] [EEG-to-Voice Decoding of Spoken and Imagined speech Using Non-Invasive EEG](https://arxiv.org/abs/2512.22146)
*Hanbeot Park,Yunjeong Cho,Hunhee Kim*

Main category: eess.SP

TL;DR: 提出无需动态时间规整或显式时间对齐的EEG-to-Voice范式，用于从无创EEG信号重建语音，评估效果良好，证明其可行性。


<details>
  <summary>Details</summary>
Motivation: 脑机接口研究旨在从神经信号恢复语音通信，但基于EEG的语音重建因空间分辨率有限、易受噪声影响和想象语音中缺乏时间对齐的声学目标而具有挑战性。

Method: 提出EEG-to-Voice范式，用特定主体生成器从EEG生成梅尔频谱图，经预训练声码器和自动语音识别模块合成语音波形和解码文本，对发声和想象语音分别训练生成器，应用基于迁移学习的领域自适应，可选应用基于最小语言模型的校正模块。

Result: 在发声和想象语音中都观察到稳定的声学重建和相当的语言准确性，长话语声学相似度降低但文本解码性能基本保留，语言模型校正能降低错误率且不引入语义失真。

Conclusion: 证明了无需显式时间对齐的直接、开环EEG-to-Voice重建发声和想象语音的可行性。
...
### [314] [Machine Learning-Based Basil Yield Prediction in IoT-Enabled Indoor Vertical Hydroponic Farms](https://arxiv.org/abs/2512.22151)
...
TL;DR: 探讨室内垂直水培结合机器学习技术优化罗勒产量与节水，对比三种机器学习模型性能，DNN模型兼顾计算速度与预测质量，适用于实际部署。


<details>
  <summary>Details</summary>
Motivation: 农业面临水资源短缺压力，需创新高效节水方案优化作物产量。

Method: 开发预测系统，用物联网传感器收集二氧化碳、光照等环境参数数据，训练和测试线性回归（LR）、长短期记忆网络（LSTM）和深度神经网络（DNN）三种机器学习模型。

Result: LSTM预测能力强、准确率达99%，但执行时间长、内存使用高；DNN准确率98%，在计算速度和预测质量间取得平衡；LR处理基础预测快，执行时间11秒。

Conclusion: 基于DNN的解决方案适合构建农业环境决策支持系统，适合未来边缘设备部署。
...
### [315] [PaperNet: Efficient Temporal Convolutions and Channel Residual Attention for EEG Epilepsy Detection](https://arxiv.org/abs/2512.22172)
*Md Shahriar Sajid,Abhijit Kumar Ghosh,Fariha Nusrat*

Main category: eess.SP

TL;DR: 本文提出PaperNet紧凑混合架构用于EEG短窗口分类，在测试集达0.96 macro - F1，参数约0.6M，证明无需高计算成本可实现强EEG分类性能。


<details>
  <summary>Details</summary>
Motivation: EEG信号含丰富时谱结构但难建模，现有轻量级深度学习模型有局限，如仅用局部卷积或需重型循环模块。

Method: 提出结合时间卷积、通道残差注意力模块和轻量级双向循环块的PaperNet架构，使用公开BEED数据集，在明确的独立于受试者的训练协议下进行评估，并与既有轻量级基线模型对比。

Result: 模型在测试集上macro - F1达0.96，约0.6M参数，各类别性能均衡；消融研究证明各组件贡献；通道注意力权重显示电极相关性；计算分析表明可在资源受限系统高效部署。

Conclusion: 精心结合时间滤波、通道重新加权和循环上下文建模，可在不过度增加计算成本的情况下实现出色的EEG分类性能。
...
### [316] [Simultaneous Approximation of the Score Function and Its Derivatives by Deep Neural Networks](https://arxiv.org/abs/2512.23643)
*Konstantin Yakovlev,Nikita Puchkin*

Main category: math.NA

TL;DR: 提出同时近似得分函数及其导数的理论，放宽有界支持要求，误差界无维数灾难且能处理任意阶导数。


<details>
  <summary>Details</summary>
Motivation: 处理具有低维结构和无界支持的数据分布，同时放宽通常有界支持的要求。

Method: 提出同时近似得分函数及其导数的理论。

Result: 近似误差界与文献匹配，无维数灾难，能给出任意阶导数的近似保证。

Conclusion: 该理论在处理特定数据分布上有优势，且在近似导数方面有更广泛的适用性。
...
### [317] [Differentiable Inverse Modeling with Physics-Constrained Latent Diffusion for Heterogeneous Subsurface Parameter Fields](https://arxiv.org/abs/2512.22421)
*Zihan Lin,QiZhi He*

Main category: math.NA

TL;DR: 提出基于潜在扩散的可微反演方法LD - DIM用于PDE约束反问题，在多孔介质流动反问题中验证其效果优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决涉及高维空间分布系数的PDE约束反问题，处理稀疏观测下的数值条件和梯度优化稳定性问题。

Method: 将预训练潜在扩散先验与端到端可微数值求解器结合，在潜在空间直接进行反演，通过伴随梯度和反向自动微分传播灵敏度。

Result: 在多孔介质流动反问题中，LD - DIM比PINNs和VAE基线方法在数值稳定性、重建精度上表现更好，能保持尖锐不连续性并降低对初始化的敏感性。

Conclusion: LD - DIM是解决PDE约束反问题的有效方法，具有良好性能。
...
### [318] [Mining the Gold: Student-AI Chat Logs as Rich Sources for Automated Knowledge Gap Detection](https://arxiv.org/abs/2512.22404)
*Quanzhi Fu,Qiyu Wu,Dan Williams*

Main category: cs.HC

TL;DR: 随着计算机相关专业招生增加，大课堂教学面临识别学生知识漏洞的挑战。本文提出QueryQuilt框架，通过分析学生与AI聊天记录检测知识漏洞，评估结果良好，计划在真实课堂部署。


<details>
  <summary>Details</summary>
Motivation: 过去20年计算机相关专业招生增加，大课堂授课中教师难以及时识别学生知识漏洞，现有课堂反馈系统有局限性，而学生与大语言模型的对话可作为识别知识漏洞的数据源。

Method: 提出QueryQuilt多智能体大语言模型框架，包含对话智能体和知识漏洞识别智能体，通过分析学生与AI助手的聊天记录来检测知识漏洞，并生成漏洞频率分布。

Result: QueryQuilt在模拟学生中识别知识漏洞准确率达100%，在真实学生 - AI对话数据中完整性达95%。

Conclusion: 初步结果表明该系统在真实学习环境中有辅助教学的潜力，计划在实际课堂部署进行全面评估。
...
### [319] [Reimagining the Traditional Flight Computer: E6BJA as a Modern, Multi-Platform Tool for Flight Calculations and Training](https://arxiv.org/abs/2512.23055)
*Jamie J. Alnasir*

Main category: cs.HC

TL;DR: 本文介绍多平台飞行计算机E6BJA，对比传统工具展示优势，探讨设计权衡，称其是飞行员飞行工具的有意义演进。


<details>
  <summary>Details</summary>
Motivation: 传统飞行计算机形式固定、交互受限、扩展性不足，与现代数字环境不匹配，需开发新工具。

Method: 开发多平台软件E6BJA，在功能、认知和技术维度与传统飞行计算机对比。

Result: E6BJA在准确性、减少错误、可发现性和教育价值方面有改进。

Conclusion: E6BJA结合传统方法严谨性与现代人机交互设计，是飞行员飞行工具的有意义演进，支持航空训练计算与教学。
...
### [320] [SPECTRE: Spectral Pre-training Embeddings with Cylindrical Temporal Rotary Position Encoding for Fine-Grained sEMG-Based Movement Decoding](https://arxiv.org/abs/2512.22481)
*Zihan Weng,Chanlin Yi,Pouya Bashivan,Jing Lu,Fali Li,Dezhong Yao,Jingming Hou,Yangsong Zhang,Peng Xu*

Main category: cs.HC

TL;DR: 介绍特定领域自监督学习框架SPECTRE用于非侵入式表面肌电信号细粒度运动解码，评估显示其表现优异。


<details>
  <summary>Details</summary>
Motivation: 非侵入式表面肌电信号解码因信号非平稳性和低信噪比存在挑战，通用自监督学习框架效果不佳。

Method: 提出SPECTRE框架，包括基于生理学的预训练任务和新型位置编码，预训练为对聚类的短时傅里叶变换表示进行掩码离散伪标签预测，位置编码采用CyRoPE对嵌入进行线性时间和环形空间维度分解。

Result: 在多个数据集上评估表明SPECTRE实现运动解码新的最优，显著优于监督基线和通用自监督方法，消融实验验证预训练和CyRoPE的关键作用。

Conclusion: SPECTRE为处理现实世界sEMG复杂性的实用肌电接口提供坚实基础。
...
### [321] [Emotion classification using EEG headset signals and Random Forest](https://arxiv.org/abs/2512.22333)
*Ricardo Vasquez,Diego Riofrío-Luzcando,Joe Carrion-Jumbo,Cesar Guevara*

Main category: cs.HC

TL;DR: 本文提出基于EEG信号对人的情绪进行分类的模型，用随机森林模型预测情绪，准确率较高，并生成实时情绪预测算法。


<details>
  <summary>Details</summary>
Motivation: 情绪是人类重要组成部分，用计算系统检测、识别和理解情绪可改善人机交流。

Method: 使用脑机接口EMOTIV EPOC收集50人脑电图信息，存入数据库，创建随机森林模型进行情绪预测。

Result: 幸福情绪预测准确率97.21%，放松和悲伤情绪预测准确率76%。

Conclusion: 成功创建基于EEG信号的情绪分类模型，并生成实时情绪预测算法。
...
### [322] [It's a TRAP! Task-Redirecting Agent Persuasion Benchmark for Web Agents](https://arxiv.org/abs/2512.23128)
...
TL;DR: 介绍TRAP评估，发现大语言模型驱动的Web智能体易受提示注入攻击，不同模型受影响程度不同，还提供框架用于扩展评估。


<details>
  <summary>Details</summary>
Motivation: 大语言模型驱动的Web智能体依赖动态网页内容，易受提示注入攻击。

Method: 引入Task - Redirecting Agent Persuasion Benchmark (TRAP)评估，对六种前沿模型测试，并进行高保真网站克隆的可控实验。

Result: 六种模型平均25%的任务易受提示注入攻击，GPT - 5为13%，DeepSeek - R1为43%，小的界面或上下文变化常使成功率翻倍。

Conclusion: 基于网页的智能体存在系统性、受心理因素驱动的漏洞，提供的模块化框架可进一步扩展评估。
...
### [323] [Clinically Calibrated Machine Learning Benchmarks for Large-Scale Multi-Disorder EEG Classification](https://arxiv.org/abs/2512.22656)
*Argha Kamal Samanta,Deepak Mewada,Monalisa Sarma,Debasis Samanta*

Main category: cs.HC

TL;DR: 研究自动化脑电图多疾病分类，处理数据后训练模型，评估显示多数疾病类别召回率超80%，为多疾病脑电图分类建立性能基线。


<details>
  <summary>Details</summary>
Motivation: 临床脑电图解读手动、耗时且专家间差异大，现有自动化方法多针对孤立诊断问题，对多疾病临床筛查支持有限。

Method: 使用标准纵向双极导联处理脑电图记录，通过多域特征集表示，在严重类别不平衡下训练疾病感知机器学习模型，并校准决策阈值以优先考虑诊断敏感性。

Result: 敏感性导向建模使多数疾病类别召回率超80%，部分低流行率疾病校准后召回率提升15 - 30%，特征重要性分析显示符合临床标记的生理模式。

Conclusion: 为多疾病脑电图分类建立现实性能基线，证明优先考虑敏感性的自动化分析可支持现实临床中的脑电图筛查和分诊。
...
### [324] [On the Choice of Model Space Priors and Multiplicity Control in Bayesian Variable Selection: An Application to Streaming Logistic Regression](https://arxiv.org/abs/2512.22504)
*Joyee Ghosh*

Main category: stat.ME

TL;DR: 本文研究逻辑回归中贝叶斯变量选择（BVS）不同模型空间先验的实际影响，尤其在流式数据场景。对比不同先验，提出简单近似方法，模拟表明无单一先验在各场景占优，新提出的MD先验是中间选择。


<details>
  <summary>Details</summary>
Motivation: BVS对模型空间先验的设定很敏感，本文旨在研究逻辑回归中不同模型空间先验在流式数据场景下对BVS的实际影响。

Method: 回顾流行的Beta - Binomial先验和新提出的MD先验；引入MD先验的简单近似方法；使用基于BIC的近似边际似然，通过模拟研究比较不同先验对后验包含概率和系数估计的影响。

Result: 没有单一模型空间先验在所有场景中占优，新提出的MD先验在不同稀疏程度的常用Beta - Binomial先验之间占据中间位置。

Conclusion: MD先验为BVS在逻辑回归的流式数据场景中提供了一个有用的额外选择。
...
### [325] [Intrinsic Whittle--Matérn fields and sparse spatial extremes](https://arxiv.org/abs/2512.23395)
*David Bolin,Peter Braunsteins,Sebastian Engelke,Raphaël Huser*

Main category: stat.ME

TL;DR: 引入新的固有Whittle - Matérn高斯随机场类，开发快速估计和模拟方法，应用于不同领域。


<details>
  <summary>Details</summary>
Motivation: 现有固有模型数量和灵活性有限，相关理论、快速推理和软件发展不足。

Method: 引入由随机偏微分方程（SPDE）解得到的固有Whittle - Matérn高斯随机场类，利用有限元近似的稀疏性开发快速方法。

Result: 展示了固有SPDE方法在外部推设置下克里金法的优势，将理论转化为用于空间极端事件稀疏建模的SPDE方法。
...
### [326] [Deep classifier kriging for probabilistic spatial prediction of air quality index](https://arxiv.org/abs/2512.23474)
*Junyu Chen,Pratik Nag,Huixia Judy-Wang,Ying Sun*

Main category: stat.ME

TL;DR: 提出深度分类器克里金（DCK）方法用于空气质量指数（AQI）空间预测，经实验验证其优于传统方法，并应用于AQI概率空间预测。


<details>
  <summary>Details</summary>
Motivation: 经典空间预测方法如克里金是线性且依赖高斯假设，难以捕捉AQI场的非高斯和复杂非线性空间结构，无法提供可靠预测分布。

Method: 提出深度分类器克里金（DCK）框架用于估计单变量和双变量空间过程的全预测分布函数，结合数据融合机制处理非共位双变量过程和整合异质空气污染数据源。

Result: 通过大量模拟实验表明，DCK在预测准确性和不确定性量化方面始终优于传统方法。

Conclusion: DCK可通过融合不同数据源进行AQI概率空间预测，得到的空间解析预测分布能支持监管风险评估和政策制定等下游任务。
...
### [327] [Beyond-Diagonal Reconfigurable Intelligent Surfaces for 6G Networks: Principles, Challenges, and Quantum Horizons](https://arxiv.org/abs/2512.23400)
*Abd Ullah Khan,Uman Khalid,Muhammad Tanveer,Trung Q. Duong,Hyundong Shin*

Main category: cs.SI

TL;DR: 本文系统介绍BD - RIS，阐述其原理、优势与分类，介绍进展、挑战与机遇，通过四种算法设计波束赋形并分析性能，还分析混合量子 - 经典机器学习模型提升波束预测性能以获取实际应用见解。


<details>
  <summary>Details</summary>
Motivation: BD - RIS虽有创新但实现其优势存在诸多挑战，特别是特定环境下的被动波束赋形设计是研究重点，因此开展相关研究。

Method: 系统介绍BD - RIS，通过四种算法进行波束赋形设计并分析性能，利用DeepSense 6G数据集中的真实通信场景8分析混合量子 - 经典机器学习模型。

Result: 分析了四种算法在和速率与计算成本方面的性能，对混合量子 - 经典机器学习模型提升波束预测性能进行了研究。

Conclusion: 获得了关于BD - RIS实际应用的有用见解。
...
### [328] [Rethinking Leveraging Pre-Trained Multi-Layer Representations for Speaker Verification](https://arxiv.org/abs/2512.22148)
*Jin Sob Kim,Hyun Joon Park,Wooseok Shin,Sung Won Han*

Main category: cs.SD

TL;DR: 提出层注意力池化（LAP）用于说话人验证特征聚合，结合ASTP构建轻量级后端模型，在VoxCeleb基准测试中表现优异且训练时间大幅减少。


<details>
  <summary>Details</summary>
Motivation: 现有研究在聚合预训练Transformer模型多层特征时多采用静态加权平均，缺乏更优策略。

Method: 提出LAP动态评估各层重要性并采用最大池化，结合LAP和ASTP构建轻量级后端说话人模型。

Result: 在VoxCeleb基准测试中，紧凑架构达到了最先进性能，大幅减少训练时间。

Conclusion: LAP及其动态加权机制能有效捕捉说话人特征，所提架构具有良好性能和效率。
...
### [329] [Chord Recognition with Deep Learning](https://arxiv.org/abs/2512.22621)
*Pierre Mackenzie*

Main category: cs.SD

TL;DR: 对现有自动和弦识别方法实验分析，发现和弦分类器对稀有和弦表现差，音高增强可提高准确率，生成模型特征无用，合成数据有前景，最后改进输出可解释性取得较好结果。


<details>
  <summary>Details</summary>
Motivation: 探究深度学习在自动和弦识别领域应用后进展缓慢的原因。

Method: 对现有方法进行实验，利用生成模型发展进行假设检验，用节拍检测改进模型输出可解释性。

Result: 和弦分类器对稀有和弦表现差，音高增强可提高准确率，生成模型特征无助于提升，合成数据是未来方向，取得该领域较好结果。

Conclusion: 自动和弦识别仍有大量工作要做，希望本文能为他人提供路径。
...
### [330] [Mobile-Efficient Speech Emotion Recognition Using DistilHuBERT: A Cross-Corpus Validation Study](https://arxiv.org/abs/2512.23435)
*Saifelden M. Ismail*

Main category: cs.SD

TL;DR: 本文提出基于DistilHuBERT的移动高效语音情感识别系统，经多数据集验证，实现模型大小与准确率的权衡，可用于资源受限移动设备。


<details>
  <summary>Details</summary>
Motivation: 现有语音情感识别中，先进的transformer架构计算需求高，限制其在移动应用的部署，需开发移动高效的系统。

Method: 采用DistilHuBERT，进行8位量化；在IEMOCAP数据集上进行5折留一会话交叉验证；在CREMA - D上进行跨语料库训练。

Result: 模型参数减少92%，跨语料库训练加权准确率提升1.2%、Macro F1分数提升1.4%、跨折方差降低32%；量化模型大小23MB，未加权准确率61.4%；RAVDESS上准确率43.29%，但对愤怒和悲伤的唤醒检测表现良好。

Conclusion: 该方法实现了模型大小和准确率的帕累托最优权衡，可用于资源受限的移动设备。
...
### [331] [PHANTOM: Physics-Aware Adversarial Attacks against Federated Learning-Coordinated EV Charging Management System](https://arxiv.org/abs/2512.22381)
*Mohammad Zakaria Haider,Amit Kumar Podder,Prabin Mali,Aranya Chakrabortty,Sumit Paudyal,Mohammad Ashiqur Rahman*

Main category: cs.ET

TL;DR: 提出PHANTOM并构建多智能体RL环境研究虚假数据注入策略及电网后果，强调物理感知网络安全的关键


<details>
  <summary>Details</summary>
Motivation: 电动汽车充电站在配电网中快速部署，需智能自适应控制维护电网弹性和可靠性，研究相关网络攻击影响

Method: 提出PHANTOM，将联邦学习的物理信息神经网络作为数字孪生模型；构建多智能体RL环境，用DQN和SAC方法推导攻击策略；开发输配电双仿真平台

Result: 学习到的攻击策略会破坏负荷平衡，引发跨输配电边界的电压不稳定

Conclusion: 强调物理感知网络安全对大规模车网融合的弹性至关重要
...
### [332] [LogosQ: A High-Performance and Type-Safe Quantum Computing Library in Rust](https://arxiv.org/abs/2512.23183)
*Shiwen An,Jiayi Wang,Konstantinos Slavakis*

Main category: quant-ph

TL;DR: 介绍了用Rust实现的量子计算库LogosQ，能通过编译时类型安全保证正确性，有多种优化技术，性能超现有框架，还验证了数值稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有基于Python的量子软件框架有动态特性，存在运行时错误和可扩展性瓶颈，需要开发更强大高性能的量子软件。

Method: 用Rust实现LogosQ，利用Rust静态分析消除运行时错误，引入直接状态向量操作、自适应并行处理和FFT优化的量子傅里叶变换等优化技术。

Result: 在状态制备（QFT）上加速达900倍，变分工作负载上比Python框架快2 - 5倍，比Julia实现快6 - 22倍，与Q sharp性能相当，在VQE实验中验证了数值稳定性。

Conclusion: LogosQ结合系统编程安全性和高级电路优化，为可靠高效的量子模拟树立了新标准。
...
### [333] [Revisiting finite Abelian hidden subgroup problem and its distributed exact quantum algorithm](https://arxiv.org/abs/2512.22959)
*Ziyuan Dong,Xiang Fan,Tengxun Zhong,Daowen Qiu*

Main category: quant-ph

TL;DR: 从数学角度重新审视有限阿贝尔隐藏子群问题（AHSP），提出精确量子算法、分布式精确量子算法和并行精确经典算法。


<details>
  <summary>Details</summary>
Motivation: 重新审视有限阿贝尔隐藏子群问题，改进已有算法。

Method: 采用振幅放大提出精确量子算法；利用中国剩余定理提出分布式精确量子算法；开发并行精确经典算法。

Result: 精确量子算法更简洁且适用于任何有限阿贝尔群；分布式精确量子算法所需量子位更少、查询复杂度更低且无需量子通信，还可扩展到某些非阿贝尔群；并行精确经典算法降低查询复杂度。

Conclusion: 所提出的算法在有限阿贝尔隐藏子群问题上有更好的性能和适用性。
...
### [334] [Deep Learning for Art Market Valuation](https://arxiv.org/abs/2512.23078)
*Jianping Mei,Michael Moses,Jan Waelty,Yucheng Yang*

Main category: q-fin.GN

TL;DR: 研究深度学习结合艺术品视觉内容改进艺术市场估值，发现多模态深度学习对首次销售估值有显著价值。


<details>
  <summary>Details</summary>
Motivation: 探索深度学习如何通过纳入艺术品视觉内容改进艺术市场估值。

Method: 使用大型重复销售数据集，对比经典享乐回归和基于树的方法与现代深度架构，包括融合表格和图像数据的多模态模型。

Result: 艺术家身份和过往交易历史主导整体预测能力，视觉嵌入对无历史参考的新入市作品有独特且经济上有意义的贡献，模型关注构图和风格线索。

Conclusion: 多模态深度学习在估值最难的首次销售时能带来显著价值，为艺术市场估值的学术研究和实践提供新见解。
...
### [335] [An Inference-Based Architecture for Intent and Affordance Saturation in Decision-Making](https://arxiv.org/abs/2512.23144)
*Wendyam Eric Lionel Ilboudo,Saori C Tanaka*

Main category: q-bio.NC

TL;DR: 本文针对决策瘫痪问题，提出基于分层决策过程收敛失败的计算解释，并用模拟重现决策惯性和关闭的关键特征。


<details>
  <summary>Details</summary>
Motivation: 决策瘫痪对假设选项已明确且可比的选择模型提出挑战，需要新的解释。

Method: 将意图选择和可供性选择分离，将承诺形式化为反向和正向 Kullback - Leibler (KL) 目标混合下的推断，在静态和动态模型中进行研究。

Result: 正向 KL 偏差推断产生缓慢、重尾的响应时间和两种失败模式，模拟重现决策惯性和关闭的关键特征。

Conclusion: 将自闭症视为基于推理的一般决策连续体的极端情况。
...
### [336] [Nonlinear Dynamical Modeling of Human Intracranial Brain Activity with Flexible Inference](https://arxiv.org/abs/2512.22785)
*Kiarash Vaziri,Lucine L. Oganesian,HyeongChan Jo,Roberto M. C. Vera,Charles Y. Liu,Brian Lee,Maryam M. Shanechi*

Main category: q-bio.NC

TL;DR: 本文将DFINE框架拓展到多站点人类颅内脑电图记录建模，发现其在预测未来神经活动上显著优于线性状态空间模型，对缺失观测处理更稳健，突出其作为建模人类iEEG动力学的强大灵活框架的地位。


<details>
  <summary>Details</summary>
Motivation: 线性动态模型无法捕捉神经活动的非线性结构，循环神经网络模型处理缺失观测存在不足，且现有DFINE是针对皮层内记录，因此需要将其拓展到多站点人类颅内脑电图记录建模。

Method: 将DFINE拓展到多站点人类颅内脑电图（iEEG）记录的建模。

Result: DFINE在预测未来神经活动上显著优于线性状态空间模型，在神经预测中准确度与门控循环单元模型相当或更优，对缺失观测处理比基线模型更稳健，在高伽马光谱波段相比线性状态空间模型优势更明显。

Conclusion: DFINE是用于建模人类iEEG动力学的强大且灵活的框架，在下一代脑机接口中有潜在应用。
...
### [337] [Geometry-Aware Optimization for Respiratory Sound Classification: Enhancing Sensitivity with SAM-Optimized Audio Spectrogram Transformers](https://arxiv.org/abs/2512.22564)
*Atakan Işık,Selin Vulga Işık,Ahmet Feridun Işık,Mahşuk Taylan*

Main category: eess.AS

TL;DR: 针对呼吸音分类数据集的问题，引入用Sharpness - Aware Minimization增强的Audio Spectrogram Transformer框架及加权采样策略，在ICBHI 2017数据集上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 基准数据集存在规模有限、噪音高和类别不平衡问题，Transformer模型在这些受限医疗数据上易过拟合并收敛到尖锐最小值。

Method: 使用Sharpness - Aware Minimization增强Audio Spectrogram Transformer，优化损失面几何结构；实施加权采样策略处理类别不平衡。

Result: 在ICBHI 2017数据集上达到68.10%的先进分数，灵敏度达68.31%，通过t - SNE和注意力图分析表明模型学习到鲁棒、有区分性的特征。

Conclusion: 所提方法有效解决了现有问题，可用于可靠的临床筛查。
...
### [338] [Axiomatic Foundations of Bayesian Persuasion](https://arxiv.org/abs/2512.23409)
*Youichiro Higashi,Kemal Ozbek,Norio Takeoka*

Main category: econ.TH

TL;DR: 研究贝叶斯说服的公理基础，提出新模型及仅用可观测菜单选择数据的引出方法。


<details>
  <summary>Details</summary>
Motivation: 研究贝叶斯说服的公理基础，探讨委托人在告知有偏差的代理人不确定状态后如何决策。

Method: 构建新颖的贝叶斯说服模型，提供仅使用委托人可观测菜单选择数据的引出方法。

Result: 提出新的贝叶斯说服模型，展示了即使委托人预期管理代理人偏差时，如何构建其获取信息的主观成本。

Conclusion: 通过新模型和引出方法，为贝叶斯说服的研究提供了新的视角和方法。
...
### [339] [Literature Mining System for Nutraceutical Biosynthesis: From AI Framework to Biological Insight](https://arxiv.org/abs/2512.22225)
*Xinyang Sun,Nipon Sarmah,Miao Guo*

Main category: q-bio.QM

TL;DR: 本文提出基于大语言模型的领域自适应系统，自动从非结构化科学文本中识别营养保健品生产微生物，生成相关数据集，为生产提供见解。


<details>
  <summary>Details</summary>
Motivation: 解决营养保健品研究中从科学文献提取结构化知识的瓶颈，特别是识别化合物生物合成中涉及的微生物菌株。

Method: 采用大语言模型构建领域自适应系统，利用少样本提示和定制查询设计。

Result: 生成包含35个营养保健品 - 菌株关联的结构化验证数据集，发现不同培养系统中微生物多样性，确定主要贡献菌株。

Conclusion: 该AI驱动框架提高文献挖掘的可扩展性和可解释性，为高价值营养保健品生产提供行动见解。
...
### [340] [INSIGHT: Spatially resolved survival modelling from routine histology crosslinked with molecular profiling reveals prognostic epithelial-immune axes in stage II/III colorectal cancer](https://arxiv.org/abs/2512.22262)
...
<details>
  <summary>Details</summary>
Motivation: 常规组织学包含结直肠癌丰富预后信息，需从复杂空间组织结构中挖掘

Method: 开发图神经网络INSIGHT，在TCGA和SURGEN上训练和交叉验证，并进行独立验证，结合多种组学数据

Result: INSIGHT预测性能优于pTNM分期，其空间风险图揭示相关风险关联，整合数据后发现风险流形

Conclusion: INSIGHT有效挖掘组织学预后信息，揭示患者特异性异质性和治疗靶点
...
### [341] [Adaptive Fusion Graph Network for 3D Strain Field Prediction in Solid Rocket Motor Grains](https://arxiv.org/abs/2512.23443)
*Jiada Huang,Hao Ma,Zhibin Shen,Yizhou Qiao,Haiyang Li*

Main category: physics.app-ph

TL;DR: 提出自适应图网络GrainGNet评估发动机结构安全，联合预测任务中效果好，训练效率高。


<details>
  <summary>Details</summary>
Motivation: 传统数值模拟计算成本高，现有代理模型无法显式建立几何模型并准确捕捉高应变区域，而局部高应变是固体火箭发动机药柱结构失效主因。

Method: 提出自适应图网络GrainGNet，采用自适应池化动态节点选择机制保留关键机械特征，利用特征融合传递深度特征。

Result: 在联合预测任务中，与基线图U - Net模型相比，均方误差降低62.8%，参数仅增加5.2%，训练效率提高约7倍；在脱粘缝高应变区域，预测误差较次优方法进一步降低33%。

Conclusion: GrainGNet为评估发动机结构安全提供了计算高效且高保真度的方法。
...
### [342] [Reach-Avoid Differential game with Reachability Analysis for UAVs: A decomposition approach](https://arxiv.org/abs/2512.22793)
*Minh Bui,Simon Monckton,Mo Chen*

Main category: eess.SY

TL;DR: 提出降维框架解三维RA游戏，通过HJ可达性分析和追踪控制算法解决问题，仿真和实验验证方法有效性。


<details>
  <summary>Details</summary>
Motivation: 传统HJ可达性分析难用于三维RA游戏，其他方法缺乏考虑三维和非平凡系统动力学的通用性，故寻求解决方法。

Method: 将问题分解为水平和垂直RA子游戏，用HJ可达性分析求解，考虑二阶动力学，引入基于HJ的追踪控制算法。

Result: 数值仿真表明分解保持原问题最优性和保证，在Gazebo物理模拟器中首次在三维空间成功捕获四旋翼。

Conclusion: 所提方法对解决三维空间下RA游戏问题有效。
...
### [343] [NashOpt -- A Python Library for Computing Generalized Nash Equilibria](https://arxiv.org/abs/2512.23636)
*Alberto Bemporad*

Main category: eess.SY

TL;DR: NashOpt是用于计算和设计非合作博弈广义纳什均衡的开源Python库，介绍其方法、适用问题和示例且给出代码地址。


<details>
  <summary>Details</summary>
Motivation: 开发能计算和设计具有共享约束与实值决策变量的非合作博弈广义纳什均衡的工具。

Method: 利用参与者的联合KKT条件处理博弈，非线性通过非线性最小二乘法结合JAX自动微分求解，线性二次型转换为混合整数线性规划；支持逆博弈和Stackelberg博弈设计问题。

Result: 通过线性二次调节和模型预测控制等非合作博弈论控制问题示例展示能力。

Conclusion: NashOpt可解决相关非合作博弈问题，代码可在指定链接获取。
...
### [344] [A Neural Network-Based Real-time Casing Collar Recognition System for Downhole Instruments](https://arxiv.org/abs/2512.22901)
...
TL;DR: 提出基于嵌入式神经网络的井下套管接箍实时识别系统，紧凑模型计算复杂度低且F1分数高，硬件验证推理延迟短。


<details>
  <summary>Details</summary>
Motivation: 传统地面套管接箍定位器信号易退化，需准确的井下定位方法。

Method: 引入为资源受限的ARM Cortex - M7微处理器优化的轻量级“Collar Recognition Nets”（CRNs），利用时间和深度可分离卷积。

Result: 最紧凑模型计算复杂度降至8,208 MACs，F1分数达0.972，硬件验证平均推理延迟343.2 μs。
...
### [345] [Tree Meets Transformer: A Hybrid Architecture for Scalable Power Allocation in Cell-Free Networks](https://arxiv.org/abs/2512.22639)
*Irched Chafaa,Giacomo Bacci,Luca Sanguinetti*

Main category: eess.SY

TL;DR: 提出混合Tree - Transformer架构实现可扩展的用户级功率分配，在无蜂窝大规模MIMO系统中评估，性能接近最优且减少推理时间。


<details>
  <summary>Details</summary>
Motivation: 无线通信网络功率分配有挑战，基于Transformer的模型计算成本随用户数量增加而增加。

Method: 提出混合Tree - Transformer架构，通过二叉树压缩用户特征到全局根表示，对根应用Transformer编码器，通过共享解码器解码用户上下行功率。

Result: 在无蜂窝大规模MIMO系统的最大最小公平性问题上评估，性能接近最优，显著减少推理时间。

Conclusion: 该混合Tree - Transformer架构能实现高效的用户级功率分配，无需重新训练或更改架构。
...
### [346] [Revealing design archetypes and flexibility in e-molecule import pathways using Modeling to Generate Alternatives and interpretable machine learning](https://arxiv.org/abs/2512.23284)
*Mahdi Kchaou,Francesco Contino,Diederik Coppitters*

Main category: eess.SY

TL;DR: 传统研究仅提供单一成本最优解，但在现实中其可行性差，本文用建模生成替代方案结合机器学习处理不确定性，应用于氢进口路径选择，结果显示近最优解可行且灵活。


<details>
  <summary>Details</summary>
Motivation: 传统研究得到的成本最优解在现实中受多种因素影响难以实施，存在局限性。

Method: 使用“建模生成替代方案”方法生成可接受成本范围内的近成本最优方案集，再应用可解释机器学习从解决方案空间中提取见解。

Result: 发现广阔且灵活的近最优解空间，在成本最优10%范围内，太阳能、风能和储能并非必需；风能受限利于太阳能 - 储能甲醇路径，储能有限则利于风能制氨或甲烷路径。

Conclusion: 该方法能有效应对能源进口路径选择中的不确定性，提供更具现实可行性的方案。
...
### [347] [Lower bounds on pure dynamic programming for connectivity problems on graphs of bounded path-width](https://arxiv.org/abs/2512.23121)
...
### [348] [Coloring Hardness on Low Twin-Width Graphs](https://arxiv.org/abs/2512.23680)
*Édouard Bonnet*
...
### [349] [ReVEAL: GNN-Guided Reverse Engineering for Formal Verification of Optimized Multipliers](https://arxiv.org/abs/2512.22260)
*Chen Chen,Daniela Kaufmann,Chenhui Deng,Zhan Song,Hongce Zhang,Cunxi Yu*

Main category: cs.LO

TL;DR: 提出基于图学习的ReVEAL方法用于逆向工程乘法器架构以改进代数电路验证技术，在多个乘法器基准测试中展示了优势。


<details>
  <summary>Details</summary>
Motivation: 改进代数电路验证技术。

Method: 利用结构图形特征和学习驱动的推理来大规模识别架构模式，提出ReVEAL方法。

Result: 在多个乘法器基准测试中展示出比传统基于规则方法更好的可扩展性和准确性，可与现有验证流程集成并支持下游代数证明策略。

Conclusion: 该基于图学习的方法能有效逆向工程乘法器架构，提升代数电路验证效果。
...
### [350] [Persistent Homology via Finite Topological Spaces](https://arxiv.org/abs/2512.23348)
*Selçuk Kayacan*

Main category: math.AT

TL;DR: 提出基于有限拓扑空间及其关联偏序集的持久同调函子框架，可获持久模块，证明相关简化保不变性及图稳定性。


<details>
  <summary>Details</summary>
Motivation: 构建一个有效的持久同调框架。

Method: 从有限度量空间关联有限拓扑过滤，通过交叉切割构造传递到偏序集和单纯复形。

Result: 得到了无需复形间包含关系的持久模块，证明标准偏序集简化保留持久不变量，以及基于密度实例中输入度量扰动下的稳定性。

Conclusion: 所提出的函子框架在持久同调方面是有效的且具有稳定性。
...
### [351] [Choice Modeling and Pricing for Scheduled Services](https://arxiv.org/abs/2512.22271)
*Adam N. Elmachtoub,Kumar Goutam,Roger Lederman*

Main category: econ.GN

TL;DR: 提出用于离散选择建模和价格优化的新框架，适用于提供预定服务选项的业务，在亚马逊某业务中实验效果好且已投入生产。


<details>
  <summary>Details</summary>
Motivation: 在提供预定服务选项的业务场景中，不同选项成本和需求差异大，需不同价格，现有系统待优化。

Method: 用决策树分割市场，在各细分市场用参数化离散选择模型估计转化行为，并提供价格优化启发式方法。

Result: 在亚马逊某业务的A/B实验中，新框架在关键指标上超越现有定价系统，目标性能指标提升19%。

Conclusion: 新框架效果良好，自2023年Q4起已在该业务全面投入生产。
...
### [352] [Sorting of Working Parents into Family-Friendly Firms](https://arxiv.org/abs/2512.22810)
*Ross Chu,Sohee Jeon,Hyun Seung Lee,Tammy Lee*

Main category: econ.GN

TL;DR: 利用韩国数据，研究父母产后如何进入家庭友好型企业，发现主要通过劳动力留存而非跳槽，母亲因雇主缺乏福利更可能退出劳动力市场。


<details>
  <summary>Details</summary>
Motivation: 研究父母产后如何进入家庭友好型企业。

Method: 分析韩国工作福利详细数据，研究离职和跳槽模式，进行两个准实验案例研究，并用简单的排序模型解释结果。

Result: 引入家庭友好型变更吸引更多受益父母入职，有幼儿的父母即便薪资增长慢也留下，排序主要通过劳动力留存而非跳槽，母亲产后多不主动换工作，雇主无福利时更易退出劳动力市场。

Conclusion: 母亲集中在家庭友好型企业是因为雇主缺乏福利时退出劳动力市场，而非产后跳槽。
...
### [353] [Salary Matching and Pay Cut Reduction for Job Seekers with Loss Aversion](https://arxiv.org/abs/2512.22818)
*Ross Chu*

Main category: econ.GN

TL;DR: 研究损失厌恶对雇主提供和求职者接受工资的影响，提出行为搜索模型，用韩国行政数据验证预测，发现考虑损失厌恶能改善模型拟合，影响补贴传导，薪资历史禁令效果有限。
...
Method: 引入行为搜索模型，结合韩国行政数据进行实证检验，用最小距离法估计模型参数。

Result: 零工资增长处超额质量是无损失厌恶时的8.5倍；零上方密度比下方大8.8%；零下方密度斜率是上方的6.5倍；考虑损失厌恶改善模型拟合，补贴传导降低18%。

Conclusion: 损失厌恶对工资决定和补贴传导有显著影响，薪资历史禁令不能有效缓解相关影响。
...
### [354] [Assortative Mating, Inequality, and Rising Educational Mobility in Spain](https://arxiv.org/abs/2512.22848)
...
Method: 使用分样本技术研究流动性的区域相关性。

Result: 近几十年流动性上升三分之一，择偶匹配度能解释近一半区域代际相关性差异，是不平等与流动性负相关关系的关键中介。

Conclusion: 代际教育流动性与择偶匹配密切相关，择偶匹配是不平等和流动性关系的重要影响因素。
...
### [355] [Equilibrium Transition from Loss-Leader Competition: How Advertising Restrictions Facilitate Price Coordination in Chilean Pharmaceutical Retail](https://arxiv.org/abs/2512.22917)
*Yu Hao*

Main category: econ.GN

TL;DR: 本文利用智利药店数据研究禁止价格比较广告的影响，发现该禁令消除需求溢出效应，使定价转向协调高价，表明监管可能无意促进价格协调。


<details>
  <summary>Details</summary>
Motivation: 研究监管如何推动寡头垄断从一种定价模式转向另一种。

Method: 使用智利药店连锁店数据研究禁止比较价格广告，构建结构需求模型并进行反事实分析。

Result: 禁令消除需求溢出效应，使价格转向协调更高水平，降低了价格弹性和交叉产品溢出效应，溢出效应丧失而非弹性降低导致新的协调定价。

Conclusion: 善意的监管可能会因削弱支持竞争结果的机制，无意地促进价格协调。
...
### [356] [R&D Networks under Heterogeneous Firm Productivities](https://arxiv.org/abs/2512.23337)
*M. Sadra Heydari,Zafer Kanik,Santiago Montoya-Blandón*

Main category: econ.GN

TL;DR: 本文将异质性研发生产率引入内生研发网络形成模型，发现生产率差距影响网络稳定性和福利，高平均研发生产率经济体福利可能更低，政策应考虑对研发联盟和努力的影响。


<details>
  <summary>Details</summary>
Motivation: 推广Goyal和Moraga - Gonzalez (2001)的框架，研究异质性研发生产率对研发网络形成和福利的影响。

Method: 构建引入异质性研发生产率的内生研发网络形成模型，并进行模拟分析。

Result: 生产率差距大时，完全网络不稳定，正分类网络稳定；正分类网络福利更高，但福利与高生产率企业比例呈倒U型关系。

Conclusion: 高平均研发生产率经济体可能因网络结构形成或挤出效应福利更低，生产率提升政策应考虑对内生研发联盟和努力的影响。
...
### [357] [A Political Economy Definition of the Middle Class](https://arxiv.org/abs/2512.23523)
*Alejandro Corvalan*

Main category: econ.GN

TL;DR: 本文提出基于收入分配特性定义中产阶级，理论论证其存在并实证刻画世界中产阶级。


<details>
  <summary>Details</summary>
Motivation: 经济学家基于收入分配定义中产阶级时选取‘中间’部分存在随意性，需新定义方法。

Method: 先进行理论论证存在对不平等变化不敏感的中产阶级群体，再用世界收入数据库（WID）的百分位数据进行实证分析。

Result: 世界中产阶级偏向收入分配的上半部分，包含很多富人群体但非巨富，其边界在不同时间和国家较稳定。

Conclusion: 该定义与政治经济学中中产阶级作为缓和力量的主流观点相符，因其无参与分配冲突的动机。
...
### [358] [The Big Three in Marriage Talk: LLM-Assisted Analysis of Moral Ethics and Sentiment on Weibo and Xiaohongshu](https://arxiv.org/abs/2512.23609)
*Frank Tian-Fang Ye,Xiaozi Gao*

Main category: econ.GN

TL;DR: 研究分析中国两大社交媒体平台上219,358条婚姻相关帖子，发现微博话语偏积极，小红书偏中性，多数帖子无明确道德框架，不同道德伦理与情感有显著关联，揭示当代中国负面婚姻态度成因并证明大语言模型在定性分析中的作用。


<details>
  <summary>Details</summary>
Motivation: 中国婚姻登记数量大幅下降，需研究公众对婚姻的态度，不仅关注情感倾向，还需考察背后的道德推理。

Method: 使用大语言模型辅助的内容分析法，分析来自新浪微博和小红书的219,358条婚姻相关帖子，并依据Shweder的三大道德伦理框架对帖子进行情感和道德维度编码。

Result: 微博话语偏积极，小红书偏中性；多数帖子无明确道德框架；涉及自主伦理和社群伦理的帖子多为负面，涉及神性伦理的帖子倾向于中性或积极情感。

Conclusion: 个人自主受限和社群义务方面的担忧导致当代中国负面婚姻态度，大语言模型可用于扩大定性分析规模，为制定应对中国婚姻下降的文化相关政策提供见解。
...
### [359] [Scalable Residual Feature Aggregation Framework with Hybrid Metaheuristic Optimization for Robust Early Pancreatic Neoplasm Detection in Multimodal CT Imaging](https://arxiv.org/abs/2512.23597)
*Janani Annur Thiruvengadam,Kiran Mayee Nabigaru,Anusha Kovi*

Main category: cs.CV

TL;DR: 提出SRFA框架用于胰腺癌早期检测，实验显示性能显著提升，有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 胰腺癌早期检测因CT图像肿瘤边界不明显、患者解剖差异大而困难，需有效可扩展系统辅助。

Method: 提出SRFA框架，包括预处理、MAGRes - UNet分割、DenseNet - 121特征提取、HHO - BA特征选择、ViT与EfficientNet - B3集成的混合模型分类及SSA和GWO双优化机制调参。

Result: 所提模型准确率达96.23%，F1 - score达95.58%，特异性达94.83%，优于传统CNN和基于Transformer的模型。

Conclusion: SRFA框架在胰腺癌早期检测中有潜在实用价值。
...
### [360] [An Architecture-Led Hybrid Report on Body Language Detection Project](https://arxiv.org/abs/2512.23028)
*Thomson Tong,Diba Darooneh*

Main category: cs.CV

TL;DR: 对两个视觉语言模型进行架构分析，介绍其在视频到工件管道中的应用及运行机制，指出模型行为与系统约束的联系。


<details>
  <summary>Details</summary>
Motivation: 通过架构分析明确两个视觉语言模型在实际视频处理管道中的特性和应用，为后续开发和评估提供基础。

Method: 先总结模型共享的多模态基础，再描述各模型架构，最后将模型行为与系统约束关联。

Result: 明确了系统流程，发现结构化输出可能存在语义错误、模式验证不考虑几何正确性等问题。

Conclusion: 这些模型行为与系统约束的区分对撰写合理主张、设计稳健接口和规划评估至关重要。
...
### [361] [Characterizing Motion Encoding in Video Diffusion Timesteps](https://arxiv.org/abs/2512.22175)
*Vatsal Baherwani,Yixuan Ren,Abhinav Shrivastava*

Main category: cs.CV

TL;DR: 本文通过大规模定量研究，刻画视频扩散模型时间步中运动编码，确定运动主导和外观主导阶段，简化运动定制范式。


<details>
  <summary>Details</summary>
Motivation: 当前对文本到视频扩散模型中运动如何跨时间步编码理解不足，且未系统刻画早期时间步塑造运动布局、后期细化外观这一经验启发式行为。

Method: 通过在指定时间步范围内注入新条件时外观编辑和运动保留之间的权衡来代理运动编码，并进行大规模定量研究。

Result: 在不同架构中均确定了早期运动主导阶段和后期外观主导阶段，得到时间步空间中的运动 - 外观边界；简化了单步运动定制范式，无需辅助去偏模块或专门目标即可实现强运动迁移。

Conclusion: 将广泛使用的启发式方法转化为时空解纠缠原则，时间步约束方法可集成到现有运动迁移和编辑方法中。
...
### [362] [Enhancing Medical Data Analysis through AI-Enhanced Locally Linear Embedding: Applications in Medical Point Location and Imagery](https://arxiv.org/abs/2512.22182)
*Hassan Khalid,Muhammad Mahad Khaliq,Muhammad Jawad Bashir*

Main category: cs.CV

TL;DR: 本文介绍将AI与LLE集成处理高维医疗数据，应用于医疗计费和转录，实验显示可提升准确性和效率，为未来研究奠基。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域快速发展，为改进医疗计费和转录等流程，需处理高维医疗数据，减少人为错误，提高效率。

Method: 将AI与局部线性嵌入（LLE）集成，构建AI增强的LLE模型，并提供综合数学模型。

Result: 实验表明该模型显著提高了数据处理的准确性和运营效率。

Conclusion: AI增强的LLE在医疗数据分析中有潜力，为更广泛医疗应用研究奠定基础。
...
### [363] [Unbiased Visual Reasoning with Controlled Visual Inputs](https://arxiv.org/abs/2512.22183)
...
TL;DR: 提出VISTA框架，解耦感知与推理，用强化学习训练视觉推理，提升对虚假关联的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型回答视觉问题时易利用虚假关联，微调时更易走捷径。

Method: 引入VISTA框架，通过信息瓶颈解耦感知与推理，用强化学习训练。

Result: 在SpuriVerse上显著提升鲁棒性，在MMVP和SeedBench子集有竞争力，能跨VLM传感器迁移，识别并恢复感知失败。

Conclusion: VISTA推理痕迹更中立，更少依赖虚假属性，更基于视觉证据。
...
### [364] [HookMIL: Revisiting Context Modeling in Multiple Instance Learning for Computational Pathology](https://arxiv.org/abs/2512.22188)
*Xitong Ling,Minxi Ouyang,Xiaoxiao Li,Jiawen Li,Ying Chen,Yuxuan Sun,Xinrui Chen,Tian Guan,Xiaoping Liu,Yonghong He*

Main category: cs.CV

TL;DR: 提出HookMIL框架解决传统MIL方法问题，在多数据集达SOTA


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法丢失关键上下文信息，基于Transformer的变体有复杂度高和计算冗余问题

Method: 提出HookMIL框架，使用可学习的hook tokens进行结构化上下文聚合，支持多模态初始化，训练时通过双向注意力交互，引入Hook Diversity Loss，设置hook-to-hook通信机制

Result: 在四个公共病理数据集上取得了最先进的性能，提高了计算效率和可解释性

Conclusion: HookMIL框架有效解决传统MIL方法局限，有良好效果
...
### [365] [TCFormer: A 5M-Parameter Transformer with Density-Guided Aggregation for Weakly-Supervised Crowd Counting](https://arxiv.org/abs/2512.22203)
*Qiang Guo,Rubo Zhang,Bingbing Zhang,Junjie Liu,Jianqing Liu*

Main category: cs.CV

TL;DR: 提出TCFormer人群计数框架，轻量级且效果好，适合边缘设备。


<details>
  <summary>Details</summary>
Motivation: 传统人群计数依赖人力标注和高计算量骨干网络，限制可扩展性和资源受限部署，需要解决方案。

Method: 采用强大高效的视觉变换器作特征提取器；设计可学习密度加权平均模块聚合特征；引入密度级别分类损失，正则化训练过程。

Result: 在四个基准数据集上实验表明，该方法在参数效率和计数准确性上取得了良好平衡。

Conclusion: TCFormer虽在弱监督下训练，但能实现高估计精度，可作为边缘设备人群计数任务的好方案。
...
### [366] [Signal-SGN++: Topology-Enhanced Time-Frequency Spiking Graph Network for Skeleton-Based Action Recognition](https://arxiv.org/abs/2512.22214)
*Naichuan Zheng,Xiahai Lun,Weiyi Li,Yuchen Du*

Main category: cs.CV

TL;DR: 本文提出Signal - SGN++拓扑感知脉冲图框架，结合结构适应性与时间频率脉冲动态，在大规模基准测试中验证其实现了精度与效率的良好权衡。


<details>
  <summary>Details</summary>
Motivation: GCNs用于动作识别时浮点计算能耗高，SNNs在捕捉人体运动的时频和拓扑依赖方面有局限，需弥补二者差距。

Method: 提出Signal - SGN++框架，采用1D - SGC和FSC组成的骨干网络进行特征提取，嵌入TSSA机制，设置MWTF辅助分支和TATF单元。

Result: 在大规模基准测试中，Signal - SGN++实现了优越的精度 - 效率权衡，优于现有基于SNN的方法，与最先进的GCNs相比在大幅降低能耗下取得有竞争力的结果。

Conclusion: Signal - SGN++框架有效解决了GCNs能耗高和SNNs捕捉依赖能力有限的问题，实现了良好的精度与效率平衡。
...
### [367] [VLM-PAR: A Vision Language Model for Pedestrian Attribute Recognition](https://arxiv.org/abs/2512.22217)
*Abdellah Zakaria Sellam,Salah Eddine Bekhouche,Fadi Dornaika,Cosimo Distante,Abdenour Hadid*

Main category: cs.CV

TL;DR: 提出视觉语言框架VLM - PAR处理行人属性识别问题，在多个基准测试中取得佳绩，展现结合预训练和模态细化的有效性。


<details>
  <summary>Details</summary>
Motivation: 行人属性识别面临严重的类别不平衡、属性复杂的相互依赖和领域转移问题。

Method: 引入基于冻结SigLIP 2多语言编码器的模块化视觉语言框架VLM - PAR，利用紧凑的交叉注意力融合细化视觉特征以对齐图像和提示嵌入。

Result: 在高度不平衡的PA100K基准上显著提高准确性，在PETA和Market - 1501基准上平均准确率也大幅提升。

Conclusion: 将大规模视觉语言预训练与有针对性的跨模态细化相结合，能有效克服行人属性识别中的类别不平衡和泛化挑战。
...
### [368] [On Extending Semantic Abstraction for Efficient Search of Hidden Objects](https://arxiv.org/abs/2512.22220)
*Tasha Pais,Nikhilesh Belulkar*

Main category: cs.CV

TL;DR: 利用语义抽象框架处理隐藏物体3D定位与补全，模型定位快，有望助家用机器人找物。


<details>
  <summary>Details</summary>
Motivation: 为家用机器人提供寻找丢失物品节省时间和精力的技能，解决隐藏物体定位问题。

Method: 将相关性图作为“抽象对象”表示，利用历史数据提高隐藏物体定位效率。

Result: 模型能首次准确识别隐藏物体完整3D位置，比随机搜索快。

Conclusion: 语义抽象的扩展有望使家用机器人在找物时更高效。
...
### [369] [VideoScaffold: Elastic-Scale Visual Hierarchies for Streaming Video Understanding in MLLMs](https://arxiv.org/abs/2512.22226)
*Naishan Zheng,Jie Huang,Qingpei Guo,Feng Zhao*

Main category: cs.CV

TL;DR: 提出用于流式视频理解的动态表示框架VideoScaffold，在离线和流式视频理解基准测试中取得SOTA，并可扩展现有基于图像的MLLMs。


<details>
  <summary>Details</summary>
Motivation: 现有静态策略在处理长视频时存在输出碎片化或过度压缩问题，难以满足流式视频理解需求。

Method: 提出包含Elastic - Scale Event Segmentation（EES）和Hierarchical Event Consolidation（HEC）的VideoScaffold框架，EES动态细化事件边界，HEC将相关片段聚合为多级抽象。

Result: 在离线和流式视频理解基准测试中取得了最优性能，框架具有模块化和即插即用特点。

Conclusion: VideoScaffold能有效解决流式长视频理解问题，可扩展现有图像MLLMs用于连续视频理解，代码公开。
...
### [370] [Meta-information Guided Cross-domain Synergistic Diffusion Model for Low-dose PET Reconstruction](https://arxiv.org/abs/2512.22237)
*Mengxiao Geng,Ran Hong,Xiaoling Xu,Bingxuan Li,Qiegen Liu*

Main category: cs.CV

TL;DR: 本文提出MiG - DM模型用于低剂量PET成像，在多个数据集上验证其能提升图像质量并保留生理细节。


<details>
  <summary>Details</summary>
Motivation: 低剂量PET成像面临噪声干扰等挑战，现有方法忽视投影域物理知识和患者特定元信息，需要挖掘功能 - 语义相关性。

Method: 引入元信息引导的跨域协同扩散模型MiG - DM，包含元信息编码模块和跨域架构，结合投影域和图像域处理。

Result: 在UDPET公共数据集和不同剂量水平的临床数据集实验表明，MiG - DM在提升PET图像质量和保留生理细节方面优于现有方法。

Conclusion: MiG - DM模型能有效提升PET图像质量并保留生理细节，在低剂量PET成像中有良好表现。
...
### [371] [Multi-objective hybrid knowledge distillation for efficient deep learning in smart agriculture](https://arxiv.org/abs/2512.22239)
*Phi-Hung Hoang,Nam-Thuan Trinh,Van-Manh Tran,Thi-Thu-Hong Phan*

Main category: cs.CV

TL;DR: 本文提出混合知识蒸馏框架开发轻量级高性能卷积神经网络，在水稻种子和植物叶片病害数据集上验证其高效性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决在资源受限的边缘设备上部署深度学习模型时计算效率和识别精度的权衡问题。

Method: 设计结合倒置残差块和密集连接的定制学生模型，在ResNet18教师网络指导下，采用集成硬标签监督、特征级蒸馏、响应级蒸馏和自蒸馏的多目标策略进行训练。

Result: 在水稻种子分类任务中，蒸馏后的学生模型准确率达98.56%，计算成本降低约2.7倍，模型大小减少超10倍；与代表性预训练模型相比，参数大幅减少且精度相当或更优。在多个植物叶片病害数据集上表现一致良好。

Conclusion: 所提框架对硬件受限的智慧农业系统具有鲁棒性、高效性和强大的部署潜力。
...
### [372] [The Illusion of Clinical Reasoning: A Benchmark Reveals the Pervasive Gap in Vision-Language Models for Clinical Competency](https://arxiv.org/abs/2512.22275)
*Dingyu Wang,Zimu Yuan,Jiajun Liu,Shanggui Liu,Nan Zhou,Tianxing Xu,Di Huang,Dong Jiang*

Main category: cs.CV

TL;DR: 文章开发B&J基准评估AI模型临床推理能力，发现模型在多模态推理任务上表现欠佳，目前不适用于复杂临床任务。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估基础模型在临床实践中的真实推理能力，需要新方法。

Method: 开发B&J基准，涵盖1245个来自真实病例的问题，评估11个视觉语言模型和6个大语言模型。

Result: 模型在不同任务上表现差距大，多模态任务准确率低，视觉语言模型有图像解释和幻觉问题，微调模型无明显优势。

Conclusion: 当前AI模型不具备复杂多模态推理临床能力，应限于文本辅助角色，需突破多模态集成和视觉理解技术。
...
### [373] [A Three-Level Alignment Framework for Large-Scale 3D Retrieval and Controlled 4D Generation](https://arxiv.org/abs/2512.22294)
*Philip Xu,David Elizondo,Raouf Hamzaoui*

Main category: cs.CV

TL;DR: 介绍基于多模态结构对齐的Uni4D框架，实现大规模开放词汇3D检索与可控4D生成，实验效果良好。


<details>
  <summary>Details</summary>
Motivation: 实现大规模开放词汇的3D检索和可控4D生成，推动动态多模态理解和实际应用。

Method: 基于Align3D 130数据集，使用3D文本多头注意力和搜索模型优化文本到3D检索，通过三个组件加强跨模态对齐。

Result: Uni4D实现了高质量3D检索和可控4D生成。

Conclusion: Uni4D框架在动态多模态理解和实际应用方面取得进展。
...
### [374] [Attack-Aware Deepfake Detection under Counter-Forensic Manipulations](https://arxiv.org/abs/2512.22303)
*Noor Fatima,Hasan Faraz Khan,Muzammil Behzad*

Main category: cs.CV

TL;DR: 本文提出攻击感知的深度伪造和图像取证检测器，结合红队训练与随机测试防御，在多基准测试中效果良好，建立实用检测基线。
...
Method: 采用两流架构结合红队训练与随机测试防御，一流编码语义内容，一流提取取证残差，通过轻量级适配器融合分类，浅特征金字塔网络生成篡改热图，红队训练采用多种反取证操作，测试防御注入低成本抖动，用面部掩码引导热图。

Result: 在现有基准测试中，排名接近完美，校准误差低，弃权风险小，重噪下性能可控下降。

Conclusion: 建立了模块化、数据高效且可实际部署的攻击感知检测基线。
...
### [375] [VideoZoomer: Reinforcement-Learned Temporal Focusing for Long Video Reasoning](https://arxiv.org/abs/2512.22315)
*Yang Ding,Yizhen Zhang,Xin Lai,Ruihang Chu,Yujiu Yang*

Main category: cs.CV

TL;DR: 提出VideoZoomer框架让MLLMs在长视频理解中动态控制视觉焦点，经两阶段训练，7B模型表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs因上下文窗口受限，在长视频理解中存在不足，主流方法易忽略关键证据且推理中难纠错。

Method: 提出VideoZoomer框架，从低帧率 overview 开始，用时间缩放工具获取高帧率片段；采用两阶段训练策略，包括监督微调与强化学习。

Result: 7B模型能呈现多样复杂推理模式，在长视频理解和推理基准测试中表现良好，超越开源模型，在挑战性任务中与专有系统竞争，且低帧预算下效率更高。

Conclusion: VideoZoomer框架能让MLLMs在长视频理解中动态控制视觉焦点，具备良好性能和效率。
...
### [376] [SpotEdit: Selective Region Editing in Diffusion Transformers](https://arxiv.org/abs/2512.22323)
*Zhibin Qin,Zhenxiong Tan,Zeqing Wang,Songhua Liu,Xinchao Wang*

Main category: cs.CV

TL;DR: 提出无训练的扩散编辑框架SpotEdit，选择性更新修改区域，实现高效精确图像编辑。


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时对所有区域统一处理，存在冗余计算且可能破坏未修改区域，需解决此问题。

Method: 提出SpotEdit框架，包含通过感知相似性识别稳定区域并复用特征的SpotSelector，以及通过动态融合机制混合特征与编辑标记的SpotFusion。

Result: 减少不必要的计算，在未修改区域保持高保真度。

Conclusion: SpotEdit能实现高效且精确的图像编辑。
...
### [377] [The Multi-View Paradigm Shift in MRI Radiomics: Predicting MGMT Methylation in Glioblastoma](https://arxiv.org/abs/2512.22331)
*Mariya Miteva,Maria Nisheva-Pavlova*

Main category: cs.CV

TL;DR: 提出基于VAE的多视图潜在表征学习框架，整合MRI影像特征进行MGMT启动子甲基化分类。


<details>
  <summary>Details</summary>
Motivation: 无创推断肿瘤分子特征是放射基因组学目标，传统方法有特征冗余和模态信息建模不完整问题。

Method: 引入基于变分自编码器（VAE）的多视图潜在表征学习框架，对不同模态分别编码并在潜在空间融合。
...
### [378] [Feature Learning with Multi-Stage Vision Transformers on Inter-Modality HER2 Status Scoring and Tumor Classification on Whole Slides](https://arxiv.org/abs/2512.22335)
*Olaide N. Oyelade,Oliver Hoxey,Yulia Humrye*

Main category: cs.CV

TL;DR: 本文提出基于视觉Transformer的端到端流程用于HER2评分，实验显示该方法在肿瘤定位和HER2状态预测上有良好准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在HER2评分时难以提供像素级定位，联合分析H&E和IHC图像进行HER2评分较难。

Method: 提出单端到端流程，包括对H&E全切片图像进行逐块处理、提出映射函数、嵌入HER2评分机制。

Result: 肿瘤定位分类准确率良好，HER2状态预测分类准确率0.94，特异度0.933。

Conclusion: 基于端到端ViTs模型联合评估H&E和IHC图像用于HER2评分具有可用性。
...
### [379] [Human-like visual computing advances explainability and few-shot learning in deep neural networks for complex physiological data](https://arxiv.org/abs/2512.22349)
*Alaa Alahmadi,Mohamed Hasan*

Main category: cs.CV

TL;DR: 本文提出感知启发的伪着色技术可提升分析生理数据的深度神经网络的可解释性和少样本学习能力，以药物性长QT综合征为例验证，证明类人感知编码能解决医疗机器智能的数据效率、可解释性和因果推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器视觉模型分析生理信号时数据效率和可解释性不足，限制临床可靠性和与人类推理的契合度。

Method: 采用感知启发的伪着色技术，将临床显著的时间特征编码为结构化颜色表示，用原型网络和ResNet - 18架构对单心动周期和10秒完整心律的心电图图像进行少样本学习评估。

Result: 模型能从一到五个训练样本中学习到有区分性和可解释性的特征，伪着色引导注意力关注临床有意义的心电图特征，聚合多个心动周期可提升性能。

Conclusion: 类人感知编码能在医疗机器智能中实现数据效率、可解释性和因果推理的平衡。
...
### [380] [VULCAN: Tool-Augmented Multi Agents for Iterative 3D Object Arrangement](https://arxiv.org/abs/2512.22351)
*Zhengfei Kuang,Rui Lin,Long Zhao,Gordon Wetzstein,Saining Xie,Sanghyun Woo*

Main category: cs.CV

TL;DR: 本文针对MLLMs在3D场景操作应用不足的问题，提出解决3D对象排列任务的三个关键方法，在25个任务中效果优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在2D视觉语言任务取得进展，但在复杂3D场景操作应用未充分探索，需解决3D对象排列任务的关键挑战。

Method: 1. 引入基于MCP的API，解决MLLMs视觉定位弱的问题；2. 用专业视觉工具增强MLLM的3D场景理解；3. 提出协作多智能体框架处理迭代易出错的更新。

Result: 在25个复杂对象排列任务中，显著优于现有基线。

Conclusion: 提出的方法有效解决了MLLMs在3D对象排列任务中的关键挑战。
...
### [381] [Self-Evaluation Unlocks Any-Step Text-to-Image Generation](https://arxiv.org/abs/2512.22374)
*Xin Yu,Xiaojuan Qi,Zhengqi Li,Kai Zhang,Richard Zhang,Zhe Lin,Eli Shechtman,Tianyu Wang,Yotam Nitzan*

Main category: cs.CV

TL;DR: 介绍了支持任意步推理的文本到图像生成从零训练方法Self-E，实验显示其在少步和多步生成上均有良好表现。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖局部监督或预训练教师，本文旨在提出一种新的文本到图像生成方法，能从零训练且支持任意步推理。

Method: 提出Self-E模型，学习数据类似Flow Matching模型，同时采用自我评估机制，结合即时局部学习和自我驱动全局匹配。

Result: 在大规模基准测试中，Self-E在少步生成中表现出色，50步时与最先进的Flow Matching模型有竞争力，性能随推理步数增加而提升。

Conclusion: Self-E是首个从零开始、支持任意步的文本到图像模型，提供了高效可扩展的统一框架。
...
### [382] [Bright 4B: Scaling Hyperspherical Learning for Segmentation in 3D Brightfield Microscopy](https://arxiv.org/abs/2512.22423)
*Amil Khan,Matheus Palhares Viana,Suraj Mishra,B. S. Manjunath*

Main category: cs.CV

TL;DR: 提出40亿参数基础模型Bright - 4B，可直接从3D明场体积中分割亚细胞结构，性能优于现有模型，代码及模型将开放。


<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜下稳健的体积分割通常依赖荧光或大量后期处理，解决这一方法空白。

Method: 引入在单位超球面上学习的Bright - 4B，结合Native Sparse Attention机制、深度 - 宽度残差HyperConnections、软混合专家机制，采用即插即用各向异性块嵌入。

Result: 该模型能单独从明场堆栈中准确分割细胞核、线粒体等细胞器，在多数据集上保留精细结构细节，优于当代CNN和Transformer基线。

Conclusion: 发布代码、预训练权重和模型以推动大规模无标记3D细胞图谱绘制。
...
### [383] [FluenceFormer: Transformer-Driven Multi-Beam Fluence Map Regression for Radiotherapy Planning](https://arxiv.org/abs/2512.22425)
*Ujunwa Mgboh,Rafi Ibn Sultan,Joshua Kim,Kundan Thind,Dongxiao Zhu*

Main category: cs.CV

TL;DR: 提出FluenceFormer框架用于直接、几何感知的注量回归，在前列腺IMRT数据集上评估，Swin UNETR骨干的FluenceFormer性能最佳。


<details>
  <summary>Details</summary>
Motivation: 注量图预测是自动放射治疗规划的核心，但因解剖结构与束流强度调制关系复杂而成为不适定逆问题，先前卷积方法难以捕捉长距离依赖。

Method: 引入FluenceFormer，采用统一两阶段设计，第一阶段从解剖输入预测全局剂量先验，第二阶段结合束流几何回归物理校准的注量图，使用Fluence - Aware Regression (FAR)损失。

Result: 在多个变压器骨干网络上评估，Swin UNETR骨干的FluenceFormer性能最强，降低能量误差到4.5%，结构保真度有显著提升。

Conclusion: FluenceFormer框架有效，能改善注量图预测效果，优于现有基准CNN和单阶段方法。
...
### [384] [SuperiorGAT: Graph Attention Networks for Sparse LiDAR Point Cloud Reconstruction in Autonomous Systems](https://arxiv.org/abs/2512.22439)
*Khalfalla Awedat,Mohamed Abidalrekab,Gurcan Comert,Mustafa Ayad*

Main category: cs.CV

TL;DR: 本文提出SuperiorGAT框架重建稀疏LiDAR点云缺失的高程信息，实验表明其比PointNet和更深的GAT基线模型表现更好，且无需额外硬件提升分辨率。


<details>
  <summary>Details</summary>
Motivation: 基于LiDAR的感知受固定垂直光束分辨率限制，环境遮挡还会导致光束丢失，需解决缺失高程信息问题。

Method: 提出基于图注意力的SuperiorGAT框架，将LiDAR扫描建模为光束感知图，结合门控残差融合和前馈细化，不增加网络深度。通过移除每第四个垂直扫描光束模拟结构化光束丢失。

Result: 在不同KITTI环境实验中，SuperiorGAT重建误差更低、几何一致性更好，定性X - Z投影显示能保持结构完整性且垂直失真小。

Conclusion: 架构改进提供了一种计算高效的方法，无需额外传感器硬件即可提高LiDAR分辨率。
...
### [385] [Towards Robust Optical-SAR Object Detection under Missing Modalities: A Dynamic Quality-Aware Fusion Framework](https://arxiv.org/abs/2512.22447)
*Zhicheng Zhao,Yuancheng Xu,Andong Lu,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 提出QDFNet用于光学 - SAR目标检测，在模态缺失或退化情况下有效。


<details>
  <summary>Details</summary>
Motivation: 光学和SAR图像融合的目标检测实用部署受限，现有方法对随机模态缺失鲁棒性有限，缺乏有效性能提升机制。

Method: 提出质量感知动态融合网络QDFNet，设计DMQA模块和OCNF模块，前者迭代评估特征可靠性，后者用正交约束调整融合权重。

Result: 在SpaceNet6 - OTD和OGSOD - 2.0数据集的实验中，QDFNet比现有方法具有优越性。

Conclusion: QDFNet能有效处理光学 - SAR数据模态缺失和退化问题，表现优于现有方法。
...
### [386] [CoAgent: Collaborative Planning and Consistency Agent for Coherent Video Generation](https://arxiv.org/abs/2512.22536)
*Qinglin Zeng,Kaitong Cai,Ruiqi Chen,Qinhan Lv,Keze Wang*

Main category: cs.CV

TL;DR: 为解决开放域视频生成连贯性和视觉一致性问题，提出CoAgent框架，经实验证明其有效提升长视频相关质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型常独立处理每个镜头，存在身份漂移、场景不一致和时间结构不稳定问题。

Method: 提出CoAgent框架，构建计划 - 合成 - 验证流程，含故事板规划器、全局上下文管理器、合成模块、验证代理和节奏感知编辑器。

Result: 大量实验表明CoAgent在长视频生成中显著提高连贯性、视觉一致性和叙事质量。

Conclusion: CoAgent框架有效解决了开放域视频生成中保持叙事连贯性和视觉一致性的挑战。
...
### [387] [Self-Rewarded Multimodal Coherent Reasoning Across Diverse Visual Domains](https://arxiv.org/abs/2512.22545)
*Jesen Zhang,Ningyuan Liu,Kaitong Cai,Sidi Liu,Jing Yang,Ziliang Chen,Xiaofei Sun,Keze Wang*

Main category: cs.CV

TL;DR: 提出轻量级无标签框架SR - MCR，利用模型输出的内在过程信号对齐推理，在视觉基准测试中提升性能。
...
Method: 引入SR - MCR框架，集成五个自引用线索到归一化、可靠性加权奖励中提供细粒度过程级指导，使用无批评者的GRPO目标并增强置信度感知冷却机制。

Result: 基于Qwen2.5 - VL，SR - MCR在多个视觉基准测试中提高答案准确性和推理连贯性，SR - MCR - 7B在同类开源模型中达SOTA，平均准确率81.4%。

Conclusion: 消融实验证实每个奖励项和冷却模块的独立贡献。
...
### [388] [Investigating Deep Learning Models for Ejection Fraction Estimation from Echocardiography Videos](https://arxiv.org/abs/2512.22657)
*Shravan Saranyan,Pramit Saha*

Main category: cs.CV

TL;DR: 研究几种深度学习架构对超声心动图视频左心室射血分数（LVEF）的估计效果，发现修改后的3D Inception架构表现最佳。


<details>
  <summary>Details</summary>
Motivation: 手动评估超声心动图中心脏功能耗时且观察者间差异大，深度学习方法有潜力替代。

Method: 研究3D Inception、双流和CNN - RNN等深度学习架构，系统评估架构修改和融合策略，在EchoNet - Dynamic数据集上训练和评估模型。

Result: 修改后的3D Inception架构整体性能最佳，RMSE为6.79%；各架构有过拟合倾向，小而简单的模型泛化性更好；模型性能对超参数选择敏感。

Conclusion: 研究聚焦超声心动图的LVEF估计，其架构设计和训练策略的见解可用于更广泛的视频分析任务。
...
### [389] [Unleashing Foundation Vision Models: Adaptive Transfer for Diverse Data-Limited Scientific Domains](https://arxiv.org/abs/2512.22664)
*Qiankun Li,Feng He,Huabao Chen,Xin Ning,Kun Wang,Zengfu Wang*

Main category: cs.CV

TL;DR: 提出CLAdapter，可将大规模数据学习的特征适配到数据有限的下游任务，在多领域数据集实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 大数据时代计算机视觉虽在大规模数据集上预训练模型获益，但数据有限的专业下游任务仍具挑战。

Method: 提出CLAdapter，引入注意力机制和聚类中心，通过分布相关性和变换矩阵增强特征，统一接口可集成多种模型架构。

Result: 在10个跨多领域数据集实验中，CLAdapter取得了当前最优性能。

Conclusion: CLAdapter能有效释放基础视觉模型潜力，实现自适应迁移。
...
### [390] [Next Best View Selections for Semantic and Dynamic 3D Gaussian Splatting](https://arxiv.org/abs/2512.22771)
*Yiqian Li,Wen Jiang,Kostas Daniilidis*

Main category: cs.CV

TL;DR: 将视图选择问题转化为主动学习问题，提出带Fisher信息的主动学习算法，在多数据集实验中提升了渲染质量和语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 理解语义和动态对具身智能体在各种任务中很关键，且这两个任务比静态场景理解任务有更多数据冗余，需解决视图选择问题。

Method: 将视图选择问题表述为主动学习问题，提出带Fisher信息的主动学习算法，量化候选视图的信息性。

Result: 在大规模静态图像和动态视频数据集实验中，该方法能持续提升渲染质量和语义分割性能，优于随机选择和基于不确定性启发式的基线方法。

Conclusion: 该方法为启发式或随机策略提供了原则性替代方案，可联合处理语义推理和动态场景建模。
...
### [391] [EgoReAct: Egocentric Video-Driven 3D Human Reaction Generation](https://arxiv.org/abs/2512.22808)
...
TL;DR: 文章构建了Human Reaction Dataset（HRD），提出EgoReAct框架，实现从第一人称视频流实时生成3D对齐的人类反应动作，实验显示优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集存在第一人称视频和反应动作的空间不一致问题，忠实建模第一人称视频中的人类反应具有挑战。

Method: 构建HRD数据集；通过Vector Quantised - Variational AutoEncoder将反应动作压缩到潜在空间，训练Generative Pre - trained Transformer从视觉输入生成反应；生成时融入3D动态特征。

Result: EgoReAct在真实感、空间一致性和生成效率上显著优于先前方法，且生成过程严格遵循因果关系。

Conclusion: 提出的方法在第一人称视频生成人类反应动作任务上效果良好。
...
### [392] [SwinTF3D: A Lightweight Multimodal Fusion Approach for Text-Guided 3D Medical Image Segmentation](https://arxiv.org/abs/2512.22878)
*Hasan Faraz Khan,Noor Fatima,Muzammil Behzad*

Main category: cs.CV

TL;DR: 提出轻量级多模态融合方法SwinTF3D用于文本引导3D医学图像分割，实验证明其效果好、泛化性强且高效。


<details>
  <summary>Details</summary>
Motivation: 现有3D分割框架依赖视觉学习，缺乏语义理解，适应性受限，无法满足灵活的用户定义分割目标。

Method: 提出SwinTF3D，使用基于变压器的视觉编码器提取体积特征，并通过高效融合机制将其与紧凑文本编码器集成。

Result: 在BTCV数据集上，SwinTF3D尽管架构紧凑，但在多个器官上实现了有竞争力的Dice和IoU分数，对未见数据泛化性好，比传统基于变压器的分割网络效率更高。

Conclusion: SwinTF3D将视觉感知与语言理解相结合，为交互式、文本驱动的3D医学图像分割建立了实用且可解释的范式，为临床成像提供了更具适应性和资源效率的解决方案。
...
### [393] [Evaluating an Adaptive Multispectral Turret System for Autonomous Tracking Across Variable Illumination Conditions](https://arxiv.org/abs/2512.22263)
*Aahan Sachdeva,Dhanvinkumar Ganeshkumar,James E. Gallagher,Tyler Treat,Edward J. Oughton*

Main category: cs.CV

TL;DR: 提出自适应框架融合RGB和LWIR视频流，训练多光照下YOLO模型，评估显示不同光照下有良好表现，提升机器人视觉性能。


<details>
  <summary>Details</summary>
Motivation: 传统RGB检测管道在低光照环境表现不佳，热成像系统缺乏颜色和纹理信息，需克服这些局限。

Method: 提出自适应框架，以不同比例融合RGB和LWIR视频流，在三种光照水平下用超22000张标注图像训练33个YOLO模型。

Result: 最佳全光照和微光模型置信度分别达92.8%和92.0%，显著优于基线；无光照下40/60融合达71.0%。

Conclusion: 自适应RGB - LWIR融合提高了各光照条件下检测置信度和可靠性，提升了自主机器人视觉性能。
...
### [394] [Real-Time In-Cabin Driver Behavior Recognition on Low-Cost Edge Hardware](https://arxiv.org/abs/2512.22298)
*Vesal Ahsani,Babak Hossein Khalaj*

Main category: cs.CV

TL;DR: 提出适用于树莓派5和谷歌珊瑚边缘TPU的单摄像头车内驾驶员行为识别系统，能实时识别17种行为。


<details>
  <summary>Details</summary>
Motivation: 车内驾驶员监测系统需在计算、功耗和成本受限下，低延迟识别分心和困倦相关行为。

Method: 结合紧凑的逐帧视觉模型、混淆因素感知的标签设计和时间决策头，使用许可数据集训练评估，并进行实车测试。

Result: 在树莓派5和谷歌珊瑚边缘TPU上分别实现约16 FPS和25 FPS，可实时监测并稳定生成警报。
...
### [395] [OpenGround: Active Cognition-based Reasoning for Open-World 3D Visual Grounding](https://arxiv.org/abs/2512.23020)
*Wenyuan Huang,Zhao Wang,Zhou Wei,Ting Huang,Fang Zhao,Jian Yang,Zhenyu Zhang*

Main category: cs.CV

TL;DR: 提出OpenGround框架用于开放世界3D视觉定位，利用ACR模块克服预定义OLT的局限，还有新数据集OpenTarget，实验表现佳。


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉定位方法依赖预定义的OLT查询VLM限制了在未定义或不可预见目标场景的应用。

Method: 提出OpenGround零样本框架，核心是ACR模块通过认知任务链感知目标、主动推理相关对象，动态更新OLT；还提出数据集OpenTarget。

Result: 在Nr3D上性能有竞争力，在ScanRefer上达到最优，在OpenTarget上提升17.6%。

Conclusion: OpenGround可用于预定义和开放世界类别，能有效解决现有方法的局限性。
...
### [396] [MedSAM-based lung masking for multi-label chest X-ray classification](https://arxiv.org/abs/2512.23089)
*Brayden Miao,Zain Rehman,Xin Miao,Siming Liu,Jianjie Wang*

Main category: cs.CV

TL;DR: 文章提出基于MedSAM的CXR分类流水线，研究不同肺掩码对多标签异常分类的影响，指出肺掩码应根据骨干网络和临床目标选择。


<details>
  <summary>Details</summary>
Motivation: CXR自动解读因疾病信号弱、数据集偏差和空间监督有限存在挑战，MedSAM为改善CXR分析的鲁棒性和可解释性提供机会。

Method: 提出整合MedSAM作为肺区域提取模块的CXR分类流水线，微调MedSAM，用NIH CXR数据集训练和评估CNN进行多标签异常预测。

Result: MedSAM可生成合理肺掩码，掩码效果与任务和架构有关，不同掩码在异常分类和正常筛查中有不同表现。

Conclusion: 肺掩码应视为可控空间先验，根据骨干网络和临床目标选择，而非统一应用。
...
### [397] [PathoSyn: Imaging-Pathology MRI Synthesis via Disentangled Deviation Diffusion](https://arxiv.org/abs/2512.23130)
*Jian Wang,Sixing Rong,Jiarui Xing,Yuling Xu,Weide Liu*

Main category: cs.CV

TL;DR: 提出PathoSyn统一生成框架用于MRI图像合成，解决现有模型特征纠缠问题，性能优于基线，代码将开源。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在全局像素域或依赖二进制掩码，存在特征纠缠，导致解剖结构损坏或不连续，需要改进。

Method: 将合成任务分解为确定性解剖重建和随机偏差建模，采用偏差空间扩散模型学习病理残差的条件分布，结合接缝感知融合策略和推理时稳定模块。

Result: 在肿瘤成像基准上的定量和定性评估表明，PathoSyn在感知真实性和解剖保真度上显著优于整体扩散和掩码条件基线。

Conclusion: PathoSyn为生成高保真患者特定合成数据集提供了数学上合理的管道，有助于低数据下诊断算法开发，支持精准干预规划和临床决策支持系统基准测试。
...
### [398] [ForCM: Forest Cover Mapping from Multispectral Sentinel-2 Image by Integrating Deep Learning with Object-Based Image Analysis](https://arxiv.org/abs/2512.23196)
...
TL;DR: 研究提出结合OBIA与DL的ForCM方法用于森林覆盖制图，评估多种DL模型，与OBIA结合提升精度，结果显示ForCM优于传统OBIA。


<details>
  <summary>Details</summary>
Motivation: 提升森林覆盖制图的准确性，探索新的制图方法，支持全球环境监测和保护工作。

Method: 提出ForCM方法，结合OBIA和DL，评估UNet、UNet++等多种DL模型，将有效模型与OBIA结合，使用QGIS工具。

Result: ForCM方法提升了森林覆盖制图精度，ResUNet - OBIA总体准确率达94.54%，AttentionUNet - OBIA达95.64%，传统OBIA为92.91%。

Conclusion: ForCM方法有效提升森林覆盖制图精度，QGIS等免费工具在一定限制内可用于准确制图，支持全球环境监测和保护。
...
### [399] [Exploring Syn-to-Real Domain Adaptation for Military Target Detection](https://arxiv.org/abs/2512.23208)
*Jongoh Jeong,Youngjin Oh,Gyeongrae Nam,Jeongeun Lee,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: 提出用虚幻引擎生成基于RGB的合成数据用于跨域军事目标检测，实验发现用少量图像提示的域适应方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有域自适应目标检测算法不适用于军事多环境，SAR数据采集处理成本高，军事目标检测数据集缺乏，需低成本方法。

Method: 用虚幻引擎生成RGB合成数据，进行合成到真实的迁移实验，在提出的训练-验证数据集上对不同监督程度的域适应方法进行基准测试。

Result: 使用少量图像提示（如目标类别）的当前方法比无监督或半监督DA方法有显著改进。

Conclusion: 认识到目前仍有待克服的挑战。
...
### [400] [INTERACT-CMIL: Multi-Task Shared Learning and Inter-Task Consistency for Conjunctival Melanocytic Intraepithelial Lesion Grading](https://arxiv.org/abs/2512.22666)
...
TL;DR: 提出INTERACT - CMIL多头部深度学习框架用于结膜黑素细胞上皮内病变（CMIL）分级，在新数据集上表现优于基线模型，为CMIL诊断提供基准。


<details>
  <summary>Details</summary>
Motivation: CMIL准确分级对治疗和黑色素瘤预测至关重要，但因细微形态线索和相关诊断标准导致分级困难。

Method: 引入INTERACT - CMIL框架，通过共享特征学习与组合部分监督及相互依赖损失，联合预测五个组织病理学轴。

Result: 在新策划的多中心数据集上，INTERACT - CMIL相比CNN和基础模型基线有持续改进，相对宏观F1增益最高达55.1%（WHO4）和25.0%（垂直扩散）。

Conclusion: 该框架提供连贯、可解释的多标准预测，为CMIL诊断提供可重复计算基准，迈向标准化数字眼病理学。
...
### [401] [Holi-DETR: Holistic Fashion Item Detection Leveraging Contextual Information](https://arxiv.org/abs/2512.23221)
*Youngchae Kwon,Jinyoung Choi,Injung Kim*

Main category: cs.CV

TL;DR: 提出了一种新的整体检测Transformer（Holi - DETR）用于整体检测时尚单品，能利用上下文信息，实验显示提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 由于时尚单品外观多样和子类相似带来的模糊性，传统检测器难以有效检测时尚单品，因此需要新的解决方案。

Method: 提出Holi - DETR，通过利用时尚单品之间的共现关系、基于单品间空间排列的相对位置和大小、单品与人体关键点的空间关系这三种上下文信息，将其整合到Detection Transformer及其后续模型中。

Result: 提出的方法在平均精度（AP）方面，分别使原始DETR和最近开发的Co - DETR性能提升了3.6个百分点和1.1个百分点。

Conclusion: Holi - DETR利用上下文信息整体检测时尚单品的方法有效，能提高检测性能。
...
### [402] [Multimodal Diffeomorphic Registration with Neural ODEs and Structural Descriptors](https://arxiv.org/abs/2512.22689)
*Salvador Rodriguez-Sanz,Monica Hernandez*

Main category: cs.CV

TL;DR: 提出用神经常微分方程的多模态微分同胚配准方法，在不同实验中取得超现有基线的结果，展示了鲁棒性、不同尺度适用性和效率。


<details>
  <summary>Details</summary>
Motivation: 非刚性配准算法在准确性、变形模型计算复杂度和正则化方面存在权衡，且多适用于单模态场景，基于学习的模型有训练扫描要求高和对未见模态推理性能下降的问题。

Method: 提出利用神经常微分方程范式中连续深度网络与结构描述符的实例特定框架，有三种不同变体，整合基于图像或特征的结构描述符和局部互信息计算的非结构图像相似度。

Result: 在不同扫描数据集组合实验中取得超越现有基线的定性和定量结果。

Conclusion: 所提框架对不同程度显式正则化有鲁棒性，适用于不同尺度配准，对大变形配准更高效。
...
### [403] [Anomaly Detection by Effectively Leveraging Synthetic Images](https://arxiv.org/abs/2512.23227)
*Sungho Kang,Hyunkyu Park,Yeonho Lee,Hanbyul Lee,Mijoo Jeong,YeongHyeon Park,Injae Lee,Juneho Yi*

Main category: cs.CV

TL;DR: 本文聚焦利用合成图像提升异常检测性能，提出新框架和两阶段训练策略，降低数据收集成本并提高性能，实验证明有效。


<details>
  <summary>Details</summary>
Motivation: 因真实缺陷图像稀缺，现有合成策略有成本与质量的权衡问题，需有效利用合成图像提升异常检测性能。

Method: 提出利用预训练文本引导图像到图像翻译模型和图像检索模型生成合成缺陷图像的框架，引入两阶段训练策略。

Result: 在MVTec AD数据集上的实验证明该方法有效。

Conclusion: 所提方法能显著降低数据收集成本，同时提高异常检测性能。
...
### [404] [Physics-Inspired Modeling and Content Adaptive Routing in an Infrared Gas Leak Detection Network](https://arxiv.org/abs/2512.23234)
*Dongsheng Li,Chaobo Chen,Siling Wang,Song Gao*

Main category: cs.CV

TL;DR: 提出物理边缘混合气体动态路由网络PEG - DRNet用于红外气体泄漏检测，在数据集上性能超基线且计算效率高。


<details>
  <summary>Details</summary>
Motivation: 红外气体泄漏检测因羽流特征难以检测，研究旨在找到有效检测方法。

Method: 引入Gas Block建模气体传输；提出AGPEO计算边缘先验并通过MSEPM转化为边缘特征；构建CASR - PAN选择性传播特征。

Result: 在IIG数据集上，PEG - DRNet的各项精度指标超RT - DETR - R18基线，只需43.7 Gflops和14.9 M参数。

Conclusion: PEG - DRNet整体性能优，在精度和计算效率间取得最佳平衡，优于现有CNN和Transformer检测器。
...
### [405] [ViLaCD-R1: A Vision-Language Framework for Semantic Change Detection in Remote Sensing](https://arxiv.org/abs/2512.23244)
*Xingwei Ma,Shiyang Feng,Bo Zhang,Bin Wang*

Main category: cs.CV

TL;DR: 提出ViLaCD - R1框架解决遥感变化检测问题并取得良好效果


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法难以捕捉高层语义且易受非语义干扰，现有基于多模态和VLM的方法存在空间定位不准、边界划分不精确和可解释性有限等问题

Method: 提出两阶段框架ViLaCD - R1，包括多图像推理器（MIR）和掩码引导解码器（MGD），通过监督微调（SFT）和强化学习（RL）训练VLM得到粗变化掩码，解码器结合图像特征和粗掩码预测精确二值变化图

Result: 在多个RSCD基准测试中，ViLaCD - R1显著提高真实语义变化识别和定位能力，有力抑制非语义变化，在复杂现实场景中达到了最先进的准确率

Conclusion: ViLaCD - R1框架有效解决了现有遥感变化检测方法的问题，提升了检测性能
...
### [406] [MedGemma vs GPT-4: Open-Source and Proprietary Zero-shot Medical Disease Classification from Images](https://arxiv.org/abs/2512.23304)
*Md. Sazzadul Islam Prottasha,Nabil Walid Rafi*

Main category: cs.CV

TL;DR: 研究对比MedGemma和GPT - 4在六种疾病诊断中的表现，发现微调后的MedGemma - 4b - it模型表现更优，强调领域特定微调的重要性。


<details>
  <summary>Details</summary>
Motivation: 比较不同AI架构在医学影像疾病分类中的表现，探索更有效的疾病诊断方法。

Method: 对比专业开源代理MedGemma和专有大模型GPT - 4对六种疾病的诊断能力，使用低秩自适应（LoRA）微调MedGemma - 4b - it模型，通过混淆矩阵和分类报告进行定量分析。

Result: 微调后的MedGemma - 4b - it模型平均测试准确率达80.37%，高于未调整的GPT - 4的69.58%，在癌症和肺炎检测等高风险临床任务中灵敏度更高。

Conclusion: 领域特定微调对减少临床应用中的幻觉至关重要，MedGemma可作为复杂循证医学推理的精细工具。
...
### [407] [ByteLoom: Weaving Geometry-Consistent Human-Object Interactions through Progressive Curriculum Learning](https://arxiv.org/abs/2512.22854)
*Bangya Liu,Xinyu Gong,Zelin Zhao,Ziyang Song,Yulei Lu,Suhui Wu,Jun Zhang,Suman Banerjee,Hao Zhang*

Main category: cs.CV

TL;DR: 提出基于Diffusion Transformer的ByteLoom框架生成HOI视频，解决现有方法局限，实验证明效果好。


<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法存在缺乏注入物体多视图信息机制和依赖精细手部网格注释的问题。

Method: 引入基于Diffusion Transformer的ByteLoom框架，提出RCM - cache机制，设计训练课程。

Result: 方法能忠实保留人类身份和物体多视图几何形状，保持平滑运动和物体操作。

Conclusion: 所提方法有效解决现有HOI视频生成方法的局限。
...
### [408] [SoulX-LiveTalk Technical Report](https://arxiv.org/abs/2512.23379)
*Le Shen,Qiao Qian,Tan Yu,Ke Zhou,Tianhang Yu,Yu Zhan,Zhenjie Wang,Ming Tao,Shunshun Yin,Siyuan Liu*

Main category: cs.CV

TL;DR: 为解决大规模扩散模型实时音频驱动的头像生成的工程难题，提出SoulX - LiveTalk框架，实现亚秒级启动延迟和32 FPS实时吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有大规模扩散模型在实时、无限时长、音频驱动的头像生成中存在计算负载和严格延迟限制的冲突，且现有方法常牺牲视觉保真度。

Method: 引入Self - correcting Bidirectional Distillation策略保留视频块内双向注意力；采用Multi - step Retrospective Self - Correction Mechanism确保无限生成稳定性；设计全栈推理加速套件。

Result: SoulX - LiveTalk是首个14B规模系统，实现0.87s的亚秒级启动延迟和32 FPS的实时吞吐量。
...
### [409] [Fuzzy-Logic and Deep Learning for Environmental Condition-Aware Road Surface Classification](https://arxiv.org/abs/2512.23436)
*Mustafa Demetgul,Sanja Lazarova Molnar*

Main category: cs.CV

TL;DR: 本文提出基于天气与路面状况数据的实时道路监测系统，对比不同深度学习算法分类路面状况，准确率超95%，还提出用模糊逻辑根据天气和时间分类路面。


<details>
  <summary>Details</summary>
Motivation: 经典道路监测方法昂贵且缺乏系统性，耗时久，需要更有效的监测方式。

Method: 用手机摄像头收集数据，测试大量基于图像的深度学习算法进行道路分类，结合道路加速度数据与图像数据训练，对比基于加速度和基于摄像头图像的方法及不同深度学习算法性能，使用模糊逻辑分类路面。

Result: 对于路面状况分类，实现超95%的准确率。

Conclusion: 所提基于实时数据和深度学习算法的系统能有效分类路面状况，可根据天气和时间用模糊逻辑进行分类。
...
### [410] [CoFi-Dec: Hallucination-Resistant Decoding via Coarse-to-Fine Generative Feedback in Large Vision-Language Models](https://arxiv.org/abs/2512.23453)
*Zongsheng Cao,Yangfan He,Anran Liu,Jun Xie,Feng Chen,Zepeng Wang*

Main category: cs.CV

TL;DR: 提出无训练解码框架CoFi - Dec减少大视觉语言模型幻觉，实验表现佳且框架通用。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型易产生与视觉输入不一致的幻觉内容，限制其在现实应用中的可靠性。

Method: 提出CoFi - Dec框架，结合生成式自我反馈与粗到细的视觉调节，生成中间文本响应并转化为合成图像形成视觉假设，用基于Wasserstein的融合机制统一预测。

Result: 在六个专注幻觉的基准测试中，CoFi - Dec大幅减少实体级和语义级幻觉，优于现有解码策略。

Conclusion: CoFi - Dec框架与模型无关，无需额外训练，可无缝应用于多种大视觉语言模型。
...
### [411] [HY-Motion 1.0: Scaling Flow Matching Models for Text-To-Motion Generation](https://arxiv.org/abs/2512.23464)
...
TL;DR: 介绍HY - Motion 1.0，一种基于文本描述生成3D人体运动的模型，有新训练范式，成果佳并开源。


<details>
  <summary>Details</summary>
Motivation: 推动3D人体运动生成模型发展，向商业成熟过渡，提升模型指令跟随能力和运动生成质量。

Method: 将基于Diffusion Transformer的流匹配模型扩展到十亿参数规模，采用包括大规模预训练、高质量微调、基于人类反馈和奖励模型的强化学习的全阶段训练范式，还有精心的数据处理流程。

Result: 模型覆盖范围广，涵盖6大类超200种运动类别，指令跟随能力远超现有开源基准。

Conclusion: 开源HY - Motion 1.0以促进未来研究和模型商业化。
...
### [412] [AnyMS: Bottom-up Attention Decoupling for Layout-guided and Training-free Multi-subject Customization](https://arxiv.org/abs/2512.23537)
*Binhe Yu,Zhen Wang,Kexin Li,Yuqian Yuan,Wenqiao Zhang,Long Chen,Juncheng Li,Jun Xiao,Yueting Zhuang*

Main category: cs.CV

TL;DR: 提出无训练框架AnyMS用于布局引导的多主体定制，实现文本对齐、主体身份保留和布局控制，实验表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多主体定制方法难以平衡文本对齐、主体身份保留和布局控制三个目标，且依赖额外训练限制了可扩展性和效率。

Method: 提出AnyMS框架，利用文本提示、主体图像和布局约束三个输入条件，引入自下而上的双级注意力解耦机制，使用预训练图像适配器提取特定特征。

Result: AnyMS实现了最先进的性能，支持复杂组合并能扩展到更多主体。

Conclusion: AnyMS是一种有效的无训练多主体定制框架，能解决现有方法的问题。
...
### [413] [Visual Language Hypothesis](https://arxiv.org/abs/2512.23335)
*Xiu Li*

Main category: cs.CV

TL;DR: 从结构和拓扑视角研究视觉表征学习，推导视觉观测空间结构及相关理论结果。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度深入研究视觉表征学习，探寻视觉理解的基础。

Method: 基于视觉理解需语义语言的假设和表征学习的前提，推导视觉观测空间的纤维束结构并得出理论结果。

Result: 一是语义商不是子流形，获得语义不变性需非同胚判别目标；二是近似商对模型架构有结构要求，语义抽象需能支持拓扑变化的机制。

Conclusion: 研究结果是解释性的，提供拓扑视角，与大规模判别和多模态模型经验规律及统计学习理论经典原则相符。
...
### [414] [PathFound: An Agentic Multimodal Model Activating Evidence-seeking Pathological Diagnosis](https://arxiv.org/abs/2512.23545)
*Shengyi Hua,Jianfeng Wu,Tianle Shen,Kangzhe Hu,Zhongzhen Huang,Shujuan Ni,Zhihong Zhang,Yuan Li,Zhe Wang,Xiaofan Zhang*

Main category: cs.CV

TL;DR: 提出PathFound模型支持病理诊断中的寻证推理，提升诊断准确性。
...
Method: 将病理视觉基础模型、视觉语言模型和强化学习训练的推理模型集成，分初始诊断、寻证、最终决策阶段进行。

Result: 在多个大型多模态模型中采用该策略提高诊断准确性，PathFound在不同临床场景达最优性能。

Conclusion: 寻证工作流在计算病理学中有效，PathFound有发现细微特征的潜力。
...
### [415] [RxnBench: A Multimodal Benchmark for Evaluating Large Language Models on Chemical Reaction Understanding from Scientific Literature](https://arxiv.org/abs/2512.23565)
...
TL;DR: 介绍RxnBench评估多模态大语言模型对化学反应的理解能力，发现模型存在能力差距，强调需特定视觉编码器和推理引擎。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在化学领域应用潜力大，但对真实文献中反应图形语言的理解能力未充分研究，需进行评估。

Method: 引入RxnBench，包含单图问答和全文问答两个任务，对多模态大语言模型进行评估。

Result: 模型在提取明确文本方面表现出色，但在化学逻辑和结构识别上有困难，推理模型表现更好，但在全文问答上准确率均未达50%。

Conclusion: 迫切需要特定领域视觉编码器和更强推理引擎来推进自主AI化学家发展。
...
### [416] [Towards Integrating Uncertainty for Domain-Agnostic Segmentation](https://arxiv.org/abs/2512.23427)
*Jesse Brouwers,Xiaoyan Xing,Alexander Timans*

Main category: cs.CV

TL;DR: 研究不确定性量化对提升分割基础模型泛化性的作用，构建UncertSAM基准，评估相关方法，发现拉普拉斯近似有潜力。


<details>
  <summary>Details</summary>
Motivation: 分割基础模型在特定领域易受影响，研究不确定性量化能否以领域无关方式增强模型泛化性。

Method: 构建UncertSAM基准，评估轻量级事后不确定性估计方法，评估不确定性引导的预测细化步骤。

Result: 最后一层拉普拉斯近似的不确定性估计与分割误差相关性好，细化效果初步显现。

Conclusion: 将不确定性纳入分割模型有潜力实现稳健、领域无关的性能。
...
### [417] [Memorization in 3D Shape Generation: An Empirical Study](https://arxiv.org/abs/2512.23628)
*Shu Pu,Boya Zeng,Kaichen Zhou,Mengyu Wang,Zhuang Liu*

Main category: cs.CV

TL;DR: 本文设计评估框架量化3D生成模型的记忆情况，研究数据和建模设计对记忆的影响，提出降低记忆的策略。
...
Method: 设计评估框架量化现有方法的记忆情况，通过对Vecset扩散模型进行对照实验研究。

Result: 数据方面，记忆依赖数据模态，随数据多样性和细粒度条件增加；建模方面，在适度引导规模时达到峰值，可通过更长Vecsets和简单旋转增强缓解。

Conclusion: 框架和分析提供了对3D生成模型记忆的实证理解，提出简单有效的降低记忆策略且不降低生成质量。
...
### [418] [A Deterministic Bicriteria Approximation Algorithm for the Art Gallery Problem](https://arxiv.org/abs/2512.23297)
*Khaled Elbassioni*

Main category: cs.CG

TL;DR: 提出确定性算法解决带洞多边形艺术画廊问题，找到能看到多边形大部分面积的点集。


<details>
  <summary>Details</summary>
Motivation: 解决平面多边形中找到最小点集以看到其他所有点的艺术画廊问题。

Method: 设计确定性算法，针对有h个洞、n个最大位长为L的有理顶点的多边形和参数δ。

Result: 找到大小为O(OPT⋅log(h + 2)⋅log(OPT⋅log(h + 2)))的点集，可看到至少(1 - δ)的多边形面积，算法运行时间是h、n、L和log(1/δ)的多项式。

Conclusion: 该算法能在一定条件下有效解决多边形艺术画廊问题。
...
### [419] [A general framework for deep learning](https://arxiv.org/abs/2512.23425)
*William Kengne,Modou Wade*

Main category: math.ST

TL;DR: 本文提出深度学习通用方法，含两类估计量，建立风险界，应用多种数据，证明估计量在多场景接近极小极大最优。


<details>
  <summary>Details</summary>
Motivation: 为含非参数回归和分类的场景开发深度学习通用方法。

Method: 基于满足广义Bernstein型不等式的数据构建框架，提出非惩罚和稀疏惩罚深度神经网络估计量。

Result: 建立两类估计量在Hölder光滑函数和复合Hölder函数类上期望超额风险界，得出不同数据场景下期望超额风险上界。

Conclusion: NPDNN和SPDNN估计量在许多经典场景下（除对数因子）是极小极大最优的。
...
### [420] [Why Machine Learning Models Systematically Underestimate Extreme Values II: How to Fix It with LatentNN](https://arxiv.org/abs/2512.23138)
*Yuan-Sen Ting*

Main category: astro-ph.IM

TL;DR: 本文指出神经网络存在衰减偏差问题，提出LatentNN方法解决该问题，在多种场景验证效果，可改进低信噪比下神经网络推断。


<details>
  <summary>Details</summary>
Motivation: 衰减偏差影响天文数据驱动模型，虽线性回归问题已解决，但要验证神经网络是否有同样问题并解决。

Method: 引入LatentNN方法，通过最大化观测输入和输出的联合似然来联合优化网络参数和潜在输入值。

Result: LatentNN在一系列信噪比下减少衰减偏差，在标准神经网络偏差大的情况下表现良好。

Conclusion: LatentNN为低信噪比天文数据的神经网络推断提供改进框架，测量误差小于数据固有范围约一半时偏差校正最有效。
...
### [421] [Space AI: Leveraging Artificial Intelligence for Space to Improve Life on Earth](https://arxiv.org/abs/2512.22399)
*Ziyang Wang*

Main category: astro-ph.IM

TL;DR: 本文提出太空人工智能（Space AI）这一跨学科领域，构建系统框架将其分为四个任务场景，认为其能加速人类太空探索并产生社会影响。


<details>
  <summary>Details</summary>
Motivation: 人工智能在多领域有应用，地球发展面临约束，太空是新前沿，需要人工智能在极端不确定性和有限人力监督下实现自主、有弹性的运作。

Method: 整合历史发展和当代进展，提出系统框架，将Space AI分为AI on Earth、AI in Orbit、AI in Deep Space和AI for Multi - Planetary Life四个任务场景。

Result: 构建了Space AI的系统框架，明确了四个任务场景下的具体应用。

Conclusion: Space AI能加速人类太空探索与运作能力，其在传感、机器人、优化和可信AI方面的进展可对地球产生广泛社会影响。
...
### [422] [AnalogSAGE: Self-evolving Analog Design Multi-Agents with Stratified Memory and Grounded Experience](https://arxiv.org/abs/2512.22435)
*Zining Wang,Jian Gao,Weimin Fu,Xiaolong Guo,Xuan Zhang*

Main category: cs.AR

TL;DR: 提出开源的自进化多智能体框架AnalogSAGE用于模拟电路设计，可提升自动化设计的可靠性和自主性。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的模拟电路设计方法依赖提示驱动或预定义模板，难以满足复杂规范要求。

Method: 提出AnalogSAGE框架，通过四层分层内存层协调三阶段智能体探索，利用仿真反馈迭代优化。

Result: 在开源SKY130 PDK和ngspice下评估，与现有框架相比，AnalogSAGE整体通过率提高10倍，一次性通过率提高48倍，参数搜索空间减少4倍。

Conclusion: 分层内存和基于事实的推理能显著提升模拟设计自动化的可靠性和自主性。
...
### [423] [Machine learning models for predicting catastrophe bond coupons using climate data](https://arxiv.org/abs/2512.22660)
*Julia Kończal,Michał Balcerek,Krzysztof Burnecki*

Main category: q-fin.PR

TL;DR: 文章探讨气候变异性对巨灾债券定价的影响，结合新气候指标，对比多种机器学习模型性能，发现纳入气候变量提高预测精度，极随机树表现最佳。


<details>
  <summary>Details</summary>
Motivation: 近年来自然灾害增多，对管理巨灾风险工具需求增长，巨灾债券可转移部分风险，研究气候变异性在其定价中的作用及模型预测性能

Method: 结合新气候指标与常用特征，比较线性回归和多种机器学习算法，如随机森林、梯度提升、极随机树和极端梯度提升

Result: 纳入气候相关变量提高所有模型预测精度，极随机树均方根误差最低

Conclusion: 大规模气候变异性对巨灾债券定价有可衡量影响，机器学习方法能有效捕捉复杂关系
...
### [424] [AI tutoring can safely and effectively support students: An exploratory RCT in UK classrooms](https://arxiv.org/abs/2512.23633)
...
TL;DR: 对165名英国中学生进行随机对照试验，将教学微调的生成式AI模型LearnLM融入辅导，发现其能提供可靠教学指导，学生表现良好，表明AI辅导系统有大规模提供个性化学习支持潜力。


<details>
  <summary>Details</summary>
Motivation: 评估生成式AI是否能帮助扩大一对一辅导资源的可及性。

Method: 开展有165名学生参与的随机对照试验，将LearnLM集成到数学平台的辅导会话中，由专家导师监督其生成的消息。

Result: 76.4%的消息导师零编辑或微编辑认可，学生在学习成果上至少与人类导师辅导效果相当，获LearnLM支持的学生解决新问题成功率高5.5个百分点，导师认可其在提苏格拉底式问题方面的优势，甚至能从中学到新教学方法。

Conclusion: 教学微调的AI辅导系统在大规模提供有效、个性化学习支持方面有光明前景。
...
### [425] [Hallucination Detection and Evaluation of Large Language Model](https://arxiv.org/abs/2512.22416)
*Chenggong Zhang,Haopeng Wang*

Main category: cs.CL

TL;DR: 本文针对大语言模型幻觉问题，提出HHEM评估模型，对比多种检测方法，结果显示HHEM提升效率且有较高准确率，但在总结任务有局限，还发现不同规模模型幻觉情况不同。


<details>
  <summary>Details</summary>
Motivation: 大语言模型幻觉产生误导或无法验证的内容，现有评估方法计算成本高，需更高效的评估方法。

Method: 集成轻量级的Hughes Hallucination Evaluation Model (HHEM)，对多种大语言模型的幻觉检测方法进行对比分析，引入基于段的检索。

Result: HHEM将评估时间从8小时减至10分钟，结合非编造检查的HHEM准确率达82.2%，真阳性率78.9%；HHEM在总结任务处理局部幻觉有困难；较大模型（7B - 9B参数）幻觉较少，中等规模模型不稳定。

Conclusion: 需要平衡计算效率和事实验证的结构化评估框架，以提高大语言模型生成内容的可靠性。
...
### [426] [HiFi-RAG: Hierarchical Content Filtering and Two-Pass Generation for Open-Domain RAG](https://arxiv.org/abs/2512.22442)
*Cattalyya Nuengsigkapian*

Main category: cs.CL

TL;DR: 提出HiFi - RAG应对RAG在开放域设置中的挑战，在竞赛和数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决开放域设置中RAG存在的检索文档含无关信息及生成答案与用户意图不一致的问题。

Method: 采用多阶段管道，利用Gemini 2.5 Flash进行查询制定、分层内容过滤和引用归因，用Gemini 2.5 Pro进行最终答案生成。

Result: 在MMU - RAGent验证集上，ROUGE - L提升至0.274（+19.6%），DeBERTaScore提升至0.677（+6.2%）；在Test2025数据集上，ROUGE - L比参数基线高57.4%，DeBERTaScore高14.9%。

Conclusion: HiFi - RAG是有效的RAG解决方案，能提升生成答案的质量。
...
### [427] [Nested Browser-Use Learning for Agentic Information Seeking](https://arxiv.org/abs/2512.23647)
...
TL;DR: 提出Nested Browser-Use Learning（NestBrowse）框架，实现有效深度网页信息获取，实践有优势且性能优越。


<details>
  <summary>Details</summary>
Motivation: 现有信息检索智能体工具使用局限于API级片段检索和基于URL的页面获取，全浏览器交互虽有潜力但带来复杂问题。

Method: 提出NestBrowse，引入最小且完整的浏览器动作框架，通过嵌套结构分离交互控制和页面探索。

Result: 在深度信息检索基准测试中展现出明显优势，深入分析显示其高效灵活。

Conclusion: NestBrowse可简化智能体推理，实现有效的深度网页信息获取。
...
### [428] [Anka: A Domain-Specific Language for Reliable LLM Code Generation](https://arxiv.org/abs/2512.23214)
*Saif Khalfan Saif Al Mazrouei*

Main category: cs.CL

TL;DR: 大语言模型在复杂编程任务有系统错误，引入特定领域语言Anka测试，结果显示LLMs能从上下文提示学习新DSL，受限语法可减少错误，DSL优于通用语言。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在复杂多步编程任务中的系统错误问题，验证错误源于通用语言的灵活性。

Method: 引入特定领域语言Anka，进行基准测试，并用GPT - 4o - mini进行跨模型验证。

Result: Claude 3.5 Haiku在Anka上解析成功率99.9%，整体任务准确率95.8%；Anka在多步管道任务上比Python准确率高40个百分点，跨模型验证也有优势。

Conclusion: LLMs可从上下文提示学习新DSL并接近原生准确率；受限语法显著减少复杂任务错误；专为LLM生成设计的DSL可优于通用语言。
...
### [429] [SmartSnap: Proactive Evidence Seeking for Self-Verifying Agents](https://arxiv.org/abs/2512.22322)
...
TL;DR: 文章提出SmartSnap范式，将GUI任务中任务完成的验证方式从被动事后验证转变为主动原位自我验证，实验证明该范式可扩展训练LLM驱动的智能体并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有代理强化学习在复杂GUI任务中，任务完成验证存在成本高、可靠性低的问题，阻碍其可扩展性。

Method: 提出SmartSnap范式，引入自我验证智能体，遵循3C原则，利用在线环境对最小决定性快照集进行自我验证，并提供给通用LLM验证器。

Result: 在移动任务实验中，SmartSnap范式使8B和30B模型性能分别提升26.08%和16.66%，培养出的自我验证智能体性能可与DeepSeek V3.1和Qwen3 - 235B - A22B竞争。

Conclusion: SmartSnap范式能实现可扩展的LLM驱动智能体训练，促进解决方案寻找和证据寻找的协同，培养高效、有竞争力的自我验证智能体。
...
### [430] [Towards Efficient Post-Training via Fourier-Driven Adapter Architectures](https://arxiv.org/abs/2512.22378)
*Donggyun Bae,Jongil Park*

Main category: cs.CL

TL;DR: 提出FAA框架用于大语言模型的参数高效微调，在多基准测试中表现良好，且开销低。
...
Method: 将随机傅里叶特征融入轻量级适配器模块，将中间表示分解为低频和高频分量，进行语义信息的频率感知调制。

Result: 在GLUE、E2E NLG和指令调优基准测试中，FAA表现优于现有方法，且计算和内存开销低。消融实验验证了频率感知激活和自适应加权机制的有效性。

Conclusion: FAA是一种强大且高效的大语言模型后训练方法。
...
### [431] [LLM-Guided Exemplar Selection for Few-Shot Wearable-Sensor Human Activity Recognition](https://arxiv.org/abs/2512.22385)
*Elsen Ronando,Sozo Inoue*

Main category: cs.CL

TL;DR: 提出LLM - 引导的示例选择框架用于少样本可穿戴传感器人体活动识别，表现优于经典方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有HAR方法依赖大标注数据集和纯几何示例选择，难以区分相似活动的问题。

Method: 结合LLM生成的知识先验进行语义推理，引导示例评分和选择，再与多种方法结合优化。

Result: 在UCI - HAR数据集严格少样本条件下，宏观F1分数达88.78%，优于经典方法。

Conclusion: LLM衍生的语义先验与结构和几何线索结合，为少样本可穿戴传感器HAR提供更好的示例选择基础。
...
### [432] [ManchuTTS: Towards High-Quality Manchu Speech Synthesis via Flow Matching and Hierarchical Text Representation](https://arxiv.org/abs/2512.22491)
*Suhua Wang,Zifan Wang,Xiaoxin Sun,D. J. Wang,Zhanbo Liu,Xin Li*

Main category: cs.CL

TL;DR: 本文提出针对满语的语音合成方法ManchuTTS，处理了数据稀缺和语音黏着问题，实验效果良好。


<details>
  <summary>Details</summary>
Motivation: 满语作为濒危语言，语音合成面临数据稀缺和强语音黏着的挑战。

Method: 设计三层文本表示和跨模态分层注意力机制，集成深度卷积网络与流匹配Transformer，引入分层对比损失，构建首个满语TTS数据集并采用数据增强策略。

Result: ManchuTTS在5.2小时训练子集上MOS达4.52，优于所有基线模型；分层指导提高黏着词发音准确率31%和韵律自然度27%。

Conclusion: ManchuTTS方法有效，能应对满语语音合成的挑战。
...
### [433] [Learning When Not to Attend Globally](https://arxiv.org/abs/2512.22562)
*Xuan Luo,Kailai Zhang,Xifeng Yan*

Main category: cs.CL

TL;DR: 提出All - or - Here Attention (AHA)让大语言模型动态决定何时关注全局上下文，实验显示可高效替换全注意力操作且揭示全注意力冗余。


<details>
  <summary>Details</summary>
Motivation: 受人类阅读书籍关注当前页面，必要时回顾前文启发，让大语言模型学习动态决定何时关注全局上下文。

Method: 提出All - or - Here Attention (AHA)，利用每个注意力头的二进制路由器为每个令牌在全注意力和局部滑动窗口注意力之间动态切换。

Result: 窗口大小为256个令牌时，多达93%的原始全注意力操作可被滑动窗口注意力替代且无性能损失；评估不同窗口大小，发现上下文依赖呈长尾分布。

Conclusion: 通过将局部处理与全局访问解耦，AHA表明全注意力很大程度上是冗余的，高效推理仅需按需访问全局上下文。
...
### [434] [Fragile Knowledge, Robust Instruction-Following: The Width Pruning Dichotomy in Llama-3.2](https://arxiv.org/abs/2512.22671)
*Pere Martra*

Main category: cs.CL

TL;DR: 采用MAW准则对GLU - MLP层进行结构化宽度剪枝，发现降低扩展比会对模型不同能力产生不同影响，挑战了剪枝导致均匀退化的假设，还发现知识容量与真实性指标的关联及上下文相关的效率权衡。


<details>
  <summary>Details</summary>
Motivation: 探究GLU - MLP层结构化宽度剪枝中扩展比降低对模型不同能力的影响，挑战剪枝均匀退化的假设，连接不同研究领域。

Method: 使用MAW准则进行GLU - MLP层结构化宽度剪枝，评估七种扩展比配置，用综合基准测试评估多种能力。

Result: 在依赖参数知识的任务和困惑度指标上性能可预测地下降，指令跟随能力大幅提升，多步推理保持稳健；发现事实知识容量与真实性指标呈强负相关；剪枝配置可降低能耗，但单请求延迟增加，批量处理有益。

Conclusion: 扩展比是选择性调节认知能力的关键架构参数，MAW引导的宽度剪枝是选择性过滤器，能减少参数知识同时保留或增强行为对齐。
...
### [435] [Conformal Prediction Sets for Next-Token Prediction in Large Language Models: Balancing Coverage Guarantees with Set Efficiency](https://arxiv.org/abs/2512.22682)
*Yoshith Roy Kotla,Varshith Roy Kotla*

Main category: cs.CL

TL;DR: 研究将自适应预测集应用于大词汇量变压器模型的下一个令牌预测，提出VACP框架，实验显示其在保持覆盖率的同时大幅提高效率。
...
Result: 在Gemma - 2B上使用SQUAD和WikiText基准测试，VACP实现89.7%的经验覆盖率（目标90%），平均预测集大小从847个令牌减少到4.3个令牌，效率提高197倍。

Conclusion: 提供了词汇减少的理论分析并发布实现以保证可重复性。
...
### [436] [GHaLIB: A Multilingual Framework for Hope Speech Detection in Low-Resource Languages](https://arxiv.org/abs/2512.22705)
*Ahmed Abdullah,Sana Fatima,Haroon Mahmood*

Main category: cs.CL

TL;DR: 本文提出针对乌尔都语的多语言希望言论检测框架，利用预训练模型，在基准测试中表现良好，证明多语言模型可用于低资源环境。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理中希望言论研究较少，现有研究多针对英语，缺乏低资源语言（如乌尔都语）资源，基于Transformer架构应用于希望言论检测较少。

Method: 使用XLM - RoBERTa、mBERT、EuroBERT和UrduBERT等预训练模型，进行简单预处理并训练分类器。

Result: 在PolyHope - M 2025基准测试中表现出色，乌尔都语二分类F1分数达95.2%，多分类达65.2%，西班牙语、德语和英语也有不错结果。

Conclusion: 现有多语言模型可在低资源环境中实施，有助于识别希望言论和构建更具建设性的数字话语。
...
### [437] [Harnessing Large Language Models for Biomedical Named Entity Recognition](https://arxiv.org/abs/2512.22738)
*Jian Chen,Leilei Su,Cong Sun*

Main category: cs.CL

TL;DR: 提出高效数据中心框架BioSelectTune用于微调大语言模型解决生物医学命名实体识别问题，在多基准测试中达SOTA。


<details>
  <summary>Details</summary>
Motivation: 通用领域大语言模型应用于生物医学命名实体识别任务时缺乏领域知识，且低质量训练数据导致性能下降。

Method: 将生物医学命名实体识别重新表述为结构化JSON生成任务，采用混合超级过滤策略筛选出紧凑、高影响的训练数据集。

Result: BioSelectTune在多个生物医学命名实体识别基准测试中达到SOTA，用50%的精选正数据训练的模型超越全量训练的基线模型和BioMedBERT等强大的领域专业模型。

Conclusion: BioSelectTune能有效解决现有问题，在生物医学命名实体识别任务中表现出色。
...
### [438] [Open-Source Multimodal Moxin Models with Moxin-VLM and Moxin-VLA](https://arxiv.org/abs/2512.22208)
...
TL;DR: 介绍了大语言模型的发展，引出全开源的Moxin 7B模型及三个变体，实验显示性能优异并开源。


<details>
  <summary>Details</summary>
Motivation: 推进大语言模型发展，构建更具包容性和协作性的研究环境，维持健康的开源生态系统。

Method: 依照模型开放框架开发Moxin 7B，基于其开发三个变体，采用开源框架和开放数据进行训练。

Result: 模型在各项评估中取得优异性能。

Conclusion: 推出Moxin 7B及三个变体模型，开源模型、数据和代码，利于推动相关领域研究与发展。
...
### [439] [CNSight: Evaluation of Clinical Note Segmentation Tools](https://arxiv.org/abs/2512.22795)
*Risha Surana,Adrian Law,Sunwoo Kim,Rishab Sridhar,Angxiao Han,Peiyu Hong*

Main category: cs.CL

TL;DR: 文章评估规则基线、领域特定transformer模型和大模型对临床记录进行分段的效果，表明基于API的大模型表现最佳，为方法选择和下游任务奠定基础。


<details>
  <summary>Details</summary>
Motivation: 电子病历系统中的临床记录多为非结构化或半结构化，可靠识别部分边界有助于结构化这些记录，用于二次分析和下游临床应用。

Method: 使用MIMIC - IV中的1000条临床记录评估规则基线、领域特定transformer模型和大模型。

Result: 基于API的大模型整体性能最佳，GPT - 5 - mini在句子级和自由文本分段中平均F1值达72.4，轻量级基线在结构化句子任务有竞争力，在非结构化自由文本表现不佳。
...
### [440] [Hierarchical Geometry of Cognitive States in Transformer Embedding Spaces](https://arxiv.org/abs/2512.22227)
*Sophie Zhao*

Main category: cs.CL

TL;DR: 研究变压器语言模型句子嵌入是否编码与人类认知属性对齐的层次结构，结果表明嵌入空间存在此结构。


<details>
  <summary>Details</summary>
Motivation: 前期工作显示变压器语言模型嵌入空间有几何结构，但高层认知组织研究不足，本文要调查句子嵌入是否编码与人类认知属性对齐的层级结构。

Method: 构建含连续序数能量分和离散层级标签的480句数据集，用多个变压器模型的固定句子嵌入，通过线性和浅非线性探针评估注释可恢复性。

Result: 连续分数和层级标签可可靠解码，浅非线性探针性能更好，词法TF - IDF基线表现差，非参数排列测试表明探针性能超随机，定性分析显示嵌入空间有平滑梯度和相邻层级混淆。

Conclusion: 变压器嵌入空间呈现与人类定义认知属性对齐的层次几何组织。
...
### [441] [AutoForge: Automated Environment Synthesis for Agentic Reinforcement Learning](https://arxiv.org/abs/2512.22857)
...
TL;DR: 本文提出自动化合成模拟环境的统一流程和环境级强化学习算法，经评估有效且有良好泛化性。


<details>
  <summary>Details</summary>
Motivation: 以往基于模拟环境的强化学习存在环境合成半自动化、任务难度不足、模拟用户不稳定和环境异质性等问题。

Method: 提出自动化且可扩展的模拟环境合成统一流程，以及能减轻用户不稳定性、在环境级进行优势估计的环境级强化学习算法。

Result: 在tau - bench、tau2 - Bench和VitaBench等基准测试上验证了方法的有效性。

Conclusion: 所提方法有效，且具备良好的域外泛化能力。
...
### [442] [LENS: LLM-Enabled Narrative Synthesis for Mental Health by Aligning Multimodal Sensing with Language Models](https://arxiv.org/abs/2512.23025)
...
TL;DR: 提出LENS框架将多模态传感数据与语言模型对齐生成心理健康叙述，表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 多模态健康传感将数值时间序列测量转换为自然语言有挑战，现有大语言模型无法原生处理长时传感器流，且配对的传感器 - 文本数据集稀缺。

Method: 构建大规模数据集，将生态瞬时评估回应转化为自然语言描述得到超10万传感器 - 文本问答对；训练补丁级编码器将原始传感器信号投影到语言模型表示空间。

Result: 在标准NLP指标和症状严重程度准确性的特定任务指标上优于强基线；用户研究显示LENS生成的叙述全面且有临床意义。

Conclusion: 推进了大语言模型作为健康传感接口，为基于原始行为信号推理和支持临床决策提供了可扩展途径。
...
### [443] [Is Chain-of-Thought Really Not Explainability? Chain-of-Thought Can Be Faithful without Hint Verbalization](https://arxiv.org/abs/2512.23032)
*Kerem Zaman,Shashank Srivastava*

Main category: cs.CL

TL;DR: 现有评判CoT忠实度的Biasing Features指标可能有误，研究提出新指标，表明大推理时间令牌预算能增加提示表达，并建议采用更广泛的可解释性工具包。


<details>
  <summary>Details</summary>
Motivation: 指出当前使用 Biasing Features 指标判断 CoT 不忠实可能混淆了不忠实和不完整性，需要重新评估。

Method: 在多跳推理任务中测试，使用新 faithful@k 指标，通过因果中介分析。

Result: Biasing Features 标记为不忠实的 CoT 被其他指标判断为忠实，大推理时间令牌预算可大幅增加提示表达，非言语提示也可通过 CoT 影响预测。

Conclusion: 不应仅依赖基于提示的评估，应采用更广泛的可解释性工具包。
...
### [444] [Reservoir Computing inspired Matrix Multiplication-free Language Model](https://arxiv.org/abs/2512.23145)
*Takumi Shiratsuchi,Yuichiro Tanaka,Hakaru Tamukoh*

Main category: cs.CL

TL;DR: 本文聚焦无矩阵乘法语言模型，结合储层计算架构提升计算效率，实验表明该架构减少参数量、训练和推理时间且性能相当。
...
Method: 聚焦无矩阵乘法语言模型，借鉴储层计算架构，部分固定和共享选定层权重、插入储层层，结合操作减少内存访问。

Result: 所提架构最多减少19%参数量、9.9%训练时间、8.0%推理时间，性能与基线模型相当。

Conclusion: 所提架构在不损失性能前提下，有效降低计算成本，提升计算效率。
...
### [445] [Not too long do read: Evaluating LLM-generated extreme scientific summaries](https://arxiv.org/abs/2512.23206)
*Zhuoqi Lyu,Qing Ke*

Main category: cs.CL

TL;DR: 研究旨在评估大语言模型生成高质量科学摘要能力，提出BiomedTLDR数据集，测试发现模型摘要更具提取性。


<details>
  <summary>Details</summary>
Motivation: 缺乏全面高质量科学TLDR数据集阻碍大语言模型摘要能力的开发与评估。

Method: 提出BiomedTLDR数据集，测试流行开源大语言模型基于摘要生成TLDR的表现。

Result: 部分模型能生成类人摘要，但总体上比人类更倾向原文词汇和修辞结构，更具提取性。

Conclusion: 可从代码和数据集链接获取相关资源进一步研究（Lyu和Ke, 2025）。
...
### [446] [Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process](https://arxiv.org/abs/2512.23213)
...
TL;DR: 提出无监督LLM集成方法LLM - PeerReview，从多个LLM生成的候选中选最优响应，在四个数据集上表现良好。


<details>
  <summary>Details</summary>
Motivation: 利用多个具有不同优势的模型的集体智慧，为每个查询选择最理想的响应，且方法需灵活可泛化。

Method: 基于受同行评审启发的框架，分三个阶段：用LLM - as - a - Judge技术评估响应；用图形模型真理推断算法或平均策略聚合分数；选最高分数响应为最佳输出。

Result: 该方法的两个变体在四个数据集上取得强结果，分别比Smoothie - Global高出6.9%和7.3%。
...
### [447] [Data Augmentation for Classification of Negative Pregnancy Outcomes in Imbalanced Data](https://arxiv.org/abs/2512.22732)
*Md Badsha Biswas*

Main category: cs.CL

TL;DR: 论文引入用社交媒体数据研究负面妊娠结果的新方法，构造NLP流程识别、分类孕妇妊娠情况，证明社交媒体数据可辅助妊娠结果流行病学调查。


<details>
  <summary>Details</summary>
Motivation: 美国婴儿死亡率仍是重要公共卫生问题，先天缺陷是主因，目前研究和干预策略仍需完善，所以需创新研究方法。

Method: 利用公开社交媒体（如Twitter）数据，采用强大预处理和数据增强策略克服数据挑战，构建NLP流程自动识别孕妇并分类妊娠结果。

Result: 研究证明社交媒体数据可用于研究孕妇妊娠结果，还具评估干预措施影响、为后续研究提供框架等潜在应用。

Conclusion: 社交媒体数据可作为妊娠结果流行病学调查的辅助资源。
...
### [448] [Interpretable Safety Alignment via SAE-Constructed Low-Rank Subspace Adaptation](https://arxiv.org/abs/2512.23260)
*Dianyun Wang,Qingsen Ma,Yuhu Shang,Zhifeng Lu,Lechen Ning,Zhenbo Xu,Huijia Wu,Zhaofeng He*

Main category: cs.CL

TL;DR: 本文提出在模型微调中利用预训练稀疏自编码器(SAEs)来识别任务相关特征并构建可解释的低秩子空间，提高性能和透明度。


<details>
  <summary>Details</summary>
Motivation: 当前低秩适应方法在学习任务相关权重更新的子空间时是黑盒方式，缺乏可解释性和直接控制，问题源于语义模糊性。

Method: 利用预训练的SAEs在解缠特征空间中识别任务相关特征，构建显式、可解释的低秩子空间来指导适配器初始化，并给出理论分析。

Result: 在安全对齐任务上达到99.6%的安全率，超过全量微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19 - 0.24%的参数，且能通过SAE特征的语义接地提供可解释的见解。

Conclusion: 将机制可解释性纳入微调过程可以同时提高性能和透明度。
...
### [449] [AI Meets Brain: Memory Systems from Cognitive Neuroscience to Autonomous Agents](https://arxiv.org/abs/2512.23343)
...
TL;DR: 文章旨在弥合跨学科差距，综合记忆的跨学科知识，连接认知神经科学与大语言模型驱动的智能体，还进行了多方面分析并展望未来方向。


<details>
  <summary>Details</summary>
Motivation: 当前自主智能体研究受跨学科障碍限制，难以吸收人类记忆机制的精髓。

Method: 系统地综合记忆的跨学科知识，从认知神经科学到大型语言模型再到智能体，逐步阐释记忆的定义和功能；对比分析生物和人工视角下的记忆分类、存储机制和管理生命周期；回顾评估智能体记忆的主流基准；从攻防双视角探索记忆安全。

Result: 无明确对应文本，但可推测完成了对相关内容的分析和探讨。

Conclusion: 展望了未来研究方向，重点关注多模态记忆系统和技能获取。
...
### [450] [Semantic Tree Inference on Text Corpa using a Nested Density Approach together with Large Language Model Embeddings](https://arxiv.org/abs/2512.23471)
*Thomas Haschka,Joseph Bakarji*

Main category: cs.CL

TL;DR: 提出嵌套密度聚类方法推断语义相关文本的层次树，用于文本数据分类，在多数据集验证其跨领域鲁棒性并探讨应用。


<details>
  <summary>Details</summary>
Motivation: 大语言模型高维嵌入用于语义文本分类，但文本语料全局结构语义关系不清晰。

Method: 通过在大语言模型嵌入空间中搜索密集簇确定语义相似度高的文本，逐步放宽密度准则合并簇形成层次树。

Result: 应用于科学摘要、20 Newsgroups和IMDB 50k Movie Reviews等数据集验证方法跨领域鲁棒性。

Conclusion: 该方法可实现数据驱动的研究领域及子领域发现，嵌套密度树能揭示文本数据集语义结构和演化。
...
### [451] [UniHetero: Could Generation Enhance Understanding for Vision-Language-Model at Large Data Scale?](https://arxiv.org/abs/2512.23512)
*Fengjiao Chen,Minhao Jing,Weitao Lu,Yan Feng,Xiaoyu Li,Xuezhi Cao*

Main category: cs.CL

TL;DR: 本文在大规模预训练下分析统一模型UniHetero，发现生成语义可提升理解等关键结论


<details>
  <summary>Details</summary>
Motivation: 探究在大规模数据上，生成任务是否能提升视觉理解

Method: 对具有简洁结构的统一模型UniHetero进行大规模（>2亿样本）预训练

Result: 1. 仅生成语义能提升理解，而非像素；2. 生成任务展现出更好的数据扩展趋势和更高的数据利用率；3. 对输入嵌入进行自回归可有效捕捉视觉细节

Conclusion: 在视觉语言大模型的统一模型中，生成任务在特定条件下可提升理解等，为模型发展提供了洞见
...
### [452] [Lie to Me: Knowledge Graphs for Robust Hallucination Self-Detection in LLMs](https://arxiv.org/abs/2512.23547)
*Sahil Kale,Antonio Luca Alfeo*

Main category: cs.CL

TL;DR: 本文利用知识图谱改进大语言模型幻觉自检测，经评估比标准方法和SOTA方法效果更好，为更安全可靠的语言模型铺平道路。


<details>
  <summary>Details</summary>
Motivation: 幻觉是大语言模型安全部署的主要障碍，基于自检测方法良好表现，研究使用知识图谱改进幻觉自检测。

Method: 将大语言模型响应转换为实体和关系的知识图谱，用这些图谱估计响应包含幻觉的可能性。

Result: 在两个数据集上用GPT - 4o和Gemini - 2.5 - Flash评估，相比标准自检测方法和SelfCheckGPT，准确率最高提升16%，F1分数最高提升20%。

Conclusion: 大语言模型以知识图谱结构分析原子事实效果更好，该低成本、模型无关方法为更安全可靠的语言模型奠定基础。
...
### [453] [Coupling Experts and Routers in Mixture-of-Experts via an Auxiliary Loss](https://arxiv.org/abs/2512.23447)
*Ang Lv,Jin Ma,Yiyuan Ma,Siyuan Qiao*

Main category: cs.CL

TL;DR: 提出专家 - 路由器耦合（ERC）损失解决MoE模型中路由器决策与专家能力对齐问题，经实验证明有效且能灵活控制和跟踪专家专业化水平。


<details>
  <summary>Details</summary>
Motivation: MoE模型缺乏显式约束确保路由器决策与专家能力良好对齐，限制了模型性能。

Method: 提出ERC损失，将每个专家的路由器嵌入作为分配给该专家的令牌的代理令牌，通过专家处理扰动后的路由器嵌入获得内部激活，并对这些激活施加两个约束。

Result: 通过对3B到15B参数的MoE - LLMs进行预训练和对数万亿令牌的广泛分析，证明了ERC损失的有效性。

Conclusion: ERC损失有效，能灵活控制和定量跟踪训练期间专家专业化水平，为MoE提供有价值的见解。
...
### [454] [Multilingual Hidden Prompt Injection Attacks on LLM-Based Academic Reviewing](https://arxiv.org/abs/2512.23684)
*Panagiotis Theocharopoulos,Ajinkya Kulkarni,Mathew Magimai. -Doss*

Main category: cs.CL

TL;DR: 研究评估在学术论文中嵌入隐藏对抗性提示对大型语言模型（LLM）评审的影响，发现不同语言提示效果不同。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型用于高影响工作流程（如学术同行评审），但易受文档级隐藏提示注入攻击，需评估这种攻击的影响。

Method: 构建约500篇ICML已录用学术论文的数据集，在其中嵌入四种不同语言的语义等效指令，用LLM进行评审。

Result: 英语、日语和中文注入会使评审分数和评审决定产生显著变化，阿拉伯语注入几乎没有影响。
...
### [455] [Eliciting Behaviors in Multi-Turn Conversations](https://arxiv.org/abs/2512.23701)
*Jing Huang,Shujian Zhang,Lun Wang,Andrew Hard,Rajiv Mathews,John Lambert*

Main category: cs.CL

TL;DR: 研究大语言模型在多轮对话中的行为诱发方法，提出分析框架，统一单轮和多轮诱发，评估方法效率，强调动态基准的需求。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型行为诱发技术主要在单轮场景研究，需研究多轮对话中的行为诱发。

Method: 提供分析框架将现有方法分类，引入在线方法的多轮公式，评估三类方法在自动生成多轮测试用例中的表现，分析查询预算和成功率的权衡。

Result: 在线方法在三个任务中仅用数千次查询就能达到一定成功率，而现有多轮对话基准的静态方法发现的失败案例少甚至没有。

Conclusion: 强调行为诱发方法在多轮对话评估中的新应用，以及社区需转向动态基准。
...
### [456] [Two-Robot Computational Landscape: A Complete Characterization of Model Power in Minimal Mobile Robot Systems](https://arxiv.org/abs/2512.22770)
*Naoki Kitamura,Yuichi Sudo,Koichi Wada*

Main category: cs.RO

TL;DR: 本文首次全面刻画了两个自主机器人在各主要模型及调度器下的计算能力，揭示其与一般情况不同的图景，并用新方法得出等价和分离结果。


<details>
  <summary>Details</summary>
Motivation: 通用n - 机器人计算能力格局已基本明确，但两个机器人的确切结构仍未解决，需对其计算能力进行研究。

Method: 采用一种全新的无模拟方法推导等价和分离结果。

Result: 发现两个机器人的计算能力格局与一般情况不同，证明在完全同步下FSTA^F和LUMI^F重合，FSTA和FCOM正交。

Conclusion: 得出首个完整精确的两个机器人计算格局，凸显最小规模下协调的内在挑战。
...
### [457] [A Unified AI, Embedded, Simulation, and Mechanical Design Approach to an Autonomous Delivery Robot](https://arxiv.org/abs/2512.22408)
*Amro Gamar,Ahmed Abduljalil,Alargam Mohammed,Ali Elhenidy,Abeer Tawakol*

Main category: cs.RO

TL;DR: 本文开发了集成多学科技术的全自主配送机器人，介绍架构、设计优化及应对挑战，展示成果并强调多学科方法有效。


<details>
  <summary>Details</summary>
Motivation: 开发一个能实际应用的全自主配送机器人。

Method: 采用异构计算架构，RPi 5和ROS 2处理感知与路径规划，ESP32运行FreeRTOS进行电机控制；优化机械设计；解决资源受限平台算法优化和通信问题。

Result: 实现基于PID的确定性电机控制，通过AWS IoT监测和故障保护增强系统可靠性。

Conclusion: 多学科统一方法有效，构建出可实际部署的自主配送系统。
...
### [458] [Emergence of Human to Robot Transfer in Vision-Language-Action Models](https://arxiv.org/abs/2512.22414)
*Simar Kareer,Karl Pertsch,James Darpinian,Judy Hoffman,Danfei Xu,Sergey Levine,Chelsea Finn,Suraj Nair*

Main category: cs.RO

TL;DR: 研究利用人类视频数据训练视觉 - 语言 - 动作（VLA）模型，提出共训练方法，发现足够多样预训练可实现人类到机器人技能迁移，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型需大量多样数据集，人类视频数据丰富易获取，但单独用其训练VLA有困难，且建立人与机器人映射是挑战，受大语言模型启发，探索VLA利用人类视频数据的类似现象。

Method: 引入简单的共训练方法，对VLA在足够多场景、任务和具身形式上进行预训练。

Result: 在足够多样的机器人预训练下，方法能使仅在人类数据中出现的泛化设置下的性能接近翻倍。

Conclusion: 多样的预训练能为人类和机器人数据生成与具身无关的表征，从而实现人类到机器人的技能迁移。
...
### [459] [Active Constraint Learning in High Dimensions from Demonstrations](https://arxiv.org/abs/2512.22757)
*Zheng Qiu,Chih-Yuan Chiu,Glen Chou*

Main category: cs.RO

TL;DR: 提出迭代主动约束学习（ACL）算法，在模拟和硬件实验中优于基于随机采样的基线方法。


<details>
  <summary>Details</summary>
Motivation: 在示教学习（LfD）范式下，智能获取信息性演示轨迹以推断演示者环境中的未知约束。

Method: 迭代地在可用演示数据集上训练高斯过程（GP）来表示未知约束，利用 GP 后验查询起始/目标状态，生成信息性演示并添加到数据集。

Result: 在高维非线性动力学和未知非线性约束的模拟和硬件实验中，该方法优于基于随机采样的基线方法。

Conclusion: 所提出的迭代主动约束学习算法能从迭代生成的稀疏但信息丰富的演示中更准确地进行约束推断。
...
### [460] [The body is not there to compute: Comment on "Informational embodiment: Computational role of information structure in codes and robots" by Pitti et al](https://arxiv.org/abs/2512.22868)
*Matej Hoffmann*

Main category: cs.RO

TL;DR: 文章探讨信息与计算在理解世界中的应用，目标文章将其用于身体研究，评论者认为身体主要作用并非计算。


<details>
  <summary>Details</summary>
Motivation: 对目标文章将信息和计算应用于身体研究的观点进行探讨和反驳。

Method: 通过提出观点进行论述，未提及具体方法。

Result: 提出身体主要作用并非计算的观点。

Conclusion: 强调身体主要作用不是计算。
...
### [461] [Explainable Neural Inverse Kinematics for Obstacle-Aware Robotic Manipulation: A Comparative Analysis of IKNet Variants](https://arxiv.org/abs/2512.23312)
*Sheng-Kai Chen,Yi-Ling Tsai,Chun-Chih Chang,Yan-Chen Chen,Po-Chiang Lin*

Main category: cs.RO

TL;DR: 本文提出以可解释性为中心的工作流程，结合Shapley值归因和基于物理的避障评估，应用于机器人手臂逆运动学，证明可解释AI技术能助力学习型逆运动学。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络加速逆运动学推理，但模型不透明性与负责任AI监管的透明和安全要求矛盾。

Method: 提出可解释性工作流程，训练两个轻量级IKNet变体，用SHAP进行重要性排名，InterpretML可视化，在模拟器中进行避障测试。

Result: 定性热图显示，在姿态维度上更均匀分配重要性的架构能在不影响位置精度的情况下保持更宽安全裕度。

Conclusion: 可解释AI技术能揭示隐藏故障模式、指导架构改进和告知避障部署策略，为符合负责任AI标准的数据驱动操作提供路径。
...
### [462] [Theory of Mind for Explainable Human-Robot Interaction](https://arxiv.org/abs/2512.23482)
*Marie Bauer,Julia Gachot,Matthias Kerzel,Cornelius Weber,Stefan Wermter*

Main category: cs.RO

TL;DR: 本文探讨将人机交互中的心智理论（ToM）视为可解释人工智能（XAI）的一种形式，指出ToM应用的差距，提议将其融入XAI框架以关注用户需求。


<details>
  <summary>Details</summary>
Motivation: 现有ToM在人机交互应用中很少评估解释与机器人实际内部推理的对应程度，且当前XAI研究缺乏以用户为中心的解释。

Method: 将ToM视为一种XAI形式，通过eValuation XAI（VXAI）框架及其七个要求进行评估，并将ToM原则嵌入XAI框架。

Result: 无明确提及

Conclusion: 将ToM融入XAI框架可转变视角，优先考虑用户的信息需求和观点。
...
### [463] [Act2Goal: From World Model To General Goal-conditioned Policy](https://arxiv.org/abs/2512.23541)
*Pengfei Zhou,Liliang Chen,Shengcong Chen,Di Chen,Wenzhi Zhao,Rongjun Jin,Guanghui Ren,Jianlan Luo*

Main category: cs.RO

TL;DR: 提出Act2Goal策略，集成视觉世界模型与多尺度时间控制，实现长视野操作，零样本泛化能力强，在线适应可提升成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉目标的策略在长视野操作中因单步动作预测和缺乏任务进度建模而面临挑战。

Method: 提出Act2Goal，用世界模型生成中间视觉状态序列，引入Multi - Scale Temporal Hashing分解轨迹，通过端到端交叉注意力耦合表征和运动控制，采用后见之明目标重标记和LoRA微调实现无奖励在线适应。

Result: 实现对新物体、布局和环境的零样本泛化，在几分钟自主交互下，挑战性任务成功率从30%提升到90%。

Conclusion: 目标条件世界模型结合多尺度时间控制为鲁棒长视野操作提供必要的结构化指导。
...
### [464] [A first-order method for nonconvex-strongly-concave constrained minimax optimization](https://arxiv.org/abs/2512.22909)
*Zhaosong Lu,Sanyou Mei*

Main category: math.OC

TL;DR: 本文研究非凸 - 强凹约束极小极大问题，提出一阶增广拉格朗日方法求解，复杂度有改进。


<details>
  <summary>Details</summary>
Motivation: 研究非凸 - 强凹约束极小极大问题的求解方法。

Method: 提出一阶增广拉格朗日方法，子问题用利用强凹结构的一阶方法求解。

Result: 在合适假设下，方法找到约束极小极大问题的 ε - KKT 解的操作复杂度为 $O(\varepsilon^{-3.5}\log\varepsilon^{-1})$，优于之前结果。

Conclusion: 所提方法在求解非凸 - 强凹约束极小极大问题上有更优的操作复杂度。
...
### [465] [Clipped Gradient Methods for Nonsmooth Convex Optimization under Heavy-Tailed Noise: A Refined Analysis](https://arxiv.org/abs/2512.23178)
*Zijian Liu*

Main category: math.OC

TL;DR: 本文针对重尾噪声下的Clipped SGD提供了更精细分析，给出更快收敛速率，扩展到期望收敛并提出新下界。


<details>
  <summary>Details</summary>
Motivation: 近年来重尾噪声下的优化更符合现代机器学习任务，需对Clipped SGD进行更精细分析以得到更快收敛速率。

Method: 更好地利用Freedman不等式和更精细的裁剪误差界限改善现有方法，将精细分析扩展到期望收敛。

Result: 给出了比现有结果更快的收敛速率，扩展到期望收敛得到打破已知下界的新速率，建立了高概率和期望收敛的新下界。

Conclusion: 期望收敛的新下界与上界匹配，表明精细分析对期望收敛的最优性。
...
### [466] [Constraint programming model and biased random-key genetic algorithm for the single-machine coupled task scheduling problem with exact delays to minimize the makespan](https://arxiv.org/abs/2512.23150)
*Vítor A. Barbosa,Rafael A. Melo*

Main category: math.OC

TL;DR: 研究单机耦合任务调度问题，提出约束编程（CP）模型和偏随机键遗传算法（BRKGA），实验表明BRKGA求解效率高，CP模型长时间多线程运行效果好。


<details>
  <summary>Details</summary>
Motivation: 解决强NP难的单机耦合任务调度问题以最小化完工时间。

Method: 使用CP进行问题建模，提出BRKGA算法，该算法结合初始解生成器、定期重启和抖动以及局部搜索算法，其解码器注重效率。

Result: BRKGA能高效探索解空间，在短时间内提供高质量近似解且优于CP模型；CP模型长时间多线程运行可显著提升结果。

Conclusion: BRKGA的抖动和局部搜索组件组合能显著提升标准BRKGA结果。
...
### [467] [Deep Learning for the Multiple Optimal Stopping Problem](https://arxiv.org/abs/2512.22961)
*Mathieu Laurière,Mehdi Talbi*

Main category: math.OC

TL;DR: 提出新深度学习框架解决高维多重最优停时问题，结合动态规划原理与神经网络逼近值函数，数值实验证明方法有效可扩展。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在解决单停时问题有前景，但多重行使情况存在复杂递归依赖，解决该挑战。

Method: 结合动态规划原理与神经网络逼近值函数，显式学习值表面，先处理离散时间问题分析训练误差，再处理连续问题分析离散化误差。

Result: 在高维美式篮子期权和非线性效用最大化的数值实验中，方法对多重最优停时问题高效且可扩展。

Conclusion: 提出的方法为多重最优停时问题提供了一种有效且可扩展的解决方案。
...
### [468] [Risk-Averse Learning with Varying Risk Levels](https://arxiv.org/abs/2512.22986)
*Siyi Wang,Zifan Wang,Karl H. Johansson*

Main category: math.OC

TL;DR: 研究动态环境中不同风险水平下的风险规避在线优化问题，采用CVaR度量风险，开发算法并分析动态遗憾界，通过实验验证方法有效性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键决策中，环境和风险水平会随时间变化，需研究动态环境中不同风险水平下的风险规避在线优化。

Method: 采用函数变化度量和引入风险水平变化度量，考虑一阶和零阶两种信息设置，开发有限采样预算的风险规避学习算法。

Result: 对算法的动态遗憾界进行分析，证明其在非平稳和风险敏感环境中的适应性。
...
### [469] [Cyber Resilience in Next-Generation Networks: Threat Landscape, Theoretical Foundations, and Design Paradigms](https://arxiv.org/abs/2512.22721)
*Junaid Farooq,Quanyan Zhu*

Main category: cs.NI

TL;DR: 本书探讨网络系统变革下关键基础设施韧性的重新定义与工程实现，介绍应对风险的方法及AI作用。


<details>
  <summary>Details</summary>
Motivation: 网络系统的创新驱动变革，重新定义关键基础设施的运营和威胁状况，需重新概念化和工程化韧性以应对挑战。

Method: 全书分六章，先调查当代风险格局，再建立韧性定义和评估框架，核心探讨先进范式和实用策略，还强调AI在其中的作用。
...
### [470] [Canonical correlation regression with noisy data](https://arxiv.org/abs/2512.22697)
*Isaac Meza,Rahul Singh*

Main category: econ.EM

TL;DR: 研究数据丰富环境下的工具变量回归，通过两阶段最小二乘法与谱正则化方法估计线性模型，推导估计误差上下界并给出谱正则化使用建议。


<details>
  <summary>Details</summary>
Motivation: 在数据丰富环境中，从大量有噪声的协变量和工具变量里估计线性模型。

Method: 基于两阶段最小二乘法和谱正则化的估计器，第一阶段学习协变量和工具变量间的典型相关性，第二阶段用作回归因子。

Result: 推导估计误差的上下界，证明方法在噪声数据中的最优性。

Conclusion: 给出不同情况下应使用的谱正则化类型的建议。
...
### [471] [Causal-Policy Forest for End-to-End Policy Learning](https://arxiv.org/abs/2512.22846)
*Masahiro Kato*

Main category: econ.EM

TL;DR: 提出因果推断中策略学习的端到端算法因果策略森林，有三项优势。


<details>
  <summary>Details</summary>
Motivation: 利用观察数据进行策略学习，训练出为个体推荐最优处理的策略以最大化策略值。

Method: 证明在特定回归模型下最大化策略值等价于最小化条件平均处理效应（CATE）的均方误差，修改因果森林算法用于策略学习。

Result: 得到因果策略森林算法。

Conclusion: 该算法优势为：衔接策略学习与CATE估计；端到端训练策略；高效训练避免计算难题。
...
### [472] [Fast mixing in Ising models with a negative spectral outlier via Gaussian approximation](https://arxiv.org/abs/2512.22803)
*Dan Mikulincer,Youngtak Sohn*

Main category: math.PR

TL;DR: 研究含单个负谱异常值的Ising模型Glauber动力学混合时间，提出新方法得界及证明下界


<details>
  <summary>Details</summary>
Motivation: 现有快速混合方法在有负异常值时失效，需解决该困难

Method: 基于高斯近似开发新的协方差近似方法，结合Eldan和Chen方法

Result: 得到全相关结构的算子范数控制，修正对数Sobolev不等式和近最优混合时间界，还有稀疏图上低温反铁磁Ising模型混合时间指数下界

Conclusion: 新方法能解决负谱异常值情况，得出混合时间界
...
### [473] [IANEC: Digital Forensic Investigation of Contemporary Writers' Archives](https://arxiv.org/abs/2512.22167)
*Emmanuel Giguet*

Main category: cs.DL

TL;DR: IANEC项目由GREYC研究实验室牵头，获法国文化部资助，旨在开发数字取证调查工具以分析IMEC档案语料库。


<details>
  <summary>Details</summary>
Motivation: 当代档案机构中原生数字档案材料日益普遍，数字取证技术对处理此类档案语料库至关重要。

Method: 开发专门的数字取证调查工具以实现自动分析。
...
### [474] [Pre-review to Peer review: Pitfalls of Automating Reviews using Large Language Models](https://arxiv.org/abs/2512.22145)
*Akhil Pandey Akella,Harish Varma Siravuri,Shaurya Rohatgi*

Main category: cs.DL

TL;DR: 本文通过实验研究评估大语言模型用于学术同行评审的安全性和可靠性，发现模型作为预评审筛选工具有潜力，但也存在对齐风险。


<details>
  <summary>Details</summary>
Motivation: 多数评估大语言模型生成评审的研究未结合出版物结果评估，且自动化同行评审存在安全性等问题，因此开展此研究。

Method: 收集OpenReview上的真实评审评分，用前沿开放权重LLM生成论文评审。

Result: 模型与人类评审员相关性弱，有高估偏差和高置信度；但与出版物指标相关性更强。

Conclusion: 前沿开放权重LLM作为预评审筛选工具有潜力，指出了自动化同行评审的潜力和陷阱，并开源数据集助力相关安全框架研究。
...
### [475] [Interpretable Link Prediction in AI-Driven Cancer Research: Uncovering Co-Authorship Patterns](https://arxiv.org/abs/2512.22181)
*Shahab Mosallaie,Andrea Schiffauerova,Ashkan Ebadi*

Main category: cs.DL

TL;DR: 研究分析AI驱动的癌症研究合作网络，构建分类器预测合作模式，发现学科相似度等因素影响合作，结果可指导团队组建与政策制定。


<details>
  <summary>Details</summary>
Motivation: 癌症研究需跨学科合作，但组建有效团队有挑战，理解和预测合作模式有助于优化资源和推动研究。

Method: 以论文合著网络代表合作，利用Scopus的7738篇出版物构建36个合著网络，设计特征并构建四个机器学习分类器，用SHAP解释模型。

Result: 随机森林对三种合作类型召回率最高，学科相似度得分影响合作模式，高生产力和资历与合作中断正相关。

Conclusion: 研究结果可指导有效研究团队的组建，加强跨学科合作，为战略政策决策提供依据。
...
### [476] [AETAS: Analysis of Evolving Temporal Affect and Semantics for Legal History](https://arxiv.org/abs/2512.22196)
*Qizhi Wang*

Main category: cs.DL

TL;DR: 提出可复现的专家系统式流程，量化和可视化老贝利语料库词汇演变，实现结果复现。


<details>
  <summary>Details</summary>
Motivation: 解决数字人文语义转移研究中手工细读和不透明嵌入机制交替的问题，量化和可视化老贝利语料库的词汇演变。

Method: 将程序按年代分组，训练skip - gram嵌入，通过正交Procrustes对齐空间，测量几何位移和邻域翻转。

Result: 得到漂移幅度、语义轨迹和宽严轴上的运动三种可视化分析输出，展示法律相关概念随改革的演变。

Conclusion: 该流程可复现为可审核脚本，能在其他历史语料库中复现结果。
...
### [477] [RobustMask: Certified Robustness against Adversarial Neural Ranking Attack via Randomized Masking](https://arxiv.org/abs/2512.23307)
*Jiawei Liu,Zhuo Chen,Rui Zhu,Miaokun Chen,Yuyang Gong,Wei Lu,Xiaofeng Wang*

Main category: cs.CR

TL;DR: 提出RobustMask防御方法增强神经排序模型对抗鲁棒性，有理论证明和实验验证效果。


<details>
  <summary>Details</summary>
Motivation: 神经排序模型易受对抗操纵，现有防御方法存在泛化性差或假设过强等问题，需新防御方法。

Method: 结合预训练语言模型上下文预测能力和随机掩码平滑机制，利用排序模型成对比较能力和概率统计分析。

Result: 能为前10排名中超20%候选文档提供针对内容30%受影响的对抗扰动的认证。
...
### [478] [Practical challenges of control monitoring in frontier AI deployments](https://arxiv.org/abs/2512.22154)
*David Lindner,Charlie Griffin,Tomek Korbak,Roland S. Zimmermann,Geoffrey Irving,Sebastian Farquhar,Alan Cooney*

Main category: cs.CR

TL;DR: 分析自动化控制监控在现实部署中面临的挑战，研究不同延迟 - 安全权衡的监控形式并引入安全案例草图工具，通过案例研究探讨挑战。


<details>
  <summary>Details</summary>
Motivation: 现有工作在简化环境探索控制监控，将监控扩展到现实部署面临新动态，需解决相关挑战。

Method: 分析三种不同延迟 - 安全权衡的监控形式（同步、半同步、异步监控），引入高 - 级安全案例草图工具，通过四个未来AI部署案例研究。

Result: 确定了监督、延迟和恢复三个挑战。

Conclusion: 对解决现实中自动化控制监控AI代理的挑战有一定指导意义。
...
### [479] [Beyond Single Bugs: Benchmarking Large Language Models for Multi-Vulnerability Detection](https://arxiv.org/abs/2512.22306)
*Chinmay Pushkar,Sanchit Kabra,Dhruv Kumar,Jagat Sesh Challa*

Main category: cs.CR

TL;DR: 本文引入多漏洞检测综合基准，构建含不同数量漏洞的数据集评估五个大语言模型，发现漏洞密度增加会导致模型性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法反映真实软件复杂性，大语言模型在多标签任务中的偏差未在代码安全领域严格量化。

Method: 引入多语言多漏洞检测基准，构建含不同数量漏洞的40000个文件数据集，评估五个大语言模型。

Result: 漏洞密度增加时模型性能急剧下降，不同语言有不同失败模式。

Conclusion: 大语言模型在多漏洞检测中受漏洞密度影响大，不同语言表现有差异。
...
### [480] [LLA: Enhancing Security and Privacy for Generative Models with Logic-Locked Accelerators](https://arxiv.org/abs/2512.22307)
*You Li,Guannan Zhao,Yuhao Ju,Yunqi He,Jie Gu,Hai Zhou*

Main category: cs.CR

TL;DR: 介绍用于生成式AI模型的知识产权保护方案LLA，兼具软硬件协同，能抵御多种攻击且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: 为了解决生成式AI模型面临的如模型盗窃、损坏和信息泄露等供应链安全威胁，保护其知识产权。

Method: 软件方面，将关键位嵌入神经元触发异常并进行不变性变换；硬件方面，在AI加速器集成轻量级锁定模块，以预存密钥的加速器作为使用模型服务的许可。

Result: LLA能抵御一系列神谕引导的密钥优化攻击，7168个关键位的计算开销不到0.1%。

Conclusion: LLA是一种有效且计算开销小的生成式AI模型知识产权保护方案。
...
### [481] [Agentic AI for Cyber Resilience: A New Security Paradigm and Its System-Theoretic Foundations](https://arxiv.org/abs/2512.22883)
*Tao Li,Quanyan Zhu*

Main category: cs.CR

TL;DR: 基础模型人工智能重塑网络安全，应从预防转向智能弹性，介绍系统框架并以案例说明均衡设计有助实现弹性。


<details>
  <summary>Details</summary>
Motivation: 基础模型人工智能改变网络安全形势，传统基于静态规则等的安全架构受挑战。

Method: 将转变置于网络安全范式历史演变中；开发系统级的智能AI工作流框架；分析攻防工作流；用博弈论公式设计；进行案例研究。

Result: 博弈论公式提供统一设计语言，均衡设计可实现系统级弹性设计。

Conclusion: 应从预防为主的安全模式转向智能网络弹性，均衡设计可助力系统级弹性设计。
...
### [482] [DECEPTICON: How Dark Patterns Manipulate Web Agents](https://arxiv.org/abs/2512.22894)
*Phil Cuvin,Hao Zhu,Diyi Yang*

Main category: cs.CR

TL;DR: 本文指出暗黑模式对智能体轨迹影响大，引入DECEPTICON环境测试，发现其成功率高，现有对抗措施效果不佳，凸显需强大防御机制。
...
Method: 引入DECEPTICON环境，包含700个带暗黑模式的网页导航任务，测试不同智能体。

Result: 暗黑模式在超70%测试任务中使智能体走向恶意结果，高于人类；其有效性与模型大小和推理能力正相关，现有对抗措施效果不佳。

Conclusion: 暗黑模式是网络智能体潜在且未解决的风险，急需针对操纵性设计的强大防御措施。
...
### [483] [EquaCode: A Multi-Strategy Jailbreak Approach for Large Language Models via Equation Solving and Code Completion](https://arxiv.org/abs/2512.23173)
*Zhen Liang,Hai Huang,Zhengkui Chen*

Main category: cs.CR

TL;DR: 提出Equacode多策略越狱方法评估大语言模型鲁棒性，实验显示成功率高且多策略有协同效应。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的可信度受关注，现有越狱攻击在自然语言层面且单策略，难以全面评估模型鲁棒性。

Method: 提出Equacode，将恶意意图转化为数学问题，让模型用代码求解，利用跨领域任务复杂性转移模型对安全约束的关注。

Result: Equacode在GPT系列平均成功率91.19%，在3个先进大模型中达98.65%，单查询即可；消融实验表明多策略有强协同效应。

Conclusion: 多策略越狱方法能取得比单一策略更好的结果，可更有效评估大语言模型鲁棒性。
...
### [484] [Adaptive Trust Consensus for Blockchain IoT: Comparing RL, DRL, and MARL Against Naive, Collusive, Adaptive, Byzantine, and Sleeper Attacks](https://arxiv.org/abs/2512.22860)
*Soham Padia,Dhananjay Vaidya,Ramchandra Mangrulkar*

Main category: cs.CR

TL;DR: 提出基于信任的委托共识框架，对比三种强化学习方法抵御五种攻击，实验显示多智能体学习面对复杂攻击有优势，定时中毒攻击危害大。


<details>
  <summary>Details</summary>
Motivation: 保护区块链物联网网络免受复杂对抗攻击。

Method: 提出集成全同态加密与基于属性访问控制的信任委托共识框架，对比三种强化学习方法（RL、DRL、MARL）应对五种攻击。

Result: 在16节点模拟网络中，MARL在共谋攻击中检测效果优，DRL和MARL在自适应攻击中检测完美，都能抵御拜占庭攻击，定时中毒攻击对所有代理危害大。

Conclusion: 协调的多智能体学习在抵御区块链物联网环境中复杂信任操纵攻击方面有可衡量的优势。
...
### [485] [Multi-Agent Framework for Threat Mitigation and Resilience in AI-Based Systems](https://arxiv.org/abs/2512.23132)
*Armstrong Foundjem,Lionel Nganyewou Tidjon,Leuson Da Silva,Foutse Khomh*

Main category: cs.CR

TL;DR: 本文识别机器学习安全风险，通过多种方式分析威胁，发现未报告威胁，揭示主导TTPs等，提出需自适应、特定于ML的安全框架。


<details>
  <summary>Details</summary>
Motivation: 传统网络安全缺乏针对基础、多模态和RAG系统的特定ML威胁建模，需表征ML安全风险。

Method: 从多个来源提取威胁，分析GitHub/Python仓库，用多智能体RAG系统构建本体驱动的威胁图。

Result: 识别未报告威胁，确定主导TTPs，主要影响预训练和推理阶段，发现库中存在漏洞簇。

Conclusion: 需结合依赖卫生、威胁情报和监控的自适应ML安全框架来降低供应链和推理风险。
...
### [486] [Agentic AI for Autonomous Defense in Software Supply Chain Security: Beyond Provenance to Vulnerability Mitigation](https://arxiv.org/abs/2512.23480)
*Toqeer Ali Syed,Mohammad Riyaz Belgaum,Salman Jan,Asadullah Abdullah Khan,Saad Said Alqahtani*

Main category: cs.CR

TL;DR: 文章提出基于智能体AI的自主软件供应链安全框架，结合LLM推理、强化学习和多智能体协调，经测试效果良好，能推动供应链向主动防御转变。


<details>
  <summary>Details</summary>
Motivation: 软件供应链攻击聚焦可信开发和交付流程，现有框架无法主动识别和消除软件生产中的漏洞。

Method: 构建基于智能体AI的系统，结合LLM推理、强化学习和多智能体协调，利用LangChain和LangGraph协调安全智能体，通过MCP与CI/CD环境通信，用区块链记录。

Result: 在模拟管道和实际CI/CD集成上测试，比基于规则、仅溯源和仅强化学习的基线有更好的检测准确性、更短的缓解延迟和合理的构建时间开销。

Conclusion: 智能体AI可推动软件供应链从被动验证向主动防御转变。
...
### [487] [Certifying the Right to Be Forgotten: Primal-Dual Optimization for Sample and Label Unlearning in Vertical Federated Learning](https://arxiv.org/abs/2512.23171)
*Yu Jiang,Xindi Tong,Ziyao Liu,Xiaoxi Zhang,Kwok-Yan Lam,Chee Wei Tan*

Main category: cs.CR

TL;DR: 本文提出FedORA方法解决垂直联邦学习（VFL）中的联合遗忘问题，理论分析证明有效性，实验显示能在降低开销下达到与从头训练相近效果。


<details>
  <summary>Details</summary>
Motivation: 水平联邦学习的遗忘研究广泛，但VFL因分布式特征架构，样本和标签遗忘存在计算开销和特征依赖复杂性问题，需有效解决方法。

Method: 提出FedORA方法，将样本或标签移除转化为约束优化问题，用原始 - 对偶框架求解，引入新遗忘损失函数，使用自适应步长和非对称批量设计。

Result: 理论分析证明FedORA与从头训练的模型差异有界；在表格和图像数据集实验中，FedORA在降低计算和通信开销下，遗忘效果和效用保留与从头训练相近。

Conclusion: FedORA能有效解决VFL中的遗忘问题，在保证遗忘效果和效用保留的同时，降低计算和通信开销。
...
### [488] [Toward Trustworthy Agentic AI: A Multimodal Framework for Preventing Prompt Injection Attacks](https://arxiv.org/abs/2512.23557)
*Toqeer Ali Syed,Mishal Ateeq Almutairi,Mahmoud Abdel Moaty*

Main category: cs.CR

TL;DR: 大语言模型等使强大自治系统成为可能，但代理环境增加多模态提示注入攻击风险，本文提出跨代理多模态溯源感知防御框架，实验证明可提升检测精度等，增强系统安全性。


<details>
  <summary>Details</summary>
Motivation: 强大自治系统带来多模态提示注入攻击风险，如导致意外行为、策略违规等，需降低这些风险。

Method: 提出跨代理多模态溯源感知防御框架，包含文本和视觉清理代理及输出验证代理，由溯源账本协调，确保代理间通信遵循信任框架。

Result: 显著提升多模态注入检测精度，最小化跨代理信任泄漏，使代理执行路径稳定。

Conclusion: 该框架将溯源跟踪和验证扩展到多智能体编排，有助于建立安全、可理解和可靠的代理人工智能系统。
...
TL;DR: 本文基于AIOCJ实现了可适应的TeaStore，展示该方法优势与局限并给出未来改进方向。


<details>
  <summary>Details</summary>
Motivation: 以AIOCJ实现可适应的TeaStore，展示其在建模可适应微服务架构方面的能力。

Method: 使用AIOCJ这一编排语言对可适应的TeaStore进行建模。

Result: 展示了该方法的优势和当前局限性。

Conclusion: 为完善该范式（特别是AIOCJ语言）以更好适应现实云架构提出了未来方向。
...
TL;DR: 提出基于跟踪的临界区表征改进锁集构造，去除死锁预测器的误报和漏报，且不影响性能。


<details>
  <summary>Details</summary>
Motivation: 标准的每线程锁集构造仅考虑同一线程获取的锁，忽略其他线程事件，导致死锁分析出现误报和漏报。

Method: 给出基于跟踪的临界区表征，通过偏序关系近似该表征，改进锁集构造，并将其集成到SPDOffline扩展中。

Result: 改进的锁集构造可去除DIRK死锁预测器的误报，扩展SPDOffline死锁预测器去除漏报，扩展后的方法保持正确性且更完整。

Conclusion: 改进的锁集构造能提高死锁分析精度，且不影响标准基准测试的性能。
...
### [491] [Fancy Some Chips for Your TeaStore? Modeling the Control of an Adaptable Discrete System](https://arxiv.org/abs/2512.23496)
*Anna Gallone,Simon Bliudze,Sophie Cerf,Olga Kouchnarenko*

Main category: cs.PL

TL;DR: 本文介绍了语言Chips，它可助力设计复杂系统模型，还以应用为例展示其使用方法。


<details>
  <summary>Details</summary>
Motivation: 设计新Web应用时开发者需应对多种资源约束，期望系统具备鲁棒性以提供优质服务，因此需要一种便于设计复杂系统模型的工具。

Method: 引入Chips语言，结合控制理论和通用编程语言概念，以功能块形式描述应用。

Result: 展示了如何使用Chips系统地设计、建模和分析复杂系统项目。

Conclusion: Chips语言有助于设计由各种相互交织组件构成的模型。
...
### [492] [Integrating Wide and Deep Neural Networks with Squeeze-and-Excitation Blocks for Multi-Target Property Prediction in Additively Manufactured Fiber Reinforced Composites](https://arxiv.org/abs/2512.22397)
*Behzad Parvaresh,Rahmat K. Adesunkanmi,Adel Alaeddini*

Main category: physics.comp-ph

TL;DR: 提出集成LHS与SE - WDNN的数据高效多输入多目标学习方法预测CFRC - AM性能，结果显示误差低，可指导参数选择。


<details>
  <summary>Details</summary>
Motivation: CFRC - AM性能受工艺和材料参数影响大，全量实验测试不现实，需高效预测方法。

Method: 采用LHS指导实验，结合SE - WDNN模型，用Markforged Mark Two 3D打印机打印测试155个样本，与常用机器学习模型对比。

Result: 模型整体测试误差最低（MAPE = 12.33%），对部分目标变量优于基线模型，SHAP分析表明增强策略对力学性能影响大。

Conclusion: LHS与SE - WDNN集成可实现可解释且样本高效的多目标预测，指导CFRC - AM参数选择。
...
### [493] [PINNs for Electromagnetic Wave Propagation](https://arxiv.org/abs/2512.23396)
*Nilufer K. Bulut*

Main category: physics.comp-ph

TL;DR: 本文探讨了PINNs在电磁学中的应用，提出混合训练策略使PINNs接近FDTD精度和能量一致性，并展示了PINN模型在电磁场景中的良好结果。


<details>
  <summary>Details</summary>
Motivation: 尽管PINNs有优势，但在精度和能量指标上与FDTD相比存在不足，需要新方法使其更具竞争力。

Method: 提出混合方法，通过时间推进和因果感知加权解决因果崩溃问题，应用两阶段界面连续性损失缓解不连续性，开发基于坡印廷的局部正则化器抑制损失积累。

Result: PINN模型实现高场精度，NRMSE平均为0.09%、L²误差为1.01%，在2D PEC腔场景中相对能量失配仅0.024%。

Conclusion: PINNs在典型电磁示例中可与FDTD取得相当结果，是可行替代方案。
...
### [494] [Field strength-dependent performance variability in deep learning-based analysis of magnetic resonance imaging](https://arxiv.org/abs/2512.22176)
*Muhammad Ibtsaam Qadir,Duane Schonlau,Ulrike Dydak,Fiona R. Kolbinger*

Main category: eess.IV

TL;DR: 研究量化MRI扫描仪磁场强度对深度学习分割算法性能和泛化性的影响，不同数据集和模型结果各异，表明磁场强度影响显著。
...
Method: 将三个公开MRI数据集按扫描仪场强分层，开发三种基于nnU - Net的模型，用UMAP聚类和放射组学分析研究场强相关性能差异。

Result: 乳腺肿瘤和胰腺分割中，m - 3.0T模型表现最佳；颈椎分割在同场验证集表现最优；放射组学分析显示软组织有适度场强依赖性聚类。

Conclusion: 训练数据中的磁场强度显著影响深度学习分割模型性能，评估AI在MRI上的性能时应考虑磁场强度这一混淆因素。
...
### [495] [Super-Resolution Enhancement of Medical Images Based on Diffusion Model: An Optimization Scheme for Low-Resolution Gastric Images](https://arxiv.org/abs/2512.22209)
*Haozhe Jia*

Main category: eess.IV

TL;DR: 本文提出基于扩散的超分辨率框架提升胶囊内镜图像分辨率，方法优于双三次插值和基于GAN的方法，表明该方法在提升无创医学影像方面有前景。


<details>
  <summary>Details</summary>
Motivation: 胶囊内镜因硬件、功率和传输限制，图像分辨率低，影响早期诊断，需提升图像分辨率。

Method: 采用基于去噪扩散概率模型的SR3框架学习低分辨率到高分辨率图像的概率映射，使用HyperKvasir数据集进行训练和评估。

Result: 定量结果显示该方法显著优于双三次插值和基于GAN的超分辨率方法，如ESRGAN；定性结果显示能更好保留解剖边界、血管模式和病变结构。

Conclusion: 基于扩散的超分辨率是提升无创医学影像的有前景方法，尤其适用于图像分辨率受限的胶囊内镜。
...
### [496] [EIR: Enhanced Image Representations for Medical Report Generation](https://arxiv.org/abs/2512.23185)
*Qiang Sun,Zongcheng Ji,Yinlong Xiao,Peng Chang,Jun Yu*

Main category: eess.IV

TL;DR: 自动生成胸部X光检查报告的现存方法存在问题，本文提出EIR方法解决问题，实验证明有效。


<details>
  <summary>Details</summary>
Motivation: 减轻放射科医生压力，减少误诊风险，解决现有自动生成医学报告方法存在信息不对称和领域差距的问题。

Method: 用跨模态变压器融合元数据和图像表示，利用医学领域预训练模型编码医学图像。

Result: 在MIMIC和Open - I数据集上的实验证明所提方法有效。

Conclusion: 所提出的EIR方法能有效解决现存自动生成报告方法的问题。
...
### [497] [ReCollab: Retrieval-Augmented LLMs for Cooperative Ad-hoc Teammate Modeling](https://arxiv.org/abs/2512.22129)
*Conor Wallace,Umer Siddique,Yongcan Cao*

Main category: cs.MA

TL;DR: 本文提出基于大语言模型的Ad - hoc teamwork框架，包括Collab和ReCollab，在Overcooked环境验证了它们效果，展示了大语言模型作为行为世界模型潜力。


<details>
  <summary>Details</summary>
Motivation: 传统Ad - hoc teamwork方法在部分可观测和有限交互情况下较脆弱，大语言模型提供了灵活替代方案。

Method: 引入Collab框架，使用基于轨迹特征的行为准则对队友类型分类；扩展到ReCollab，结合检索增强生成稳定推理。

Result: 在Overcooked环境中，Collab能有效区分队友类型，ReCollab持续提升跨布局适应能力，在分类准确性和情节回报间实现帕累托最优权衡。

Conclusion: 大语言模型有作为Ad - hoc teamwork行为世界模型的潜力，在挑战性协调场景中检索基础很重要。
...
### [498] [Solving Multi-Agent Multi-Goal Path Finding Problems in Polynomial Time](https://arxiv.org/abs/2512.22171)
*Stefan Edelkamp*

Main category: cs.MA

TL;DR: 本文针对无向图中多智能体多目标任务规划问题展开研究，连续情况可接近最优解，离散情况能多项式时间求解，并实现规划器减少冲突。


<details>
  <summary>Details</summary>
Motivation: 解决无向图中多智能体多目标的任务规划问题，与常规多智能体路径规划有所不同。

Method: 连续情况求解接近最优，离散情况多项式时间求解；实现规划器，采用全局分配策略，用蚂蚁在杆等概念解决剩余冲突。

Result: 能为智能体找到无冲突的优化路线，全局分配策略大幅减少冲突。

Conclusion: 对于连续和离散的多智能体多目标任务规划问题有有效的解决方法，能找到优化路线并解决冲突。
...
### [499] [Hierarchical Pedagogical Oversight: A Multi-Agent Adversarial Framework for Reliable AI Tutoring](https://arxiv.org/abs/2512.22496)
*Saisab Sadhu,Ashim Dhor*

Main category: cs.MA

TL;DR: 大语言模型作自动化导师时存在教学推理问题，本文引入 HPO 框架，在 MRBench 数据集评估，小参数模型效果超 GPT - 4o，证明对抗推理在受限环境有用。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为自动化导师时存在教学推理失败，如验证错误答案、提供直接答案等问题。

Method: 引入 Hierarchical Pedagogical Oversight (HPO) 框架，采用结构化对抗合成用于教育评估，通过专家代理提炼对话背景，进行五幕辩论。

Result: 在 MRBench 数据集上，8B 参数模型 Macro F1 达 0.845，超过 GPT - 4o 的 0.812，且参数少 20 倍。
...
### [500] [Reinforcement Networks: novel framework for collaborative Multi-Agent Reinforcement Learning tasks](https://arxiv.org/abs/2512.22876)
*Maksim Kryzhanovskiy,Svetlana Glazyrina,Roman Ischenko,Konstantin Vorontsov*

Main category: cs.MA

TL;DR: 提出强化网络框架用于协作多智能体强化学习，在多个场景验证有效性，统一多种视角并指明研究方向。


<details>
  <summary>Details</summary>
Motivation: 解决现代AI系统中多可学习组件端到端训练问题，避免现有方法限制。

Method: 将智能体组织为有向无环图（DAG），形式化训练和推理方法，与LevelEnv概念结合。

Result: 开发的强化网络模型在多个协作多智能体场景中性能优于标准基线。

Conclusion: 强化网络统一多种视角，为可扩展、结构化多智能体强化学习研究提供基础，指明理论和实践方向。
...
### [501] [Heterogeneity in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2512.22941)
*Tianyi Hu,Zhiqiang Pu,Yuan Wang,Tenghai Qiu,Min Chen,Xin Yu*

Main category: cs.MA

TL;DR: 本文从定义、量化和利用角度系统探讨多智能体强化学习（MARL）中的异质性，提出分类、量化方法和基于异质性的算法，实验表明方法有效，能助力MARL领域发展。


<details>
  <summary>Details</summary>
Motivation: 当前MARL领域缺乏对异质性的严格定义和深入理解。

Method: 先基于智能体级建模对异质性分类并给出数学定义，再定义异质性距离概念并提出量化方法，最后设计基于异质性的多智能体动态参数共享算法。

Result: 案例研究表明方法能有效识别和量化各类智能体异质性，实验显示所提算法比其他参数共享基线有更好解释性和更强适应性。
...
### [502] [Assessing behaviour coverage in a multi-agent system simulation for autonomous vehicle testing](https://arxiv.org/abs/2512.23445)
*Manuel Franco-Vivo*

Main category: cs.MA

TL;DR: 研究聚焦自动驾驶车辆测试的多智能体系统仿真的行为覆盖分析，提出MPC行人智能体，为自动驾驶测试提供见解。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶技术发展，需综合测试方法评估其在复杂场景中的性能，研究行为覆盖分析很有必要。

Method: 定义驾驶场景和智能体交互，分析行为覆盖指标和基于覆盖的测试。

Result: 强调行为覆盖对验证自动驾驶系统有效性和鲁棒性的重要性，确定仿真框架的改进和优化方向，提出MPC行人智能体。

Conclusion: 研究有助于推进自动驾驶测试领域，通过严格测试方法提升自动驾驶车辆的安全性、可靠性和性能。
...
### [503] [The Complete Anatomy of the Madden-Julian Oscillation Revealed by Artificial Intelligence](https://arxiv.org/abs/2512.22144)
*Xiao Zhou,Yuze Sun,Jie Wu,Xiaomeng Huang*

Main category: physics.ao-ph

TL;DR: 引入‘AI - for - theory’范式，开发PhysAnchor - MJO - AE模型发现MJO生命周期的六阶段解剖图，构建新监测框架，降低误判率。


<details>
  <summary>Details</summary>
Motivation: 既有的线性投影方法（RMM指数）常混淆数学假象和物理状态，原始数据空间直接聚类受‘传播惩罚’干扰，需准确界定MJO生命周期。

Method: 引入‘AI - for - theory’范式，开发深度学习模型PhysAnchor - MJO - AE学习潜在表征以进行客观聚类。

Result: 发现MJO生命周期完整的六阶段解剖图，构建新的物理连贯监测框架，相比经典指数，降低了虚假传播和对流错置率。

Conclusion: 将AI从预测工具转变为发现工具，为从复杂系统中提取基本动力学结构提供可复制模板。
...
### [504] [Neural ocean forecasting from sparse satellite-derived observations: a case-study for SSH dynamics and altimetry data](https://arxiv.org/abs/2512.22152)
...
TL;DR: 提出基于卫星高度计稀疏数据的端到端深度学习框架用于全球海面动力短期预测，性能优于基线。


<details>
  <summary>Details</summary>
Motivation: 利用卫星高度计稀疏数据进行全球海面动力的短期预测。

Method: 基于U - Net和4DVarNet，将预测任务建模为序列到序列映射，用GLORYS12数据训练，采用多项指标评估并与Mercator Ocean产品对比。

Result: 端到端神经预测在所有提前期都优于基线，在高变率区域改善显著。

Conclusion: 端到端神经预测模型在海洋学业务中可行且有潜力，即使在数据稀疏条件下。
...
### [505] [QSAR-Guided Generative Framework for the Discovery of Synthetically Viable Odorants](https://arxiv.org/abs/2512.23080)
*Tim C. Pearce,Ahmed Ibrahim*

Main category: physics.chem-ph

TL;DR: 本文提出结合VAE与QSAR的框架，能用有限气味分子训练集生成新气味分子，模型生成分子有效且具独特性，能探索化学空间。


<details>
  <summary>Details</summary>
Motivation: 在香料和香精行业，高效确定有理想嗅觉特性结构存在挑战，生成式AI需大量分子学习，而现有训练集有限。

Method: 将变分自编码器（VAE）与定量构效关系（QSAR）模型结合，VAE从ChemBL数据库学习SMILES语法，用QSAR模型的损失项增强训练目标。

Result: 模型生成分子语法有效率达100%，独特性达94.8%；与已知气味分子的FCD约为6.96；74.4%候选分子有新核心框架。

Conclusion: 该框架能利用有限训练集生成新气味分子，有效探索化学空间。
